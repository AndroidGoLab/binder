package sounddose

import (
	"context"
	sounddoseISoundDose "github.com/xaionaro-go/aidl/android/hardware/audio/core/sounddose/ISoundDose"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorISoundDose = "android.hardware.audio.core.sounddose.ISoundDose"

const (
	TransactionISoundDoseSetOutputRs2UpperBound    = binder.FirstCallTransaction + 0
	TransactionISoundDoseGetOutputRs2UpperBound    = binder.FirstCallTransaction + 1
	TransactionISoundDoseRegisterSoundDoseCallback = binder.FirstCallTransaction + 2
)

type ISoundDose interface {
	AsBinder() binder.IBinder
	SetOutputRs2UpperBound(ctx context.Context, rs2ValueDbA float32) error
	GetOutputRs2UpperBound(ctx context.Context) (float32, error)
	RegisterSoundDoseCallback(ctx context.Context, callback sounddoseISoundDose.IHalSoundDoseCallback) error
}

const (
	ISoundDoseDefaultMaxRs2 int32 = 100
	ISoundDoseMinRs2        int32 = 80
)

type SoundDoseProxy struct {
	remote binder.IBinder
}

func NewSoundDoseProxy(
	remote binder.IBinder,
) *SoundDoseProxy {
	return &SoundDoseProxy{remote: remote}
}

func (p *SoundDoseProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ ISoundDose = (*SoundDoseProxy)(nil)

func (p *SoundDoseProxy) SetOutputRs2UpperBound(
	ctx context.Context,
	rs2ValueDbA float32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISoundDose)
	_data.WriteFloat32(rs2ValueDbA)

	_reply, _err := p.remote.Transact(ctx, TransactionISoundDoseSetOutputRs2UpperBound, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SoundDoseProxy) GetOutputRs2UpperBound(
	ctx context.Context,
) (float32, error) {
	var _result float32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISoundDose)

	_reply, _err := p.remote.Transact(ctx, TransactionISoundDoseGetOutputRs2UpperBound, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadFloat32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *SoundDoseProxy) RegisterSoundDoseCallback(
	ctx context.Context,
	callback sounddoseISoundDose.IHalSoundDoseCallback,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISoundDose)
	_data.WriteStrongBinder(callback.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, TransactionISoundDoseRegisterSoundDoseCallback, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
