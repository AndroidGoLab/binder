...
func (p *GnssNavigationMessageListenerProxy) OnGnssNavigationMessageReceived(
	...
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIGnssNavigationMessageListener, MethodIGnssNavigationMessageListenerOnGnssNavigationMessageReceived)
...
func (p *GnssNavigationMessageListenerProxy) OnStatusChanged(
	...
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIGnssNavigationMessageListener)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIGnssNavigationMessageListener, MethodIGnssNavigationMessageListenerOnStatusChanged)
	_compiledDescs := []string{
		"I",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
