...
import (
	"context"
	"fmt"
	common "github.com/AndroidGoLab/binder/android/hardware/input/common"
...
func (p *InputProcessorProxy) Classify(
	...
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIInputProcessor, MethodIInputProcessorClassify)
...
func (p *InputProcessorProxy) ResetDevice(
	...
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIInputProcessor)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIInputProcessor, MethodIInputProcessorResetDevice)
	_compiledDescs := []string{
		"I",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
