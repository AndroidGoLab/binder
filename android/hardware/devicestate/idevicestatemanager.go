package devicestate

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIDeviceStateManager = "android.hardware.devicestate.IDeviceStateManager"

const (
	TransactionIDeviceStateManagerGetDeviceStateInfo             = binder.FirstCallTransaction + 0
	TransactionIDeviceStateManagerRegisterCallback               = binder.FirstCallTransaction + 1
	TransactionIDeviceStateManagerRequestState                   = binder.FirstCallTransaction + 2
	TransactionIDeviceStateManagerCancelStateRequest             = binder.FirstCallTransaction + 3
	TransactionIDeviceStateManagerRequestBaseStateOverride       = binder.FirstCallTransaction + 4
	TransactionIDeviceStateManagerCancelBaseStateOverride        = binder.FirstCallTransaction + 5
	TransactionIDeviceStateManagerOnStateRequestOverlayDismissed = binder.FirstCallTransaction + 6
)

type IDeviceStateManager interface {
	AsBinder() binder.IBinder
	GetDeviceStateInfo(ctx context.Context) (DeviceStateInfo, error)
	RegisterCallback(ctx context.Context, callback IDeviceStateManagerCallback) (DeviceStateInfo, error)
	RequestState(ctx context.Context, token binder.IBinder, state int32, flags int32) error
	CancelStateRequest(ctx context.Context) error
	RequestBaseStateOverride(ctx context.Context, token binder.IBinder, state int32, flags int32) error
	CancelBaseStateOverride(ctx context.Context) error
	OnStateRequestOverlayDismissed(ctx context.Context, shouldCancelRequest bool) error
}

type DeviceStateManagerProxy struct {
	remote binder.IBinder
}

func NewDeviceStateManagerProxy(
	remote binder.IBinder,
) *DeviceStateManagerProxy {
	return &DeviceStateManagerProxy{remote: remote}
}

func (p *DeviceStateManagerProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IDeviceStateManager = (*DeviceStateManagerProxy)(nil)

func (p *DeviceStateManagerProxy) GetDeviceStateInfo(
	ctx context.Context,
) (DeviceStateInfo, error) {
	var _result DeviceStateInfo
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIDeviceStateManager)

	_reply, _err := p.remote.Transact(ctx, TransactionIDeviceStateManagerGetDeviceStateInfo, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	if _err = _result.UnmarshalParcel(_reply); _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *DeviceStateManagerProxy) RegisterCallback(
	ctx context.Context,
	callback IDeviceStateManagerCallback,
) (DeviceStateInfo, error) {
	var _result DeviceStateInfo
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIDeviceStateManager)
	_data.WriteStrongBinder(callback.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, TransactionIDeviceStateManagerRegisterCallback, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	if _err = _result.UnmarshalParcel(_reply); _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *DeviceStateManagerProxy) RequestState(
	ctx context.Context,
	token binder.IBinder,
	state int32,
	flags int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIDeviceStateManager)
	_data.WriteStrongBinder(token.Handle())
	_data.WriteInt32(state)
	_data.WriteInt32(flags)

	_reply, _err := p.remote.Transact(ctx, TransactionIDeviceStateManagerRequestState, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *DeviceStateManagerProxy) CancelStateRequest(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIDeviceStateManager)

	_reply, _err := p.remote.Transact(ctx, TransactionIDeviceStateManagerCancelStateRequest, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *DeviceStateManagerProxy) RequestBaseStateOverride(
	ctx context.Context,
	token binder.IBinder,
	state int32,
	flags int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIDeviceStateManager)
	_data.WriteStrongBinder(token.Handle())
	_data.WriteInt32(state)
	_data.WriteInt32(flags)

	_reply, _err := p.remote.Transact(ctx, TransactionIDeviceStateManagerRequestBaseStateOverride, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *DeviceStateManagerProxy) CancelBaseStateOverride(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIDeviceStateManager)

	_reply, _err := p.remote.Transact(ctx, TransactionIDeviceStateManagerCancelBaseStateOverride, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *DeviceStateManagerProxy) OnStateRequestOverlayDismissed(
	ctx context.Context,
	shouldCancelRequest bool,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIDeviceStateManager)
	_data.WriteBool(shouldCancelRequest)

	_reply, _err := p.remote.Transact(ctx, TransactionIDeviceStateManagerOnStateRequestOverlayDismissed, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
