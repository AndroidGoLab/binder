package earc

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIEArc = "android.hardware.tv.hdmi.earc.IEArc"

const (
	TransactionIEArcSetEArcEnabled                   = binder.FirstCallTransaction + 0
	TransactionIEArcIsEArcEnabled                    = binder.FirstCallTransaction + 1
	TransactionIEArcSetCallback                      = binder.FirstCallTransaction + 2
	TransactionIEArcGetState                         = binder.FirstCallTransaction + 3
	TransactionIEArcGetLastReportedAudioCapabilities = binder.FirstCallTransaction + 4
)

type IEArc interface {
	AsBinder() binder.IBinder
	SetEArcEnabled(ctx context.Context, enabled bool) error
	IsEArcEnabled(ctx context.Context) (bool, error)
	SetCallback(ctx context.Context, callback IEArcCallback) error
	GetState(ctx context.Context, portId int32) (IEArcStatus, error)
	GetLastReportedAudioCapabilities(ctx context.Context, portId int32) ([]byte, error)
}

type EArcProxy struct {
	remote binder.IBinder
}

func NewEArcProxy(
	remote binder.IBinder,
) *EArcProxy {
	return &EArcProxy{remote: remote}
}

func (p *EArcProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IEArc = (*EArcProxy)(nil)

func (p *EArcProxy) SetEArcEnabled(
	ctx context.Context,
	enabled bool,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIEArc)
	_data.WriteBool(enabled)

	_reply, _err := p.remote.Transact(ctx, TransactionIEArcSetEArcEnabled, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *EArcProxy) IsEArcEnabled(
	ctx context.Context,
) (bool, error) {
	var _result bool
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIEArc)

	_reply, _err := p.remote.Transact(ctx, TransactionIEArcIsEArcEnabled, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadBool()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *EArcProxy) SetCallback(
	ctx context.Context,
	callback IEArcCallback,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIEArc)
	_data.WriteStrongBinder(callback.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, TransactionIEArcSetCallback, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *EArcProxy) GetState(
	ctx context.Context,
	portId int32,
) (IEArcStatus, error) {
	var _result IEArcStatus
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIEArc)
	_data.WriteInt32(portId)

	_reply, _err := p.remote.Transact(ctx, TransactionIEArcGetState, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_raw, _err := _reply.ReadPaddedByte()
	if _err != nil {
		return _result, _err
	}
	_result = IEArcStatus(_raw)
	return _result, nil
}

func (p *EArcProxy) GetLastReportedAudioCapabilities(
	ctx context.Context,
	portId int32,
) ([]byte, error) {
	var _result []byte
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIEArc)
	_data.WriteInt32(portId)

	_reply, _err := p.remote.Transact(ctx, TransactionIEArcGetLastReportedAudioCapabilities, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_count, _err := _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}

	if _count >= 0 {
		_result = make([]byte, _count)
		for _i := int32(0); _i < _count; _i++ {
			_result[_i], _err = _reply.ReadPaddedByte()
			if _err != nil {
				return _result, _err
			}
		}
	}
	return _result, nil
}
