package biometrics

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIAuthService = "android.hardware.biometrics.IAuthService"

const (
	TransactionIAuthServiceCreateTestSession                     = binder.FirstCallTransaction + 0
	TransactionIAuthServiceGetSensorProperties                   = binder.FirstCallTransaction + 1
	TransactionIAuthServiceGetUiPackage                          = binder.FirstCallTransaction + 2
	TransactionIAuthServiceAuthenticate                          = binder.FirstCallTransaction + 3
	TransactionIAuthServiceCancelAuthentication                  = binder.FirstCallTransaction + 4
	TransactionIAuthServiceCanAuthenticate                       = binder.FirstCallTransaction + 5
	TransactionIAuthServiceGetLastAuthenticationTime             = binder.FirstCallTransaction + 6
	TransactionIAuthServiceHasEnrolledBiometrics                 = binder.FirstCallTransaction + 7
	TransactionIAuthServiceRegisterEnabledOnKeyguardCallback     = binder.FirstCallTransaction + 8
	TransactionIAuthServiceRegisterAuthenticationStateListener   = binder.FirstCallTransaction + 9
	TransactionIAuthServiceUnregisterAuthenticationStateListener = binder.FirstCallTransaction + 10
	TransactionIAuthServiceInvalidateAuthenticatorIds            = binder.FirstCallTransaction + 11
	TransactionIAuthServiceGetAuthenticatorIds                   = binder.FirstCallTransaction + 12
	TransactionIAuthServiceResetLockoutTimeBound                 = binder.FirstCallTransaction + 13
	TransactionIAuthServiceResetLockout                          = binder.FirstCallTransaction + 14
	TransactionIAuthServiceGetButtonLabel                        = binder.FirstCallTransaction + 15
	TransactionIAuthServiceGetPromptMessage                      = binder.FirstCallTransaction + 16
	TransactionIAuthServiceGetSettingName                        = binder.FirstCallTransaction + 17
)

type IAuthService interface {
	AsBinder() binder.IBinder
	CreateTestSession(ctx context.Context, sensorId int32, callback ITestSessionCallback, opPackageName string) (ITestSession, error)
	GetSensorProperties(ctx context.Context, opPackageName string) ([]SensorPropertiesInternal, error)
	GetUiPackage(ctx context.Context) (string, error)
	Authenticate(ctx context.Context, token binder.IBinder, sessionId int64, userId int32, receiver IBiometricServiceReceiver, opPackageName string, promptInfo PromptInfo) (int64, error)
	CancelAuthentication(ctx context.Context, token binder.IBinder, opPackageName string, requestId int64) error
	CanAuthenticate(ctx context.Context, opPackageName string, userId int32, authenticators int32) (int32, error)
	GetLastAuthenticationTime(ctx context.Context, userId int32, authenticators int32) (int64, error)
	HasEnrolledBiometrics(ctx context.Context, userId int32, opPackageName string) (bool, error)
	RegisterEnabledOnKeyguardCallback(ctx context.Context, callback IBiometricEnabledOnKeyguardCallback) error
	RegisterAuthenticationStateListener(ctx context.Context, listener AuthenticationStateListener) error
	UnregisterAuthenticationStateListener(ctx context.Context, listener AuthenticationStateListener) error
	InvalidateAuthenticatorIds(ctx context.Context, userId int32, fromSensorId int32, callback IInvalidationCallback) error
	GetAuthenticatorIds(ctx context.Context, userId int32) ([]int64, error)
	ResetLockoutTimeBound(ctx context.Context, token binder.IBinder, opPackageName string, fromSensorId int32, userId int32, hardwareAuthToken []byte) error
	ResetLockout(ctx context.Context, userId int32, hardwareAuthToken []byte) error
	GetButtonLabel(ctx context.Context, userId int32, opPackageName string, authenticators int32) (interface{}, error)
	GetPromptMessage(ctx context.Context, userId int32, opPackageName string, authenticators int32) (interface{}, error)
	GetSettingName(ctx context.Context, userId int32, opPackageName string, authenticators int32) (interface{}, error)
}

type AuthServiceProxy struct {
	remote binder.IBinder
}

func NewAuthServiceProxy(
	remote binder.IBinder,
) *AuthServiceProxy {
	return &AuthServiceProxy{remote: remote}
}

func (p *AuthServiceProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IAuthService = (*AuthServiceProxy)(nil)

func (p *AuthServiceProxy) CreateTestSession(
	ctx context.Context,
	sensorId int32,
	callback ITestSessionCallback,
	opPackageName string,
) (ITestSession, error) {
	var _result ITestSession
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteInt32(sensorId)
	_data.WriteStrongBinder(callback.AsBinder().Handle())
	_data.WriteString16(opPackageName)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "createTestSession"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_handle, _err := _reply.ReadStrongBinder()
	if _err != nil {
		return _result, _err
	}
	_result = NewTestSessionProxy(binder.NewProxyBinder(p.remote.Transport(), _handle))
	return _result, nil
}

func (p *AuthServiceProxy) GetSensorProperties(
	ctx context.Context,
	opPackageName string,
) ([]SensorPropertiesInternal, error) {
	var _result []SensorPropertiesInternal
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteString16(opPackageName)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "getSensorProperties"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_count, _err := _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}

	if _count >= 0 {
		_result = make([]SensorPropertiesInternal, _count)
		for _i := int32(0); _i < _count; _i++ {
			if _err = _result[_i].UnmarshalParcel(_reply); _err != nil {
				return _result, _err
			}
		}
	}
	return _result, nil
}

func (p *AuthServiceProxy) GetUiPackage(
	ctx context.Context,
) (string, error) {
	var _result string
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "getUiPackage"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadString16()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *AuthServiceProxy) Authenticate(
	ctx context.Context,
	token binder.IBinder,
	sessionId int64,
	userId int32,
	receiver IBiometricServiceReceiver,
	opPackageName string,
	promptInfo PromptInfo,
) (int64, error) {
	var _result int64
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteStrongBinder(token.Handle())
	_data.WriteInt64(sessionId)
	_data.WriteInt32(userId)
	_data.WriteStrongBinder(receiver.AsBinder().Handle())
	_data.WriteString16(opPackageName)
	if _err := promptInfo.MarshalParcel(_data); _err != nil {
		return _result, _err
	}

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "authenticate"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt64()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *AuthServiceProxy) CancelAuthentication(
	ctx context.Context,
	token binder.IBinder,
	opPackageName string,
	requestId int64,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteStrongBinder(token.Handle())
	_data.WriteString16(opPackageName)
	_data.WriteInt64(requestId)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "cancelAuthentication"), 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *AuthServiceProxy) CanAuthenticate(
	ctx context.Context,
	opPackageName string,
	userId int32,
	authenticators int32,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteString16(opPackageName)
	_data.WriteInt32(userId)
	_data.WriteInt32(authenticators)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "canAuthenticate"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *AuthServiceProxy) GetLastAuthenticationTime(
	ctx context.Context,
	userId int32,
	authenticators int32,
) (int64, error) {
	var _result int64
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteInt32(userId)
	_data.WriteInt32(authenticators)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "getLastAuthenticationTime"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt64()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *AuthServiceProxy) HasEnrolledBiometrics(
	ctx context.Context,
	userId int32,
	opPackageName string,
) (bool, error) {
	var _result bool
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteInt32(userId)
	_data.WriteString16(opPackageName)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "hasEnrolledBiometrics"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadBool()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *AuthServiceProxy) RegisterEnabledOnKeyguardCallback(
	ctx context.Context,
	callback IBiometricEnabledOnKeyguardCallback,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteStrongBinder(callback.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "registerEnabledOnKeyguardCallback"), 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *AuthServiceProxy) RegisterAuthenticationStateListener(
	ctx context.Context,
	listener AuthenticationStateListener,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteStrongBinder(listener.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "registerAuthenticationStateListener"), 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *AuthServiceProxy) UnregisterAuthenticationStateListener(
	ctx context.Context,
	listener AuthenticationStateListener,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteStrongBinder(listener.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "unregisterAuthenticationStateListener"), 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *AuthServiceProxy) InvalidateAuthenticatorIds(
	ctx context.Context,
	userId int32,
	fromSensorId int32,
	callback IInvalidationCallback,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteInt32(userId)
	_data.WriteInt32(fromSensorId)
	_data.WriteStrongBinder(callback.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "invalidateAuthenticatorIds"), 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *AuthServiceProxy) GetAuthenticatorIds(
	ctx context.Context,
	userId int32,
) ([]int64, error) {
	var _result []int64
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "getAuthenticatorIds"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_count, _err := _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}

	if _count >= 0 {
		_result = make([]int64, _count)
		for _i := int32(0); _i < _count; _i++ {
			_result[_i], _err = _reply.ReadInt64()
			if _err != nil {
				return _result, _err
			}
		}
	}
	return _result, nil
}

func (p *AuthServiceProxy) ResetLockoutTimeBound(
	ctx context.Context,
	token binder.IBinder,
	opPackageName string,
	fromSensorId int32,
	userId int32,
	hardwareAuthToken []byte,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteStrongBinder(token.Handle())
	_data.WriteString16(opPackageName)
	_data.WriteInt32(fromSensorId)
	_data.WriteInt32(userId)
	if hardwareAuthToken == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(hardwareAuthToken)))
		for _, _item := range hardwareAuthToken {
			_data.WritePaddedByte(_item)
		}
	}

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "resetLockoutTimeBound"), 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *AuthServiceProxy) ResetLockout(
	ctx context.Context,
	userId int32,
	hardwareAuthToken []byte,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteInt32(userId)
	if hardwareAuthToken == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(hardwareAuthToken)))
		for _, _item := range hardwareAuthToken {
			_data.WritePaddedByte(_item)
		}
	}

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "resetLockout"), 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *AuthServiceProxy) GetButtonLabel(
	ctx context.Context,
	userId int32,
	opPackageName string,
	authenticators int32,
) (interface{}, error) {
	var _result interface{}
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteInt32(userId)
	_data.WriteString16(opPackageName)
	_data.WriteInt32(authenticators)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "getButtonLabel"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	return _result, nil
}

func (p *AuthServiceProxy) GetPromptMessage(
	ctx context.Context,
	userId int32,
	opPackageName string,
	authenticators int32,
) (interface{}, error) {
	var _result interface{}
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteInt32(userId)
	_data.WriteString16(opPackageName)
	_data.WriteInt32(authenticators)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "getPromptMessage"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	return _result, nil
}

func (p *AuthServiceProxy) GetSettingName(
	ctx context.Context,
	userId int32,
	opPackageName string,
	authenticators int32,
) (interface{}, error) {
	var _result interface{}
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAuthService)
	_data.WriteInt32(userId)
	_data.WriteString16(opPackageName)
	_data.WriteInt32(authenticators)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIAuthService, "getSettingName"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	return _result, nil
}
