package sensors

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorISensorsCallback = "android.hardware.sensors.ISensorsCallback"

const (
	TransactionISensorsCallbackOnDynamicSensorsConnected    = binder.FirstCallTransaction + 0
	TransactionISensorsCallbackOnDynamicSensorsDisconnected = binder.FirstCallTransaction + 1
)

type ISensorsCallback interface {
	AsBinder() binder.IBinder
	OnDynamicSensorsConnected(ctx context.Context, sensorInfos []SensorInfo) error
	OnDynamicSensorsDisconnected(ctx context.Context, sensorHandles []int32) error
}

type SensorsCallbackProxy struct {
	remote binder.IBinder
}

func NewSensorsCallbackProxy(
	remote binder.IBinder,
) *SensorsCallbackProxy {
	return &SensorsCallbackProxy{remote: remote}
}

func (p *SensorsCallbackProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ ISensorsCallback = (*SensorsCallbackProxy)(nil)

func (p *SensorsCallbackProxy) OnDynamicSensorsConnected(
	ctx context.Context,
	sensorInfos []SensorInfo,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISensorsCallback)
	if sensorInfos == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(sensorInfos)))
		for _, _item := range sensorInfos {
			if _err := _item.MarshalParcel(_data); _err != nil {
				return _err
			}
		}
	}

	_reply, _err := p.remote.Transact(ctx, TransactionISensorsCallbackOnDynamicSensorsConnected, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SensorsCallbackProxy) OnDynamicSensorsDisconnected(
	ctx context.Context,
	sensorHandles []int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISensorsCallback)
	if sensorHandles == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(sensorHandles)))
		for _, _item := range sensorHandles {
			_data.WriteInt32(_item)
		}
	}

	_reply, _err := p.remote.Transact(ctx, TransactionISensorsCallbackOnDynamicSensorsDisconnected, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
