...
func (p *PacProxyManagerProxy) AddListener(
	...
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIPacProxyManager, MethodIPacProxyManagerAddListener)
...
func (p *PacProxyManagerProxy) RemoveListener(
	...
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIPacProxyManager)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIPacProxyManager, MethodIPacProxyManagerRemoveListener)
	_compiledDescs := []string{
		"Landroid/net/IPacProxyInstalledListener;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
