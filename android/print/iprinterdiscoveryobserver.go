...
func (p *PrinterDiscoveryObserverProxy) OnPrintersAdded(
	...
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIPrinterDiscoveryObserver, MethodIPrinterDiscoveryObserverOnPrintersAdded)
...
func (p *PrinterDiscoveryObserverProxy) OnPrintersRemoved(
	...
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIPrinterDiscoveryObserver)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIPrinterDiscoveryObserver, MethodIPrinterDiscoveryObserverOnPrintersRemoved)
	_compiledDescs := []string{
		"Landroid/content/pm/ParceledListSlice;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
