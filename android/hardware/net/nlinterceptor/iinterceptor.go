package nlinterceptor

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIInterceptor = "android.hardware.net.nlinterceptor.IInterceptor"

const (
	TransactionIInterceptorCreateSocket     = binder.FirstCallTransaction + 0
	TransactionIInterceptorCloseSocket      = binder.FirstCallTransaction + 1
	TransactionIInterceptorSubscribeGroup   = binder.FirstCallTransaction + 2
	TransactionIInterceptorUnsubscribeGroup = binder.FirstCallTransaction + 3
)

type IInterceptor interface {
	AsBinder() binder.IBinder
	CreateSocket(ctx context.Context, nlFamily int32, clientNlPid int32, clientName string) (InterceptedSocket, error)
	CloseSocket(ctx context.Context, handle InterceptedSocket) error
	SubscribeGroup(ctx context.Context, handle InterceptedSocket, nlGroup int32) error
	UnsubscribeGroup(ctx context.Context, handle InterceptedSocket, nlGroup int32) error
}

type InterceptorProxy struct {
	remote binder.IBinder
}

func NewInterceptorProxy(
	remote binder.IBinder,
) *InterceptorProxy {
	return &InterceptorProxy{remote: remote}
}

func (p *InterceptorProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IInterceptor = (*InterceptorProxy)(nil)

func (p *InterceptorProxy) CreateSocket(
	ctx context.Context,
	nlFamily int32,
	clientNlPid int32,
	clientName string,
) (InterceptedSocket, error) {
	var _result InterceptedSocket
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInterceptor)
	_data.WriteInt32(nlFamily)
	_data.WriteInt32(clientNlPid)
	_data.WriteString16(clientName)

	_reply, _err := p.remote.Transact(ctx, TransactionIInterceptorCreateSocket, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	if _err = _result.UnmarshalParcel(_reply); _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *InterceptorProxy) CloseSocket(
	ctx context.Context,
	handle InterceptedSocket,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInterceptor)
	if _err := handle.MarshalParcel(_data); _err != nil {
		return _err
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIInterceptorCloseSocket, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *InterceptorProxy) SubscribeGroup(
	ctx context.Context,
	handle InterceptedSocket,
	nlGroup int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInterceptor)
	if _err := handle.MarshalParcel(_data); _err != nil {
		return _err
	}
	_data.WriteInt32(nlGroup)

	_reply, _err := p.remote.Transact(ctx, TransactionIInterceptorSubscribeGroup, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *InterceptorProxy) UnsubscribeGroup(
	ctx context.Context,
	handle InterceptedSocket,
	nlGroup int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInterceptor)
	if _err := handle.MarshalParcel(_data); _err != nil {
		return _err
	}
	_data.WriteInt32(nlGroup)

	_reply, _err := p.remote.Transact(ctx, TransactionIInterceptorUnsubscribeGroup, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
