...
func (p *CameraProviderProxy) SetCallback(
	...
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorICameraProvider, MethodICameraProviderSetCallback)
	_compiledDescs := []string{
		"Landroid/hardware/camera/provider/ICameraProviderCallback;",
	}
...
func (p *CameraProviderProxy) GetCameraDeviceInterface(
	...
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorICameraProvider)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorICameraProvider, MethodICameraProviderGetCameraDeviceInterface)
	_compiledDescs := []string{
		"Ljava/lang/String;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
...
func (p *CameraProviderProxy) NotifyDeviceStateChange(
	...
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorICameraProvider)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorICameraProvider, MethodICameraProviderNotifyDeviceStateChange)
	_compiledDescs := []string{
		"J",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
...
func (p *CameraProviderProxy) IsConcurrentStreamCombinationSupported(
	...
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorICameraProvider)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorICameraProvider, MethodICameraProviderIsConcurrentStreamCombinationSupported)
	_compiledDescs := []string{
		"[Landroid/hardware/camera/provider/CameraIdAndStreamCombination;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		if configs == nil {
			_data.WriteInt32(-1)
...
func (s *CameraProviderStub) OnTransaction(
	...
	case TransactionICameraProviderGetCameraDeviceInterface:
		...
		_result, _err := s.Impl.GetCameraDeviceInterface(ctx, _arg_cameraDeviceName)
		_reply := parcel.New()
		if _err != nil {
			binder.WriteStatus(_reply, _err)
			return _reply, nil
	...
	case TransactionICameraProviderNotifyDeviceStateChange:
		...
		_err = s.Impl.NotifyDeviceStateChange(ctx, _arg_deviceState)
		_reply := parcel.New()
		if _err != nil {
			binder.WriteStatus(_reply, _err)
			return _reply, nil
