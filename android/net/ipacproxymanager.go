package net

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIPacProxyManager = "android.net.IPacProxyManager"

const (
	TransactionIPacProxyManagerAddListener              = binder.FirstCallTransaction + 0
	TransactionIPacProxyManagerRemoveListener           = binder.FirstCallTransaction + 1
	TransactionIPacProxyManagerSetCurrentProxyScriptUrl = binder.FirstCallTransaction + 2
)

type IPacProxyManager interface {
	AsBinder() binder.IBinder
	AddListener(ctx context.Context, listener IPacProxyInstalledListener) error
	RemoveListener(ctx context.Context, listener IPacProxyInstalledListener) error
	SetCurrentProxyScriptUrl(ctx context.Context, proxyInfo interface{}) error
}

type PacProxyManagerProxy struct {
	remote binder.IBinder
}

func NewPacProxyManagerProxy(
	remote binder.IBinder,
) *PacProxyManagerProxy {
	return &PacProxyManagerProxy{remote: remote}
}

func (p *PacProxyManagerProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IPacProxyManager = (*PacProxyManagerProxy)(nil)

func (p *PacProxyManagerProxy) AddListener(
	ctx context.Context,
	listener IPacProxyInstalledListener,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIPacProxyManager)
	_data.WriteStrongBinder(listener.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, TransactionIPacProxyManagerAddListener, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *PacProxyManagerProxy) RemoveListener(
	ctx context.Context,
	listener IPacProxyInstalledListener,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIPacProxyManager)
	_data.WriteStrongBinder(listener.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, TransactionIPacProxyManagerRemoveListener, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *PacProxyManagerProxy) SetCurrentProxyScriptUrl(
	ctx context.Context,
	proxyInfo interface{},
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIPacProxyManager)

	_reply, _err := p.remote.Transact(ctx, TransactionIPacProxyManagerSetCurrentProxyScriptUrl, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
