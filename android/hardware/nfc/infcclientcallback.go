package nfc

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorINfcClientCallback = "android.hardware.nfc.INfcClientCallback"

const (
	TransactionINfcClientCallbackSendData  = binder.FirstCallTransaction + 0
	TransactionINfcClientCallbackSendEvent = binder.FirstCallTransaction + 1
)

type INfcClientCallback interface {
	AsBinder() binder.IBinder
	SendData(ctx context.Context, data []byte) error
	SendEvent(ctx context.Context, event NfcEvent, status NfcStatus) error
}

type NfcClientCallbackProxy struct {
	remote binder.IBinder
}

func NewNfcClientCallbackProxy(
	remote binder.IBinder,
) *NfcClientCallbackProxy {
	return &NfcClientCallbackProxy{remote: remote}
}

func (p *NfcClientCallbackProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ INfcClientCallback = (*NfcClientCallbackProxy)(nil)

func (p *NfcClientCallbackProxy) SendData(
	ctx context.Context,
	data []byte,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorINfcClientCallback)
	if data == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(data)))
		for _, _item := range data {
			_data.WritePaddedByte(_item)
		}
	}

	_reply, _err := p.remote.Transact(ctx, TransactionINfcClientCallbackSendData, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *NfcClientCallbackProxy) SendEvent(
	ctx context.Context,
	event NfcEvent,
	status NfcStatus,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorINfcClientCallback)
	_data.WriteInt32(int32(event))
	_data.WriteInt32(int32(status))

	_reply, _err := p.remote.Transact(ctx, TransactionINfcClientCallbackSendEvent, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
