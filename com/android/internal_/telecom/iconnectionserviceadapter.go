package telecom

import (
	"context"
	"fmt"
	net "github.com/AndroidGoLab/binder/android/net"
	os "github.com/AndroidGoLab/binder/android/os"
	androidTelecom "github.com/AndroidGoLab/binder/android/telecom"
	"github.com/AndroidGoLab/binder/binder"
	"github.com/AndroidGoLab/binder/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIConnectionServiceAdapter = "com.android.internal.telecom.IConnectionServiceAdapter"

const (
	TransactionIConnectionServiceAdapterHandleCreateConnectionComplete   = binder.FirstCallTransaction + 0
	TransactionIConnectionServiceAdapterHandleCreateConferenceComplete   = binder.FirstCallTransaction + 1
	TransactionIConnectionServiceAdapterSetActive                        = binder.FirstCallTransaction + 2
	TransactionIConnectionServiceAdapterSetRinging                       = binder.FirstCallTransaction + 3
	TransactionIConnectionServiceAdapterSetDialing                       = binder.FirstCallTransaction + 4
	TransactionIConnectionServiceAdapterSetPulling                       = binder.FirstCallTransaction + 5
	TransactionIConnectionServiceAdapterSetDisconnected                  = binder.FirstCallTransaction + 6
	TransactionIConnectionServiceAdapterSetOnHold                        = binder.FirstCallTransaction + 7
	TransactionIConnectionServiceAdapterSetRingbackRequested             = binder.FirstCallTransaction + 8
	TransactionIConnectionServiceAdapterSetConnectionCapabilities        = binder.FirstCallTransaction + 9
	TransactionIConnectionServiceAdapterSetConnectionProperties          = binder.FirstCallTransaction + 10
	TransactionIConnectionServiceAdapterSetIsConferenced                 = binder.FirstCallTransaction + 11
	TransactionIConnectionServiceAdapterSetConferenceMergeFailed         = binder.FirstCallTransaction + 12
	TransactionIConnectionServiceAdapterAddConferenceCall                = binder.FirstCallTransaction + 13
	TransactionIConnectionServiceAdapterRemoveCall                       = binder.FirstCallTransaction + 14
	TransactionIConnectionServiceAdapterOnPostDialWait                   = binder.FirstCallTransaction + 15
	TransactionIConnectionServiceAdapterOnPostDialChar                   = binder.FirstCallTransaction + 16
	TransactionIConnectionServiceAdapterQueryRemoteConnectionServices    = binder.FirstCallTransaction + 17
	TransactionIConnectionServiceAdapterSetVideoProvider                 = binder.FirstCallTransaction + 18
	TransactionIConnectionServiceAdapterSetVideoState                    = binder.FirstCallTransaction + 19
	TransactionIConnectionServiceAdapterSetIsVoipAudioMode               = binder.FirstCallTransaction + 20
	TransactionIConnectionServiceAdapterSetStatusHints                   = binder.FirstCallTransaction + 21
	TransactionIConnectionServiceAdapterSetAddress                       = binder.FirstCallTransaction + 22
	TransactionIConnectionServiceAdapterSetCallerDisplayName             = binder.FirstCallTransaction + 23
	TransactionIConnectionServiceAdapterSetConferenceableConnections     = binder.FirstCallTransaction + 24
	TransactionIConnectionServiceAdapterAddExistingConnection            = binder.FirstCallTransaction + 25
	TransactionIConnectionServiceAdapterPutExtras                        = binder.FirstCallTransaction + 26
	TransactionIConnectionServiceAdapterRemoveExtras                     = binder.FirstCallTransaction + 27
	TransactionIConnectionServiceAdapterSetAudioRoute                    = binder.FirstCallTransaction + 28
	TransactionIConnectionServiceAdapterRequestCallEndpointChange        = binder.FirstCallTransaction + 29
	TransactionIConnectionServiceAdapterOnConnectionEvent                = binder.FirstCallTransaction + 30
	TransactionIConnectionServiceAdapterOnRttInitiationSuccess           = binder.FirstCallTransaction + 31
	TransactionIConnectionServiceAdapterOnRttInitiationFailure           = binder.FirstCallTransaction + 32
	TransactionIConnectionServiceAdapterOnRttSessionRemotelyTerminated   = binder.FirstCallTransaction + 33
	TransactionIConnectionServiceAdapterOnRemoteRttRequest               = binder.FirstCallTransaction + 34
	TransactionIConnectionServiceAdapterOnPhoneAccountChanged            = binder.FirstCallTransaction + 35
	TransactionIConnectionServiceAdapterOnConnectionServiceFocusReleased = binder.FirstCallTransaction + 36
	TransactionIConnectionServiceAdapterResetConnectionTime              = binder.FirstCallTransaction + 37
	TransactionIConnectionServiceAdapterSetConferenceState               = binder.FirstCallTransaction + 38
	TransactionIConnectionServiceAdapterSetCallDirection                 = binder.FirstCallTransaction + 39
	TransactionIConnectionServiceAdapterQueryLocation                    = binder.FirstCallTransaction + 40
)

const (
	MethodIConnectionServiceAdapterHandleCreateConnectionComplete   = "handleCreateConnectionComplete"
	MethodIConnectionServiceAdapterHandleCreateConferenceComplete   = "handleCreateConferenceComplete"
	MethodIConnectionServiceAdapterSetActive                        = "setActive"
	MethodIConnectionServiceAdapterSetRinging                       = "setRinging"
	MethodIConnectionServiceAdapterSetDialing                       = "setDialing"
	MethodIConnectionServiceAdapterSetPulling                       = "setPulling"
	MethodIConnectionServiceAdapterSetDisconnected                  = "setDisconnected"
	MethodIConnectionServiceAdapterSetOnHold                        = "setOnHold"
	MethodIConnectionServiceAdapterSetRingbackRequested             = "setRingbackRequested"
	MethodIConnectionServiceAdapterSetConnectionCapabilities        = "setConnectionCapabilities"
	MethodIConnectionServiceAdapterSetConnectionProperties          = "setConnectionProperties"
	MethodIConnectionServiceAdapterSetIsConferenced                 = "setIsConferenced"
	MethodIConnectionServiceAdapterSetConferenceMergeFailed         = "setConferenceMergeFailed"
	MethodIConnectionServiceAdapterAddConferenceCall                = "addConferenceCall"
	MethodIConnectionServiceAdapterRemoveCall                       = "removeCall"
	MethodIConnectionServiceAdapterOnPostDialWait                   = "onPostDialWait"
	MethodIConnectionServiceAdapterOnPostDialChar                   = "onPostDialChar"
	MethodIConnectionServiceAdapterQueryRemoteConnectionServices    = "queryRemoteConnectionServices"
	MethodIConnectionServiceAdapterSetVideoProvider                 = "setVideoProvider"
	MethodIConnectionServiceAdapterSetVideoState                    = "setVideoState"
	MethodIConnectionServiceAdapterSetIsVoipAudioMode               = "setIsVoipAudioMode"
	MethodIConnectionServiceAdapterSetStatusHints                   = "setStatusHints"
	MethodIConnectionServiceAdapterSetAddress                       = "setAddress"
	MethodIConnectionServiceAdapterSetCallerDisplayName             = "setCallerDisplayName"
	MethodIConnectionServiceAdapterSetConferenceableConnections     = "setConferenceableConnections"
	MethodIConnectionServiceAdapterAddExistingConnection            = "addExistingConnection"
	MethodIConnectionServiceAdapterPutExtras                        = "putExtras"
	MethodIConnectionServiceAdapterRemoveExtras                     = "removeExtras"
	MethodIConnectionServiceAdapterSetAudioRoute                    = "setAudioRoute"
	MethodIConnectionServiceAdapterRequestCallEndpointChange        = "requestCallEndpointChange"
	MethodIConnectionServiceAdapterOnConnectionEvent                = "onConnectionEvent"
	MethodIConnectionServiceAdapterOnRttInitiationSuccess           = "onRttInitiationSuccess"
	MethodIConnectionServiceAdapterOnRttInitiationFailure           = "onRttInitiationFailure"
	MethodIConnectionServiceAdapterOnRttSessionRemotelyTerminated   = "onRttSessionRemotelyTerminated"
	MethodIConnectionServiceAdapterOnRemoteRttRequest               = "onRemoteRttRequest"
	MethodIConnectionServiceAdapterOnPhoneAccountChanged            = "onPhoneAccountChanged"
	MethodIConnectionServiceAdapterOnConnectionServiceFocusReleased = "onConnectionServiceFocusReleased"
	MethodIConnectionServiceAdapterResetConnectionTime              = "resetConnectionTime"
	MethodIConnectionServiceAdapterSetConferenceState               = "setConferenceState"
	MethodIConnectionServiceAdapterSetCallDirection                 = "setCallDirection"
	MethodIConnectionServiceAdapterQueryLocation                    = "queryLocation"
)

type IConnectionServiceAdapter interface {
	AsBinder() binder.IBinder
	HandleCreateConnectionComplete(ctx context.Context, callId string, request androidTelecom.ConnectionRequest, connection androidTelecom.ParcelableConnection, sessionInfo any) error
	HandleCreateConferenceComplete(ctx context.Context, callId string, request androidTelecom.ConnectionRequest, connection androidTelecom.ParcelableConference, sessionInfo any) error
	SetActive(ctx context.Context, callId string, sessionInfo any) error
	SetRinging(ctx context.Context, callId string, sessionInfo any) error
	SetDialing(ctx context.Context, callId string, sessionInfo any) error
	SetPulling(ctx context.Context, callId string, sessionInfo any) error
	SetDisconnected(ctx context.Context, callId string, disconnectCause androidTelecom.DisconnectCause, sessionInfo any) error
	SetOnHold(ctx context.Context, callId string, sessionInfo any) error
	SetRingbackRequested(ctx context.Context, callId string, ringing bool, sessionInfo any) error
	SetConnectionCapabilities(ctx context.Context, callId string, connectionCapabilities int32, sessionInfo any) error
	SetConnectionProperties(ctx context.Context, callId string, connectionProperties int32, sessionInfo any) error
	SetIsConferenced(ctx context.Context, callId string, conferenceCallId string, sessionInfo any) error
	SetConferenceMergeFailed(ctx context.Context, callId string, sessionInfo any) error
	AddConferenceCall(ctx context.Context, callId string, conference androidTelecom.ParcelableConference, sessionInfo any) error
	RemoveCall(ctx context.Context, callId string, sessionInfo any) error
	OnPostDialWait(ctx context.Context, callId string, remaining string, sessionInfo any) error
	OnPostDialChar(ctx context.Context, callId string, nextChar uint16, sessionInfo any) error
	QueryRemoteConnectionServices(ctx context.Context, callback RemoteServiceCallback, sessionInfo any) error
	SetVideoProvider(ctx context.Context, callId string, videoProvider IVideoProvider, sessionInfo any) error
	SetVideoState(ctx context.Context, callId string, videoState int32, sessionInfo any) error
	SetIsVoipAudioMode(ctx context.Context, callId string, isVoip bool, sessionInfo any) error
	SetStatusHints(ctx context.Context, callId string, statusHints androidTelecom.StatusHints, sessionInfo any) error
	SetAddress(ctx context.Context, callId string, address net.Uri, presentation int32, sessionInfo any) error
	SetCallerDisplayName(ctx context.Context, callId string, callerDisplayName string, presentation int32, sessionInfo any) error
	SetConferenceableConnections(ctx context.Context, callId string, conferenceableCallIds []string, sessionInfo any) error
	AddExistingConnection(ctx context.Context, callId string, connection androidTelecom.ParcelableConnection, sessionInfo any) error
	PutExtras(ctx context.Context, callId string, extras os.Bundle, sessionInfo any) error
	RemoveExtras(ctx context.Context, callId string, keys []string, sessionInfo any) error
	SetAudioRoute(ctx context.Context, callId string, audioRoute int32, bluetoothAddress string, sessionInfo any) error
	RequestCallEndpointChange(ctx context.Context, callId string, endpoint androidTelecom.CallEndpoint, callback os.ResultReceiver, sessionInfo any) error
	OnConnectionEvent(ctx context.Context, callId string, event string, extras os.Bundle, sessionInfo any) error
	OnRttInitiationSuccess(ctx context.Context, callId string, sessionInfo any) error
	OnRttInitiationFailure(ctx context.Context, callId string, reason int32, sessionInfo any) error
	OnRttSessionRemotelyTerminated(ctx context.Context, callId string, sessionInfo any) error
	OnRemoteRttRequest(ctx context.Context, callId string, sessionInfo any) error
	OnPhoneAccountChanged(ctx context.Context, callId string, pHandle androidTelecom.PhoneAccountHandle, sessionInfo any) error
	OnConnectionServiceFocusReleased(ctx context.Context, sessionInfo any) error
	ResetConnectionTime(ctx context.Context, callIdi string, sessionInfo any) error
	SetConferenceState(ctx context.Context, callId string, isConference bool, sessionInfo any) error
	SetCallDirection(ctx context.Context, callId string, direction int32, sessionInfo any) error
	QueryLocation(ctx context.Context, callId string, timeoutMillis int64, provider string, callback os.ResultReceiver, sessionInfo any) error
}

type ConnectionServiceAdapterProxy struct {
	Remote binder.IBinder
}

func NewConnectionServiceAdapterProxy(
	remote binder.IBinder,
) *ConnectionServiceAdapterProxy {
	return &ConnectionServiceAdapterProxy{Remote: remote}
}

func (p *ConnectionServiceAdapterProxy) AsBinder() binder.IBinder {
	return p.Remote
}

var _ IConnectionServiceAdapter = (*ConnectionServiceAdapterProxy)(nil)

func (p *ConnectionServiceAdapterProxy) HandleCreateConnectionComplete(
	ctx context.Context,
	callId string,
	request androidTelecom.ConnectionRequest,
	connection androidTelecom.ParcelableConnection,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterHandleCreateConnectionComplete)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Landroid/telecom/ConnectionRequest;",
		"Landroid/telecom/ParcelableConnection;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(1)
		if _err := request.MarshalParcel(_data); _err != nil {
			return _err
		}
		_data.WriteInt32(1)
		if _err := connection.MarshalParcel(_data); _err != nil {
			return _err
		}
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(1)
				if _err := request.MarshalParcel(_data); _err != nil {
					return _err
				}
			case 2:
				_data.WriteInt32(1)
				if _err := connection.MarshalParcel(_data); _err != nil {
					return _err
				}
			case 3:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterHandleCreateConnectionComplete)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterHandleCreateConnectionComplete, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) HandleCreateConferenceComplete(
	ctx context.Context,
	callId string,
	request androidTelecom.ConnectionRequest,
	connection androidTelecom.ParcelableConference,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterHandleCreateConferenceComplete)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Landroid/telecom/ConnectionRequest;",
		"Landroid/telecom/ParcelableConference;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(1)
		if _err := request.MarshalParcel(_data); _err != nil {
			return _err
		}
		_data.WriteInt32(1)
		if _err := connection.MarshalParcel(_data); _err != nil {
			return _err
		}
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(1)
				if _err := request.MarshalParcel(_data); _err != nil {
					return _err
				}
			case 2:
				_data.WriteInt32(1)
				if _err := connection.MarshalParcel(_data); _err != nil {
					return _err
				}
			case 3:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterHandleCreateConferenceComplete)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterHandleCreateConferenceComplete, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetActive(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetActive)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetActive)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetActive, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetRinging(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetRinging)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetRinging)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetRinging, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetDialing(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetDialing)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetDialing)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetDialing, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetPulling(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetPulling)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetPulling)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetPulling, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetDisconnected(
	ctx context.Context,
	callId string,
	disconnectCause androidTelecom.DisconnectCause,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetDisconnected)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Landroid/telecom/DisconnectCause;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(1)
		if _err := disconnectCause.MarshalParcel(_data); _err != nil {
			return _err
		}
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(1)
				if _err := disconnectCause.MarshalParcel(_data); _err != nil {
					return _err
				}
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetDisconnected)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetDisconnected, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetOnHold(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetOnHold)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetOnHold)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetOnHold, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetRingbackRequested(
	ctx context.Context,
	callId string,
	ringing bool,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetRingbackRequested)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Z",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteBool(ringing)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteBool(ringing)
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetRingbackRequested)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetRingbackRequested, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetConnectionCapabilities(
	ctx context.Context,
	callId string,
	connectionCapabilities int32,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetConnectionCapabilities)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"I",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(connectionCapabilities)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(connectionCapabilities)
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetConnectionCapabilities)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetConnectionCapabilities, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetConnectionProperties(
	ctx context.Context,
	callId string,
	connectionProperties int32,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetConnectionProperties)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"I",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(connectionProperties)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(connectionProperties)
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetConnectionProperties)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetConnectionProperties, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetIsConferenced(
	ctx context.Context,
	callId string,
	conferenceCallId string,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetIsConferenced)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Ljava/lang/String;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteString16(conferenceCallId)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteString16(conferenceCallId)
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetIsConferenced)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetIsConferenced, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetConferenceMergeFailed(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetConferenceMergeFailed)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetConferenceMergeFailed)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetConferenceMergeFailed, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) AddConferenceCall(
	ctx context.Context,
	callId string,
	conference androidTelecom.ParcelableConference,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterAddConferenceCall)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Landroid/telecom/ParcelableConference;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(1)
		if _err := conference.MarshalParcel(_data); _err != nil {
			return _err
		}
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(1)
				if _err := conference.MarshalParcel(_data); _err != nil {
					return _err
				}
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterAddConferenceCall)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterAddConferenceCall, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) RemoveCall(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterRemoveCall)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterRemoveCall)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterRemoveCall, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) OnPostDialWait(
	ctx context.Context,
	callId string,
	remaining string,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnPostDialWait)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Ljava/lang/String;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteString16(remaining)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteString16(remaining)
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnPostDialWait)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnPostDialWait, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) OnPostDialChar(
	ctx context.Context,
	callId string,
	nextChar uint16,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnPostDialChar)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"C",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(int32(nextChar))
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(int32(nextChar))
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnPostDialChar)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnPostDialChar, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) QueryRemoteConnectionServices(
	ctx context.Context,
	callback RemoteServiceCallback,
	sessionInfo any,
) error {
	_identity := p.Remote.Identity()
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterQueryRemoteConnectionServices)
	_compiledDescs := []string{
		"Lcom/android/internal/telecom/RemoteServiceCallback;",
		"Ljava/lang/String;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		binder.WriteBinderToParcel(ctx, _data, callback.AsBinder(), p.Remote.Transport())
		_data.WriteString16(_identity.PackageName)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				binder.WriteBinderToParcel(ctx, _data, callback.AsBinder(), p.Remote.Transport())
			case 1:
				_data.WriteString16(_identity.PackageName)
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterQueryRemoteConnectionServices)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterQueryRemoteConnectionServices, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetVideoProvider(
	ctx context.Context,
	callId string,
	videoProvider IVideoProvider,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetVideoProvider)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Lcom/android/internal/telecom/IVideoProvider;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		binder.WriteBinderToParcel(ctx, _data, videoProvider.AsBinder(), p.Remote.Transport())
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				binder.WriteBinderToParcel(ctx, _data, videoProvider.AsBinder(), p.Remote.Transport())
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetVideoProvider)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetVideoProvider, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetVideoState(
	ctx context.Context,
	callId string,
	videoState int32,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetVideoState)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"I",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(videoState)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(videoState)
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetVideoState)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetVideoState, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetIsVoipAudioMode(
	ctx context.Context,
	callId string,
	isVoip bool,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetIsVoipAudioMode)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Z",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteBool(isVoip)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteBool(isVoip)
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetIsVoipAudioMode)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetIsVoipAudioMode, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetStatusHints(
	ctx context.Context,
	callId string,
	statusHints androidTelecom.StatusHints,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetStatusHints)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Landroid/telecom/StatusHints;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(1)
		if _err := statusHints.MarshalParcel(_data); _err != nil {
			return _err
		}
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(1)
				if _err := statusHints.MarshalParcel(_data); _err != nil {
					return _err
				}
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetStatusHints)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetStatusHints, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetAddress(
	ctx context.Context,
	callId string,
	address net.Uri,
	presentation int32,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetAddress)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Landroid/net/Uri;",
		"I",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(1)
		if _err := address.MarshalParcel(_data); _err != nil {
			return _err
		}
		_data.WriteInt32(presentation)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(1)
				if _err := address.MarshalParcel(_data); _err != nil {
					return _err
				}
			case 2:
				_data.WriteInt32(presentation)
			case 3:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetAddress)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetAddress, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetCallerDisplayName(
	ctx context.Context,
	callId string,
	callerDisplayName string,
	presentation int32,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetCallerDisplayName)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Ljava/lang/String;",
		"I",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteString16(callerDisplayName)
		_data.WriteInt32(presentation)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteString16(callerDisplayName)
			case 2:
				_data.WriteInt32(presentation)
			case 3:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetCallerDisplayName)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetCallerDisplayName, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetConferenceableConnections(
	ctx context.Context,
	callId string,
	conferenceableCallIds []string,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetConferenceableConnections)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Ljava/util/List;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		if conferenceableCallIds == nil {
			_data.WriteInt32(-1)
		} else {
			_data.WriteInt32(int32(len(conferenceableCallIds)))
			for _, _item := range conferenceableCallIds {
				_data.WriteString16(_item)
			}
		}
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				if conferenceableCallIds == nil {
					_data.WriteInt32(-1)
				} else {
					_data.WriteInt32(int32(len(conferenceableCallIds)))
					for _, _item := range conferenceableCallIds {
						_data.WriteString16(_item)
					}
				}
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetConferenceableConnections)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetConferenceableConnections, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) AddExistingConnection(
	ctx context.Context,
	callId string,
	connection androidTelecom.ParcelableConnection,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterAddExistingConnection)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Landroid/telecom/ParcelableConnection;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(1)
		if _err := connection.MarshalParcel(_data); _err != nil {
			return _err
		}
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(1)
				if _err := connection.MarshalParcel(_data); _err != nil {
					return _err
				}
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterAddExistingConnection)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterAddExistingConnection, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) PutExtras(
	ctx context.Context,
	callId string,
	extras os.Bundle,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterPutExtras)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Landroid/os/Bundle;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(1)
		if _err := extras.MarshalParcel(_data); _err != nil {
			return _err
		}
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(1)
				if _err := extras.MarshalParcel(_data); _err != nil {
					return _err
				}
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterPutExtras)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterPutExtras, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) RemoveExtras(
	ctx context.Context,
	callId string,
	keys []string,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterRemoveExtras)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Ljava/util/List;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		if keys == nil {
			_data.WriteInt32(-1)
		} else {
			_data.WriteInt32(int32(len(keys)))
			for _, _item := range keys {
				_data.WriteString16(_item)
			}
		}
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				if keys == nil {
					_data.WriteInt32(-1)
				} else {
					_data.WriteInt32(int32(len(keys)))
					for _, _item := range keys {
						_data.WriteString16(_item)
					}
				}
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterRemoveExtras)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterRemoveExtras, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetAudioRoute(
	ctx context.Context,
	callId string,
	audioRoute int32,
	bluetoothAddress string,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetAudioRoute)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"I",
		"Ljava/lang/String;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(audioRoute)
		_data.WriteString16(bluetoothAddress)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(audioRoute)
			case 2:
				_data.WriteString16(bluetoothAddress)
			case 3:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetAudioRoute)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetAudioRoute, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) RequestCallEndpointChange(
	ctx context.Context,
	callId string,
	endpoint androidTelecom.CallEndpoint,
	callback os.ResultReceiver,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterRequestCallEndpointChange)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Landroid/telecom/CallEndpoint;",
		"Landroid/os/ResultReceiver;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(1)
		if _err := endpoint.MarshalParcel(_data); _err != nil {
			return _err
		}
		_data.WriteInt32(1)
		if _err := callback.MarshalParcel(_data); _err != nil {
			return _err
		}
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(1)
				if _err := endpoint.MarshalParcel(_data); _err != nil {
					return _err
				}
			case 2:
				_data.WriteInt32(1)
				if _err := callback.MarshalParcel(_data); _err != nil {
					return _err
				}
			case 3:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterRequestCallEndpointChange)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterRequestCallEndpointChange, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) OnConnectionEvent(
	ctx context.Context,
	callId string,
	event string,
	extras os.Bundle,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnConnectionEvent)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Ljava/lang/String;",
		"Landroid/os/Bundle;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteString16(event)
		_data.WriteInt32(1)
		if _err := extras.MarshalParcel(_data); _err != nil {
			return _err
		}
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteString16(event)
			case 2:
				_data.WriteInt32(1)
				if _err := extras.MarshalParcel(_data); _err != nil {
					return _err
				}
			case 3:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnConnectionEvent)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnConnectionEvent, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) OnRttInitiationSuccess(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnRttInitiationSuccess)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnRttInitiationSuccess)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnRttInitiationSuccess, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) OnRttInitiationFailure(
	ctx context.Context,
	callId string,
	reason int32,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnRttInitiationFailure)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"I",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(reason)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(reason)
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnRttInitiationFailure)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnRttInitiationFailure, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) OnRttSessionRemotelyTerminated(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnRttSessionRemotelyTerminated)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnRttSessionRemotelyTerminated)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnRttSessionRemotelyTerminated, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) OnRemoteRttRequest(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnRemoteRttRequest)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnRemoteRttRequest)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnRemoteRttRequest, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) OnPhoneAccountChanged(
	ctx context.Context,
	callId string,
	pHandle androidTelecom.PhoneAccountHandle,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnPhoneAccountChanged)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Landroid/telecom/PhoneAccountHandle;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(1)
		if _err := pHandle.MarshalParcel(_data); _err != nil {
			return _err
		}
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(1)
				if _err := pHandle.MarshalParcel(_data); _err != nil {
					return _err
				}
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnPhoneAccountChanged)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnPhoneAccountChanged, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) OnConnectionServiceFocusReleased(
	ctx context.Context,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnConnectionServiceFocusReleased)
	_compiledDescs := []string{
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnConnectionServiceFocusReleased)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterOnConnectionServiceFocusReleased, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) ResetConnectionTime(
	ctx context.Context,
	callIdi string,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterResetConnectionTime)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callIdi)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callIdi)
			case 1:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterResetConnectionTime)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterResetConnectionTime, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetConferenceState(
	ctx context.Context,
	callId string,
	isConference bool,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetConferenceState)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"Z",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteBool(isConference)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteBool(isConference)
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetConferenceState)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetConferenceState, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) SetCallDirection(
	ctx context.Context,
	callId string,
	direction int32,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetCallDirection)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"I",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt32(direction)
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt32(direction)
			case 2:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetCallDirection)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterSetCallDirection, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

func (p *ConnectionServiceAdapterProxy) QueryLocation(
	ctx context.Context,
	callId string,
	timeoutMillis int64,
	provider string,
	callback os.ResultReceiver,
	sessionInfo any,
) error {
	_data := parcel.New()
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConnectionServiceAdapter)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterQueryLocation)
	_compiledDescs := []string{
		"Ljava/lang/String;",
		"J",
		"Ljava/lang/String;",
		"Landroid/os/ResultReceiver;",
		"LSession/Info;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteString16(callId)
		_data.WriteInt64(timeoutMillis)
		_data.WriteString16(provider)
		_data.WriteInt32(1)
		if _err := callback.MarshalParcel(_data); _err != nil {
			return _err
		}
		// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
	} else {
		_paramMap := binder.MatchParamsToSignature(_compiledDescs, _sig)
		for _, _pi := range _paramMap {
			switch _pi {
			case 0:
				_data.WriteString16(callId)
			case 1:
				_data.WriteInt64(timeoutMillis)
			case 2:
				_data.WriteString16(provider)
			case 3:
				_data.WriteInt32(1)
				if _err := callback.MarshalParcel(_data); _err != nil {
					return _err
				}
			case 4:
				// WARNING: param sessionInfo (type any) cannot be serialized — type not resolved
			}
		}
	}

	_code, _err := p.Remote.ResolveCode(ctx, DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterQueryLocation)
	if _err != nil {
		return fmt.Errorf("resolving %s.%s: %w", DescriptorIConnectionServiceAdapter, MethodIConnectionServiceAdapterQueryLocation, _err)
	}

	_, _err = p.Remote.Transact(ctx, _code, binder.FlagOneway, _data)
	return _err
}

// ConnectionServiceAdapterStub dispatches incoming binder transactions
// to a typed IConnectionServiceAdapter implementation.
type ConnectionServiceAdapterStub struct {
	Impl      IConnectionServiceAdapter
	Transport binder.VersionAwareTransport
}

var _ binder.TransactionReceiver = (*ConnectionServiceAdapterStub)(nil)

func (s *ConnectionServiceAdapterStub) Descriptor() string {
	return DescriptorIConnectionServiceAdapter
}

func (s *ConnectionServiceAdapterStub) OnTransaction(
	ctx context.Context,
	code binder.TransactionCode,
	_data *parcel.Parcel,
) (*parcel.Parcel, error) {
	if _, _err := _data.ReadInterfaceToken(); _err != nil {
		return nil, _err
	}

	switch code {
	case TransactionIConnectionServiceAdapterHandleCreateConnectionComplete:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_request androidTelecom.ConnectionRequest
		{
			_nullInd, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _nullInd != 0 {
				if _err = _arg_request.UnmarshalParcel(_data); _err != nil {
					return nil, _err
				}
			}
		}
		var _arg_connection androidTelecom.ParcelableConnection
		{
			_nullInd, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _nullInd != 0 {
				if _err = _arg_connection.UnmarshalParcel(_data); _err != nil {
					return nil, _err
				}
			}
		}
		var _arg_sessionInfo any
		_err = s.Impl.HandleCreateConnectionComplete(ctx, _arg_callId, _arg_request, _arg_connection, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterHandleCreateConferenceComplete:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_request androidTelecom.ConnectionRequest
		{
			_nullInd, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _nullInd != 0 {
				if _err = _arg_request.UnmarshalParcel(_data); _err != nil {
					return nil, _err
				}
			}
		}
		var _arg_connection androidTelecom.ParcelableConference
		{
			_nullInd, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _nullInd != 0 {
				if _err = _arg_connection.UnmarshalParcel(_data); _err != nil {
					return nil, _err
				}
			}
		}
		var _arg_sessionInfo any
		_err = s.Impl.HandleCreateConferenceComplete(ctx, _arg_callId, _arg_request, _arg_connection, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetActive:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetActive(ctx, _arg_callId, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetRinging:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetRinging(ctx, _arg_callId, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetDialing:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetDialing(ctx, _arg_callId, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetPulling:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetPulling(ctx, _arg_callId, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetDisconnected:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_disconnectCause androidTelecom.DisconnectCause
		{
			_nullInd, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _nullInd != 0 {
				if _err = _arg_disconnectCause.UnmarshalParcel(_data); _err != nil {
					return nil, _err
				}
			}
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetDisconnected(ctx, _arg_callId, _arg_disconnectCause, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetOnHold:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetOnHold(ctx, _arg_callId, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetRingbackRequested:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		_arg_ringing, _err := _data.ReadBool()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetRingbackRequested(ctx, _arg_callId, _arg_ringing, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetConnectionCapabilities:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		_arg_connectionCapabilities, _err := _data.ReadInt32()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetConnectionCapabilities(ctx, _arg_callId, _arg_connectionCapabilities, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetConnectionProperties:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		_arg_connectionProperties, _err := _data.ReadInt32()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetConnectionProperties(ctx, _arg_callId, _arg_connectionProperties, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetIsConferenced:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		_arg_conferenceCallId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetIsConferenced(ctx, _arg_callId, _arg_conferenceCallId, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetConferenceMergeFailed:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetConferenceMergeFailed(ctx, _arg_callId, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterAddConferenceCall:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_conference androidTelecom.ParcelableConference
		{
			_nullInd, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _nullInd != 0 {
				if _err = _arg_conference.UnmarshalParcel(_data); _err != nil {
					return nil, _err
				}
			}
		}
		var _arg_sessionInfo any
		_err = s.Impl.AddConferenceCall(ctx, _arg_callId, _arg_conference, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterRemoveCall:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.RemoveCall(ctx, _arg_callId, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterOnPostDialWait:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		_arg_remaining, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.OnPostDialWait(ctx, _arg_callId, _arg_remaining, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterOnPostDialChar:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		_raw_nextChar, _err := _data.ReadInt32()
		if _err != nil {
			return nil, _err
		}
		_arg_nextChar := uint16(_raw_nextChar)
		var _arg_sessionInfo any
		_err = s.Impl.OnPostDialChar(ctx, _arg_callId, _arg_nextChar, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterQueryRemoteConnectionServices:
		var _arg_callback RemoteServiceCallback
		{
			_callbackHandle, _err := _data.ReadStrongBinder()
			if _err != nil {
				return nil, _err
			}
			_arg_callback = NewRemoteServiceCallbackProxy(binder.NewProxyBinder(s.Transport, binder.CallerIdentity{}, _callbackHandle))
		}
		if _, _err := _data.ReadString16(); _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err := s.Impl.QueryRemoteConnectionServices(ctx, _arg_callback, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetVideoProvider:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_videoProvider IVideoProvider
		{
			_videoProviderHandle, _err := _data.ReadStrongBinder()
			if _err != nil {
				return nil, _err
			}
			_arg_videoProvider = NewVideoProviderProxy(binder.NewProxyBinder(s.Transport, binder.CallerIdentity{}, _videoProviderHandle))
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetVideoProvider(ctx, _arg_callId, _arg_videoProvider, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetVideoState:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		_arg_videoState, _err := _data.ReadInt32()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetVideoState(ctx, _arg_callId, _arg_videoState, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetIsVoipAudioMode:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		_arg_isVoip, _err := _data.ReadBool()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetIsVoipAudioMode(ctx, _arg_callId, _arg_isVoip, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetStatusHints:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_statusHints androidTelecom.StatusHints
		{
			_nullInd, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _nullInd != 0 {
				if _err = _arg_statusHints.UnmarshalParcel(_data); _err != nil {
					return nil, _err
				}
			}
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetStatusHints(ctx, _arg_callId, _arg_statusHints, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetAddress:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_address net.Uri
		{
			_nullInd, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _nullInd != 0 {
				if _err = _arg_address.UnmarshalParcel(_data); _err != nil {
					return nil, _err
				}
			}
		}
		_arg_presentation, _err := _data.ReadInt32()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetAddress(ctx, _arg_callId, _arg_address, _arg_presentation, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetCallerDisplayName:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		_arg_callerDisplayName, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		_arg_presentation, _err := _data.ReadInt32()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetCallerDisplayName(ctx, _arg_callId, _arg_callerDisplayName, _arg_presentation, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetConferenceableConnections:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_conferenceableCallIds []string
		{
			_count, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _count > 1000000 {
				return nil, fmt.Errorf("array count too large: %d", _count)
			}
			if _count >= 0 {
				_arg_conferenceableCallIds = make([]string, _count)
				for _i := int32(0); _i < _count; _i++ {
					_arg_conferenceableCallIds[_i], _err = _data.ReadString16()
					if _err != nil {
						return nil, _err
					}
				}
			}
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetConferenceableConnections(ctx, _arg_callId, _arg_conferenceableCallIds, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterAddExistingConnection:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_connection androidTelecom.ParcelableConnection
		{
			_nullInd, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _nullInd != 0 {
				if _err = _arg_connection.UnmarshalParcel(_data); _err != nil {
					return nil, _err
				}
			}
		}
		var _arg_sessionInfo any
		_err = s.Impl.AddExistingConnection(ctx, _arg_callId, _arg_connection, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterPutExtras:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_extras os.Bundle
		{
			_nullInd, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _nullInd != 0 {
				if _err = _arg_extras.UnmarshalParcel(_data); _err != nil {
					return nil, _err
				}
			}
		}
		var _arg_sessionInfo any
		_err = s.Impl.PutExtras(ctx, _arg_callId, _arg_extras, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterRemoveExtras:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_keys []string
		{
			_count, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _count > 1000000 {
				return nil, fmt.Errorf("array count too large: %d", _count)
			}
			if _count >= 0 {
				_arg_keys = make([]string, _count)
				for _i := int32(0); _i < _count; _i++ {
					_arg_keys[_i], _err = _data.ReadString16()
					if _err != nil {
						return nil, _err
					}
				}
			}
		}
		var _arg_sessionInfo any
		_err = s.Impl.RemoveExtras(ctx, _arg_callId, _arg_keys, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetAudioRoute:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		_arg_audioRoute, _err := _data.ReadInt32()
		if _err != nil {
			return nil, _err
		}
		_arg_bluetoothAddress, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetAudioRoute(ctx, _arg_callId, _arg_audioRoute, _arg_bluetoothAddress, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterRequestCallEndpointChange:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_endpoint androidTelecom.CallEndpoint
		{
			_nullInd, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _nullInd != 0 {
				if _err = _arg_endpoint.UnmarshalParcel(_data); _err != nil {
					return nil, _err
				}
			}
		}
		var _arg_callback os.ResultReceiver
		{
			_nullInd, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _nullInd != 0 {
				if _err = _arg_callback.UnmarshalParcel(_data); _err != nil {
					return nil, _err
				}
			}
		}
		var _arg_sessionInfo any
		_err = s.Impl.RequestCallEndpointChange(ctx, _arg_callId, _arg_endpoint, _arg_callback, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterOnConnectionEvent:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		_arg_event, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_extras os.Bundle
		{
			_nullInd, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _nullInd != 0 {
				if _err = _arg_extras.UnmarshalParcel(_data); _err != nil {
					return nil, _err
				}
			}
		}
		var _arg_sessionInfo any
		_err = s.Impl.OnConnectionEvent(ctx, _arg_callId, _arg_event, _arg_extras, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterOnRttInitiationSuccess:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.OnRttInitiationSuccess(ctx, _arg_callId, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterOnRttInitiationFailure:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		_arg_reason, _err := _data.ReadInt32()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.OnRttInitiationFailure(ctx, _arg_callId, _arg_reason, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterOnRttSessionRemotelyTerminated:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.OnRttSessionRemotelyTerminated(ctx, _arg_callId, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterOnRemoteRttRequest:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.OnRemoteRttRequest(ctx, _arg_callId, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterOnPhoneAccountChanged:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_pHandle androidTelecom.PhoneAccountHandle
		{
			_nullInd, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _nullInd != 0 {
				if _err = _arg_pHandle.UnmarshalParcel(_data); _err != nil {
					return nil, _err
				}
			}
		}
		var _arg_sessionInfo any
		_err = s.Impl.OnPhoneAccountChanged(ctx, _arg_callId, _arg_pHandle, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterOnConnectionServiceFocusReleased:
		var _arg_sessionInfo any
		_err := s.Impl.OnConnectionServiceFocusReleased(ctx, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterResetConnectionTime:
		_arg_callIdi, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.ResetConnectionTime(ctx, _arg_callIdi, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetConferenceState:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		_arg_isConference, _err := _data.ReadBool()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetConferenceState(ctx, _arg_callId, _arg_isConference, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterSetCallDirection:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		_arg_direction, _err := _data.ReadInt32()
		if _err != nil {
			return nil, _err
		}
		var _arg_sessionInfo any
		_err = s.Impl.SetCallDirection(ctx, _arg_callId, _arg_direction, _arg_sessionInfo)
		return nil, _err
	case TransactionIConnectionServiceAdapterQueryLocation:
		_arg_callId, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		_arg_timeoutMillis, _err := _data.ReadInt64()
		if _err != nil {
			return nil, _err
		}
		_arg_provider, _err := _data.ReadString16()
		if _err != nil {
			return nil, _err
		}
		var _arg_callback os.ResultReceiver
		{
			_nullInd, _err := _data.ReadInt32()
			if _err != nil {
				return nil, _err
			}
			if _nullInd != 0 {
				if _err = _arg_callback.UnmarshalParcel(_data); _err != nil {
					return nil, _err
				}
			}
		}
		var _arg_sessionInfo any
		_err = s.Impl.QueryLocation(ctx, _arg_callId, _arg_timeoutMillis, _arg_provider, _arg_callback, _arg_sessionInfo)
		return nil, _err
	default:
		return nil, fmt.Errorf("unknown transaction code %d", code)
	}
}

// IConnectionServiceAdapterServer is the server-side interface that user implementations
// provide to NewConnectionServiceAdapterStub. It contains only the business methods,
// without AsBinder (which is provided by the stub itself).
type IConnectionServiceAdapterServer interface {
	HandleCreateConnectionComplete(ctx context.Context, callId string, request androidTelecom.ConnectionRequest, connection androidTelecom.ParcelableConnection, sessionInfo any) error
	HandleCreateConferenceComplete(ctx context.Context, callId string, request androidTelecom.ConnectionRequest, connection androidTelecom.ParcelableConference, sessionInfo any) error
	SetActive(ctx context.Context, callId string, sessionInfo any) error
	SetRinging(ctx context.Context, callId string, sessionInfo any) error
	SetDialing(ctx context.Context, callId string, sessionInfo any) error
	SetPulling(ctx context.Context, callId string, sessionInfo any) error
	SetDisconnected(ctx context.Context, callId string, disconnectCause androidTelecom.DisconnectCause, sessionInfo any) error
	SetOnHold(ctx context.Context, callId string, sessionInfo any) error
	SetRingbackRequested(ctx context.Context, callId string, ringing bool, sessionInfo any) error
	SetConnectionCapabilities(ctx context.Context, callId string, connectionCapabilities int32, sessionInfo any) error
	SetConnectionProperties(ctx context.Context, callId string, connectionProperties int32, sessionInfo any) error
	SetIsConferenced(ctx context.Context, callId string, conferenceCallId string, sessionInfo any) error
	SetConferenceMergeFailed(ctx context.Context, callId string, sessionInfo any) error
	AddConferenceCall(ctx context.Context, callId string, conference androidTelecom.ParcelableConference, sessionInfo any) error
	RemoveCall(ctx context.Context, callId string, sessionInfo any) error
	OnPostDialWait(ctx context.Context, callId string, remaining string, sessionInfo any) error
	OnPostDialChar(ctx context.Context, callId string, nextChar uint16, sessionInfo any) error
	QueryRemoteConnectionServices(ctx context.Context, callback RemoteServiceCallback, sessionInfo any) error
	SetVideoProvider(ctx context.Context, callId string, videoProvider IVideoProvider, sessionInfo any) error
	SetVideoState(ctx context.Context, callId string, videoState int32, sessionInfo any) error
	SetIsVoipAudioMode(ctx context.Context, callId string, isVoip bool, sessionInfo any) error
	SetStatusHints(ctx context.Context, callId string, statusHints androidTelecom.StatusHints, sessionInfo any) error
	SetAddress(ctx context.Context, callId string, address net.Uri, presentation int32, sessionInfo any) error
	SetCallerDisplayName(ctx context.Context, callId string, callerDisplayName string, presentation int32, sessionInfo any) error
	SetConferenceableConnections(ctx context.Context, callId string, conferenceableCallIds []string, sessionInfo any) error
	AddExistingConnection(ctx context.Context, callId string, connection androidTelecom.ParcelableConnection, sessionInfo any) error
	PutExtras(ctx context.Context, callId string, extras os.Bundle, sessionInfo any) error
	RemoveExtras(ctx context.Context, callId string, keys []string, sessionInfo any) error
	SetAudioRoute(ctx context.Context, callId string, audioRoute int32, bluetoothAddress string, sessionInfo any) error
	RequestCallEndpointChange(ctx context.Context, callId string, endpoint androidTelecom.CallEndpoint, callback os.ResultReceiver, sessionInfo any) error
	OnConnectionEvent(ctx context.Context, callId string, event string, extras os.Bundle, sessionInfo any) error
	OnRttInitiationSuccess(ctx context.Context, callId string, sessionInfo any) error
	OnRttInitiationFailure(ctx context.Context, callId string, reason int32, sessionInfo any) error
	OnRttSessionRemotelyTerminated(ctx context.Context, callId string, sessionInfo any) error
	OnRemoteRttRequest(ctx context.Context, callId string, sessionInfo any) error
	OnPhoneAccountChanged(ctx context.Context, callId string, pHandle androidTelecom.PhoneAccountHandle, sessionInfo any) error
	OnConnectionServiceFocusReleased(ctx context.Context, sessionInfo any) error
	ResetConnectionTime(ctx context.Context, callIdi string, sessionInfo any) error
	SetConferenceState(ctx context.Context, callId string, isConference bool, sessionInfo any) error
	SetCallDirection(ctx context.Context, callId string, direction int32, sessionInfo any) error
	QueryLocation(ctx context.Context, callId string, timeoutMillis int64, provider string, callback os.ResultReceiver, sessionInfo any) error
}

type connectionServiceAdapterStubWrapper struct {
	impl       IConnectionServiceAdapterServer
	stubBinder *binder.StubBinder
}

func (w *connectionServiceAdapterStubWrapper) AsBinder() binder.IBinder {
	return w.stubBinder
}

func (w *connectionServiceAdapterStubWrapper) HandleCreateConnectionComplete(
	ctx context.Context,
	callId string,
	request androidTelecom.ConnectionRequest,
	connection androidTelecom.ParcelableConnection,
	sessionInfo any,
) error {
	return w.impl.HandleCreateConnectionComplete(ctx, callId, request, connection, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) HandleCreateConferenceComplete(
	ctx context.Context,
	callId string,
	request androidTelecom.ConnectionRequest,
	connection androidTelecom.ParcelableConference,
	sessionInfo any,
) error {
	return w.impl.HandleCreateConferenceComplete(ctx, callId, request, connection, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetActive(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	return w.impl.SetActive(ctx, callId, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetRinging(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	return w.impl.SetRinging(ctx, callId, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetDialing(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	return w.impl.SetDialing(ctx, callId, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetPulling(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	return w.impl.SetPulling(ctx, callId, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetDisconnected(
	ctx context.Context,
	callId string,
	disconnectCause androidTelecom.DisconnectCause,
	sessionInfo any,
) error {
	return w.impl.SetDisconnected(ctx, callId, disconnectCause, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetOnHold(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	return w.impl.SetOnHold(ctx, callId, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetRingbackRequested(
	ctx context.Context,
	callId string,
	ringing bool,
	sessionInfo any,
) error {
	return w.impl.SetRingbackRequested(ctx, callId, ringing, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetConnectionCapabilities(
	ctx context.Context,
	callId string,
	connectionCapabilities int32,
	sessionInfo any,
) error {
	return w.impl.SetConnectionCapabilities(ctx, callId, connectionCapabilities, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetConnectionProperties(
	ctx context.Context,
	callId string,
	connectionProperties int32,
	sessionInfo any,
) error {
	return w.impl.SetConnectionProperties(ctx, callId, connectionProperties, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetIsConferenced(
	ctx context.Context,
	callId string,
	conferenceCallId string,
	sessionInfo any,
) error {
	return w.impl.SetIsConferenced(ctx, callId, conferenceCallId, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetConferenceMergeFailed(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	return w.impl.SetConferenceMergeFailed(ctx, callId, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) AddConferenceCall(
	ctx context.Context,
	callId string,
	conference androidTelecom.ParcelableConference,
	sessionInfo any,
) error {
	return w.impl.AddConferenceCall(ctx, callId, conference, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) RemoveCall(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	return w.impl.RemoveCall(ctx, callId, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) OnPostDialWait(
	ctx context.Context,
	callId string,
	remaining string,
	sessionInfo any,
) error {
	return w.impl.OnPostDialWait(ctx, callId, remaining, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) OnPostDialChar(
	ctx context.Context,
	callId string,
	nextChar uint16,
	sessionInfo any,
) error {
	return w.impl.OnPostDialChar(ctx, callId, nextChar, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) QueryRemoteConnectionServices(
	ctx context.Context,
	callback RemoteServiceCallback,
	sessionInfo any,
) error {
	return w.impl.QueryRemoteConnectionServices(ctx, callback, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetVideoProvider(
	ctx context.Context,
	callId string,
	videoProvider IVideoProvider,
	sessionInfo any,
) error {
	return w.impl.SetVideoProvider(ctx, callId, videoProvider, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetVideoState(
	ctx context.Context,
	callId string,
	videoState int32,
	sessionInfo any,
) error {
	return w.impl.SetVideoState(ctx, callId, videoState, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetIsVoipAudioMode(
	ctx context.Context,
	callId string,
	isVoip bool,
	sessionInfo any,
) error {
	return w.impl.SetIsVoipAudioMode(ctx, callId, isVoip, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetStatusHints(
	ctx context.Context,
	callId string,
	statusHints androidTelecom.StatusHints,
	sessionInfo any,
) error {
	return w.impl.SetStatusHints(ctx, callId, statusHints, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetAddress(
	ctx context.Context,
	callId string,
	address net.Uri,
	presentation int32,
	sessionInfo any,
) error {
	return w.impl.SetAddress(ctx, callId, address, presentation, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetCallerDisplayName(
	ctx context.Context,
	callId string,
	callerDisplayName string,
	presentation int32,
	sessionInfo any,
) error {
	return w.impl.SetCallerDisplayName(ctx, callId, callerDisplayName, presentation, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetConferenceableConnections(
	ctx context.Context,
	callId string,
	conferenceableCallIds []string,
	sessionInfo any,
) error {
	return w.impl.SetConferenceableConnections(ctx, callId, conferenceableCallIds, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) AddExistingConnection(
	ctx context.Context,
	callId string,
	connection androidTelecom.ParcelableConnection,
	sessionInfo any,
) error {
	return w.impl.AddExistingConnection(ctx, callId, connection, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) PutExtras(
	ctx context.Context,
	callId string,
	extras os.Bundle,
	sessionInfo any,
) error {
	return w.impl.PutExtras(ctx, callId, extras, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) RemoveExtras(
	ctx context.Context,
	callId string,
	keys []string,
	sessionInfo any,
) error {
	return w.impl.RemoveExtras(ctx, callId, keys, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetAudioRoute(
	ctx context.Context,
	callId string,
	audioRoute int32,
	bluetoothAddress string,
	sessionInfo any,
) error {
	return w.impl.SetAudioRoute(ctx, callId, audioRoute, bluetoothAddress, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) RequestCallEndpointChange(
	ctx context.Context,
	callId string,
	endpoint androidTelecom.CallEndpoint,
	callback os.ResultReceiver,
	sessionInfo any,
) error {
	return w.impl.RequestCallEndpointChange(ctx, callId, endpoint, callback, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) OnConnectionEvent(
	ctx context.Context,
	callId string,
	event string,
	extras os.Bundle,
	sessionInfo any,
) error {
	return w.impl.OnConnectionEvent(ctx, callId, event, extras, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) OnRttInitiationSuccess(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	return w.impl.OnRttInitiationSuccess(ctx, callId, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) OnRttInitiationFailure(
	ctx context.Context,
	callId string,
	reason int32,
	sessionInfo any,
) error {
	return w.impl.OnRttInitiationFailure(ctx, callId, reason, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) OnRttSessionRemotelyTerminated(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	return w.impl.OnRttSessionRemotelyTerminated(ctx, callId, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) OnRemoteRttRequest(
	ctx context.Context,
	callId string,
	sessionInfo any,
) error {
	return w.impl.OnRemoteRttRequest(ctx, callId, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) OnPhoneAccountChanged(
	ctx context.Context,
	callId string,
	pHandle androidTelecom.PhoneAccountHandle,
	sessionInfo any,
) error {
	return w.impl.OnPhoneAccountChanged(ctx, callId, pHandle, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) OnConnectionServiceFocusReleased(
	ctx context.Context,
	sessionInfo any,
) error {
	return w.impl.OnConnectionServiceFocusReleased(ctx, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) ResetConnectionTime(
	ctx context.Context,
	callIdi string,
	sessionInfo any,
) error {
	return w.impl.ResetConnectionTime(ctx, callIdi, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetConferenceState(
	ctx context.Context,
	callId string,
	isConference bool,
	sessionInfo any,
) error {
	return w.impl.SetConferenceState(ctx, callId, isConference, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) SetCallDirection(
	ctx context.Context,
	callId string,
	direction int32,
	sessionInfo any,
) error {
	return w.impl.SetCallDirection(ctx, callId, direction, sessionInfo)
}

func (w *connectionServiceAdapterStubWrapper) QueryLocation(
	ctx context.Context,
	callId string,
	timeoutMillis int64,
	provider string,
	callback os.ResultReceiver,
	sessionInfo any,
) error {
	return w.impl.QueryLocation(ctx, callId, timeoutMillis, provider, callback, sessionInfo)
}

var _ IConnectionServiceAdapter = (*connectionServiceAdapterStubWrapper)(nil)

// NewConnectionServiceAdapterStub creates a server-side IConnectionServiceAdapter wrapping the given
// server implementation. The returned value satisfies IConnectionServiceAdapter
// and can be passed to proxy methods; its AsBinder() returns a
// *binder.StubBinder that is auto-registered with the binder
// driver on first use.
func NewConnectionServiceAdapterStub(
	impl IConnectionServiceAdapterServer,
) IConnectionServiceAdapter {
	wrapper := &connectionServiceAdapterStubWrapper{impl: impl}
	stub := &ConnectionServiceAdapterStub{Impl: wrapper}
	wrapper.stubBinder = binder.NewStubBinder(stub)
	return wrapper
}
