package appfunctions

import (
	"context"
	ondeviceintelligence "github.com/xaionaro-go/aidl/android/app/ondeviceintelligence"
	os "github.com/xaionaro-go/aidl/android/os"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIAppFunctionManager = "android.app.appfunctions.IAppFunctionManager"

const (
	TransactionIAppFunctionManagerExecuteAppFunction    = binder.FirstCallTransaction + 0
	TransactionIAppFunctionManagerSetAppFunctionEnabled = binder.FirstCallTransaction + 1
)

type IAppFunctionManager interface {
	AsBinder() binder.IBinder
	ExecuteAppFunction(ctx context.Context, request ExecuteAppFunctionAidlRequest, callback IExecuteAppFunctionCallback) (ondeviceintelligence.ICancellationSignal, error)
	SetAppFunctionEnabled(ctx context.Context, callingPackage string, functionIdentifier string, userHandle os.UserHandle, enabledState int32, callback IAppFunctionEnabledCallback) error
}

type AppFunctionManagerProxy struct {
	remote binder.IBinder
}

func NewAppFunctionManagerProxy(
	remote binder.IBinder,
) *AppFunctionManagerProxy {
	return &AppFunctionManagerProxy{remote: remote}
}

func (p *AppFunctionManagerProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IAppFunctionManager = (*AppFunctionManagerProxy)(nil)

func (p *AppFunctionManagerProxy) ExecuteAppFunction(
	ctx context.Context,
	request ExecuteAppFunctionAidlRequest,
	callback IExecuteAppFunctionCallback,
) (ondeviceintelligence.ICancellationSignal, error) {
	var _result ondeviceintelligence.ICancellationSignal
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAppFunctionManager)
	if _err := request.MarshalParcel(_data); _err != nil {
		return _result, _err
	}
	_data.WriteStrongBinder(callback.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, TransactionIAppFunctionManagerExecuteAppFunction, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_handle, _err := _reply.ReadStrongBinder()
	if _err != nil {
		return _result, _err
	}
	_result = ondeviceintelligence.NewCancellationSignalProxy(binder.NewProxyBinder(p.remote.Transport(), _handle))
	return _result, nil
}

func (p *AppFunctionManagerProxy) SetAppFunctionEnabled(
	ctx context.Context,
	callingPackage string,
	functionIdentifier string,
	userHandle os.UserHandle,
	enabledState int32,
	callback IAppFunctionEnabledCallback,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIAppFunctionManager)
	_data.WriteString16(callingPackage)
	_data.WriteString16(functionIdentifier)
	if _err := userHandle.MarshalParcel(_data); _err != nil {
		return _err
	}
	_data.WriteInt32(enabledState)
	_data.WriteStrongBinder(callback.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, TransactionIAppFunctionManagerSetAppFunctionEnabled, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
