...
func (p *RemoteLockscreenValidationCallbackProxy) OnSuccess(
	...
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIRemoteLockscreenValidationCallback, MethodIRemoteLockscreenValidationCallbackOnSuccess)
...
func (p *RemoteLockscreenValidationCallbackProxy) OnFailure(
	...
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIRemoteLockscreenValidationCallback)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIRemoteLockscreenValidationCallback, MethodIRemoteLockscreenValidationCallbackOnFailure)
	_compiledDescs := []string{
		"Ljava/lang/String;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
