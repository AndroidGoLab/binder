package flags

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIFeatureFlags = "android.flags.IFeatureFlags"

const (
	TransactionIFeatureFlagsSyncFlags          = binder.FirstCallTransaction + 0
	TransactionIFeatureFlagsRegisterCallback   = binder.FirstCallTransaction + 1
	TransactionIFeatureFlagsUnregisterCallback = binder.FirstCallTransaction + 2
	TransactionIFeatureFlagsQueryFlags         = binder.FirstCallTransaction + 3
	TransactionIFeatureFlagsOverrideFlag       = binder.FirstCallTransaction + 4
	TransactionIFeatureFlagsResetFlag          = binder.FirstCallTransaction + 5
)

type IFeatureFlags interface {
	AsBinder() binder.IBinder
	SyncFlags(ctx context.Context, flagList []SyncableFlag) ([]SyncableFlag, error)
	RegisterCallback(ctx context.Context, callback IFeatureFlagsCallback) error
	UnregisterCallback(ctx context.Context, callback IFeatureFlagsCallback) error
	QueryFlags(ctx context.Context, flagList []SyncableFlag) ([]SyncableFlag, error)
	OverrideFlag(ctx context.Context, flag SyncableFlag) error
	ResetFlag(ctx context.Context, flag SyncableFlag) error
}

type FeatureFlagsProxy struct {
	remote binder.IBinder
}

func NewFeatureFlagsProxy(
	remote binder.IBinder,
) *FeatureFlagsProxy {
	return &FeatureFlagsProxy{remote: remote}
}

func (p *FeatureFlagsProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IFeatureFlags = (*FeatureFlagsProxy)(nil)

func (p *FeatureFlagsProxy) SyncFlags(
	ctx context.Context,
	flagList []SyncableFlag,
) ([]SyncableFlag, error) {
	var _result []SyncableFlag
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIFeatureFlags)
	if flagList == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(flagList)))
		for _, _item := range flagList {
			if _err := _item.MarshalParcel(_data); _err != nil {
				return _result, _err
			}
		}
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIFeatureFlagsSyncFlags, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_count, _err := _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}

	if _count >= 0 {
		_result = make([]SyncableFlag, _count)
		for _i := int32(0); _i < _count; _i++ {
			if _err = _result[_i].UnmarshalParcel(_reply); _err != nil {
				return _result, _err
			}
		}
	}
	return _result, nil
}

func (p *FeatureFlagsProxy) RegisterCallback(
	ctx context.Context,
	callback IFeatureFlagsCallback,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIFeatureFlags)
	_data.WriteStrongBinder(callback.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, TransactionIFeatureFlagsRegisterCallback, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *FeatureFlagsProxy) UnregisterCallback(
	ctx context.Context,
	callback IFeatureFlagsCallback,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIFeatureFlags)
	_data.WriteStrongBinder(callback.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, TransactionIFeatureFlagsUnregisterCallback, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *FeatureFlagsProxy) QueryFlags(
	ctx context.Context,
	flagList []SyncableFlag,
) ([]SyncableFlag, error) {
	var _result []SyncableFlag
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIFeatureFlags)
	if flagList == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(flagList)))
		for _, _item := range flagList {
			if _err := _item.MarshalParcel(_data); _err != nil {
				return _result, _err
			}
		}
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIFeatureFlagsQueryFlags, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_count, _err := _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}

	if _count >= 0 {
		_result = make([]SyncableFlag, _count)
		for _i := int32(0); _i < _count; _i++ {
			if _err = _result[_i].UnmarshalParcel(_reply); _err != nil {
				return _result, _err
			}
		}
	}
	return _result, nil
}

func (p *FeatureFlagsProxy) OverrideFlag(
	ctx context.Context,
	flag SyncableFlag,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIFeatureFlags)
	if _err := flag.MarshalParcel(_data); _err != nil {
		return _err
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIFeatureFlagsOverrideFlag, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *FeatureFlagsProxy) ResetFlag(
	ctx context.Context,
	flag SyncableFlag,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIFeatureFlags)
	if _err := flag.MarshalParcel(_data); _err != nil {
		return _err
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIFeatureFlagsResetFlag, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
