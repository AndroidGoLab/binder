package mediaquality

import (
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

type SoundParameters struct {
	SoundParameters       []SoundParameter
	VendorSoundParameters interface{}
}

var _ parcel.Parcelable = (*SoundParameters)(nil)

func (s *SoundParameters) MarshalParcel(
	p *parcel.Parcel,
) error {
	_headerPos := parcel.WriteParcelableHeader(p)
	if s.SoundParameters == nil {
		p.WriteInt32(-1)
	} else {
		p.WriteInt32(int32(len(s.SoundParameters)))
		for _, _item := range s.SoundParameters {
			if _err := _item.MarshalParcel(p); _err != nil {
				return _err
			}
		}
	}

	parcel.WriteParcelableFooter(p, _headerPos)
	return nil
}

func (s *SoundParameters) UnmarshalParcel(
	p *parcel.Parcel,
) error {
	_endPos, _err := parcel.ReadParcelableHeader(p)
	if _err != nil {
		return _err
	}

	var _count0 int32
	_count0, _err = p.ReadInt32()
	if _err != nil {
		return _err
	}
	if _count0 >= 0 {
		s.SoundParameters = make([]SoundParameter, _count0)
		for _i := int32(0); _i < _count0; _i++ {
			if _err = s.SoundParameters[_i].UnmarshalParcel(p); _err != nil {
				return _err
			}
		}
	}

	parcel.SkipToParcelableEnd(p, _endPos)
	return nil
}
