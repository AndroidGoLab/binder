package session

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorISession = "android.media.session.ISession"

const (
	TransactionISessionSendEvent                       = binder.FirstCallTransaction + 0
	TransactionISessionGetController                   = binder.FirstCallTransaction + 1
	TransactionISessionSetFlags                        = binder.FirstCallTransaction + 2
	TransactionISessionSetActive                       = binder.FirstCallTransaction + 3
	TransactionISessionSetMediaButtonReceiver          = binder.FirstCallTransaction + 4
	TransactionISessionSetMediaButtonBroadcastReceiver = binder.FirstCallTransaction + 5
	TransactionISessionSetLaunchPendingIntent          = binder.FirstCallTransaction + 6
	TransactionISessionDestroySession                  = binder.FirstCallTransaction + 7
	TransactionISessionSetMetadata                     = binder.FirstCallTransaction + 8
	TransactionISessionSetPlaybackState                = binder.FirstCallTransaction + 9
	TransactionISessionResetQueue                      = binder.FirstCallTransaction + 10
	TransactionISessionGetBinderForSetQueue            = binder.FirstCallTransaction + 11
	TransactionISessionSetQueueTitle                   = binder.FirstCallTransaction + 12
	TransactionISessionSetExtras                       = binder.FirstCallTransaction + 13
	TransactionISessionSetRatingType                   = binder.FirstCallTransaction + 14
	TransactionISessionSetPlaybackToLocal              = binder.FirstCallTransaction + 15
	TransactionISessionSetPlaybackToRemote             = binder.FirstCallTransaction + 16
	TransactionISessionSetCurrentVolume                = binder.FirstCallTransaction + 17
)

type ISession interface {
	AsBinder() binder.IBinder
	SendEvent(ctx context.Context, event string, data interface{}) error
	GetController(ctx context.Context) (ISessionController, error)
	SetFlags(ctx context.Context, flags int32) error
	SetActive(ctx context.Context, active bool) error
	SetMediaButtonReceiver(ctx context.Context, mbr interface{}) error
	SetMediaButtonBroadcastReceiver(ctx context.Context, broadcastReceiver interface{}) error
	SetLaunchPendingIntent(ctx context.Context, pi interface{}) error
	DestroySession(ctx context.Context) error
	SetMetadata(ctx context.Context, metadata interface{}, duration int64, metadataDescription string) error
	SetPlaybackState(ctx context.Context, state PlaybackState) error
	ResetQueue(ctx context.Context) error
	GetBinderForSetQueue(ctx context.Context) (binder.IBinder, error)
	SetQueueTitle(ctx context.Context, title interface{}) error
	SetExtras(ctx context.Context, extras interface{}) error
	SetRatingType(ctx context.Context, type_ int32) error
	SetPlaybackToLocal(ctx context.Context, attributes interface{}) error
	SetPlaybackToRemote(ctx context.Context, control int32, max int32, controlId string) error
	SetCurrentVolume(ctx context.Context, currentVolume int32) error
}

type SessionProxy struct {
	remote binder.IBinder
}

func NewSessionProxy(
	remote binder.IBinder,
) *SessionProxy {
	return &SessionProxy{remote: remote}
}

func (p *SessionProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ ISession = (*SessionProxy)(nil)

func (p *SessionProxy) SendEvent(
	ctx context.Context,
	event string,
	data interface{},
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)
	_data.WriteString16(event)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionSendEvent, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SessionProxy) GetController(
	ctx context.Context,
) (ISessionController, error) {
	var _result ISessionController
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionGetController, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_handle, _err := _reply.ReadStrongBinder()
	if _err != nil {
		return _result, _err
	}
	_result = NewSessionControllerProxy(binder.NewProxyBinder(p.remote.Transport(), _handle))
	return _result, nil
}

func (p *SessionProxy) SetFlags(
	ctx context.Context,
	flags int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)
	_data.WriteInt32(flags)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionSetFlags, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SessionProxy) SetActive(
	ctx context.Context,
	active bool,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)
	_data.WriteBool(active)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionSetActive, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SessionProxy) SetMediaButtonReceiver(
	ctx context.Context,
	mbr interface{},
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionSetMediaButtonReceiver, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SessionProxy) SetMediaButtonBroadcastReceiver(
	ctx context.Context,
	broadcastReceiver interface{},
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionSetMediaButtonBroadcastReceiver, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SessionProxy) SetLaunchPendingIntent(
	ctx context.Context,
	pi interface{},
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionSetLaunchPendingIntent, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SessionProxy) DestroySession(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionDestroySession, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SessionProxy) SetMetadata(
	ctx context.Context,
	metadata interface{},
	duration int64,
	metadataDescription string,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)
	_data.WriteInt64(duration)
	_data.WriteString16(metadataDescription)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionSetMetadata, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SessionProxy) SetPlaybackState(
	ctx context.Context,
	state PlaybackState,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)
	if _err := state.MarshalParcel(_data); _err != nil {
		return _err
	}

	_reply, _err := p.remote.Transact(ctx, TransactionISessionSetPlaybackState, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SessionProxy) ResetQueue(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionResetQueue, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SessionProxy) GetBinderForSetQueue(
	ctx context.Context,
) (binder.IBinder, error) {
	var _result binder.IBinder
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionGetBinderForSetQueue, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_handle, _err := _reply.ReadStrongBinder()
	if _err != nil {
		return _result, _err
	}
	_result = binder.NewProxyBinder(p.remote.Transport(), _handle)
	return _result, nil
}

func (p *SessionProxy) SetQueueTitle(
	ctx context.Context,
	title interface{},
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionSetQueueTitle, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SessionProxy) SetExtras(
	ctx context.Context,
	extras interface{},
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionSetExtras, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SessionProxy) SetRatingType(
	ctx context.Context,
	type_ int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)
	_data.WriteInt32(type_)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionSetRatingType, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SessionProxy) SetPlaybackToLocal(
	ctx context.Context,
	attributes interface{},
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionSetPlaybackToLocal, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SessionProxy) SetPlaybackToRemote(
	ctx context.Context,
	control int32,
	max int32,
	controlId string,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)
	_data.WriteInt32(control)
	_data.WriteInt32(max)
	_data.WriteString16(controlId)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionSetPlaybackToRemote, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SessionProxy) SetCurrentVolume(
	ctx context.Context,
	currentVolume int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISession)
	_data.WriteInt32(currentVolume)

	_reply, _err := p.remote.Transact(ctx, TransactionISessionSetCurrentVolume, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
