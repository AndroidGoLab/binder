package content

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorISyncContext = "android.content.ISyncContext"

const (
	TransactionISyncContextSendHeartbeat = binder.FirstCallTransaction + 0
	TransactionISyncContextOnFinished    = binder.FirstCallTransaction + 1
)

type ISyncContext interface {
	AsBinder() binder.IBinder
	SendHeartbeat(ctx context.Context) error
	OnFinished(ctx context.Context, result SyncResult) error
}

type SyncContextProxy struct {
	remote binder.IBinder
}

func NewSyncContextProxy(
	remote binder.IBinder,
) *SyncContextProxy {
	return &SyncContextProxy{remote: remote}
}

func (p *SyncContextProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ ISyncContext = (*SyncContextProxy)(nil)

func (p *SyncContextProxy) SendHeartbeat(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISyncContext)

	_reply, _err := p.remote.Transact(ctx, TransactionISyncContextSendHeartbeat, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SyncContextProxy) OnFinished(
	ctx context.Context,
	result SyncResult,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISyncContext)
	if _err := result.MarshalParcel(_data); _err != nil {
		return _err
	}

	_reply, _err := p.remote.Transact(ctx, TransactionISyncContextOnFinished, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
