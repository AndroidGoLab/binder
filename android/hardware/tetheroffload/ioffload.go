package tetheroffload

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIOffload = "android.hardware.tetheroffload.IOffload"

const (
	TransactionIOffloadInitOffload            = binder.FirstCallTransaction + 0
	TransactionIOffloadStopOffload            = binder.FirstCallTransaction + 1
	TransactionIOffloadSetLocalPrefixes       = binder.FirstCallTransaction + 2
	TransactionIOffloadGetForwardedStats      = binder.FirstCallTransaction + 3
	TransactionIOffloadSetDataWarningAndLimit = binder.FirstCallTransaction + 4
	TransactionIOffloadSetUpstreamParameters  = binder.FirstCallTransaction + 5
	TransactionIOffloadAddDownstream          = binder.FirstCallTransaction + 6
	TransactionIOffloadRemoveDownstream       = binder.FirstCallTransaction + 7
)

type IOffload interface {
	AsBinder() binder.IBinder
	InitOffload(ctx context.Context, fd1 int32, fd2 int32, cb ITetheringOffloadCallback) error
	StopOffload(ctx context.Context) error
	SetLocalPrefixes(ctx context.Context, prefixes []string) error
	GetForwardedStats(ctx context.Context, upstream string) (ForwardedStats, error)
	SetDataWarningAndLimit(ctx context.Context, upstream string, warningBytes int64, limitBytes int64) error
	SetUpstreamParameters(ctx context.Context, iface string, v4Addr string, v4Gw string, v6Gws []string) error
	AddDownstream(ctx context.Context, iface string, prefix string) error
	RemoveDownstream(ctx context.Context, iface string, prefix string) error
}

const (
	IOffloadErrorCodeUnused int32 = 0
)

type OffloadProxy struct {
	remote binder.IBinder
}

func NewOffloadProxy(
	remote binder.IBinder,
) *OffloadProxy {
	return &OffloadProxy{remote: remote}
}

func (p *OffloadProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IOffload = (*OffloadProxy)(nil)

func (p *OffloadProxy) InitOffload(
	ctx context.Context,
	fd1 int32,
	fd2 int32,
	cb ITetheringOffloadCallback,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIOffload)
	_data.WriteFileDescriptor(fd1)
	_data.WriteFileDescriptor(fd2)
	_data.WriteStrongBinder(cb.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, TransactionIOffloadInitOffload, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *OffloadProxy) StopOffload(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIOffload)

	_reply, _err := p.remote.Transact(ctx, TransactionIOffloadStopOffload, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *OffloadProxy) SetLocalPrefixes(
	ctx context.Context,
	prefixes []string,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIOffload)
	if prefixes == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(prefixes)))
		for _, _item := range prefixes {
			_data.WriteString16(_item)
		}
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIOffloadSetLocalPrefixes, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *OffloadProxy) GetForwardedStats(
	ctx context.Context,
	upstream string,
) (ForwardedStats, error) {
	var _result ForwardedStats
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIOffload)
	_data.WriteString16(upstream)

	_reply, _err := p.remote.Transact(ctx, TransactionIOffloadGetForwardedStats, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	if _err = _result.UnmarshalParcel(_reply); _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *OffloadProxy) SetDataWarningAndLimit(
	ctx context.Context,
	upstream string,
	warningBytes int64,
	limitBytes int64,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIOffload)
	_data.WriteString16(upstream)
	_data.WriteInt64(warningBytes)
	_data.WriteInt64(limitBytes)

	_reply, _err := p.remote.Transact(ctx, TransactionIOffloadSetDataWarningAndLimit, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *OffloadProxy) SetUpstreamParameters(
	ctx context.Context,
	iface string,
	v4Addr string,
	v4Gw string,
	v6Gws []string,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIOffload)
	_data.WriteString16(iface)
	_data.WriteString16(v4Addr)
	_data.WriteString16(v4Gw)
	if v6Gws == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(v6Gws)))
		for _, _item := range v6Gws {
			_data.WriteString16(_item)
		}
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIOffloadSetUpstreamParameters, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *OffloadProxy) AddDownstream(
	ctx context.Context,
	iface string,
	prefix string,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIOffload)
	_data.WriteString16(iface)
	_data.WriteString16(prefix)

	_reply, _err := p.remote.Transact(ctx, TransactionIOffloadAddDownstream, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *OffloadProxy) RemoveDownstream(
	ctx context.Context,
	iface string,
	prefix string,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIOffload)
	_data.WriteString16(iface)
	_data.WriteString16(prefix)

	_reply, _err := p.remote.Transact(ctx, TransactionIOffloadRemoveDownstream, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
