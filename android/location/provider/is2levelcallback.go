package provider

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIS2LevelCallback = "android.location.provider.IS2LevelCallback"

const (
	TransactionIS2LevelCallbackOnResult = binder.FirstCallTransaction + 0
	TransactionIS2LevelCallbackOnError  = binder.FirstCallTransaction + 1
)

type IS2LevelCallback interface {
	AsBinder() binder.IBinder
	OnResult(ctx context.Context, s2Level int32) error
	OnError(ctx context.Context) error
}

type S2LevelCallbackProxy struct {
	remote binder.IBinder
}

func NewS2LevelCallbackProxy(
	remote binder.IBinder,
) *S2LevelCallbackProxy {
	return &S2LevelCallbackProxy{remote: remote}
}

func (p *S2LevelCallbackProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IS2LevelCallback = (*S2LevelCallbackProxy)(nil)

func (p *S2LevelCallbackProxy) OnResult(
	ctx context.Context,
	s2Level int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIS2LevelCallback)
	_data.WriteInt32(s2Level)

	_, _err := p.remote.Transact(ctx, TransactionIS2LevelCallbackOnResult, binder.FlagOneway, _data)
	return _err
}

func (p *S2LevelCallbackProxy) OnError(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIS2LevelCallback)

	_, _err := p.remote.Transact(ctx, TransactionIS2LevelCallbackOnError, binder.FlagOneway, _data)
	return _err
}
