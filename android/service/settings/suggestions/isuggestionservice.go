package suggestions

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorISuggestionService = "android.service.settings.suggestions.ISuggestionService"

const (
	TransactionISuggestionServiceGetSuggestions    = binder.FirstCallTransaction + 0
	TransactionISuggestionServiceDismissSuggestion = binder.FirstCallTransaction + 1
	TransactionISuggestionServiceLaunchSuggestion  = binder.FirstCallTransaction + 2
)

type ISuggestionService interface {
	AsBinder() binder.IBinder
	GetSuggestions(ctx context.Context) ([]Suggestion, error)
	DismissSuggestion(ctx context.Context, suggestion Suggestion) error
	LaunchSuggestion(ctx context.Context, suggestion Suggestion) error
}

type SuggestionServiceProxy struct {
	remote binder.IBinder
}

func NewSuggestionServiceProxy(
	remote binder.IBinder,
) *SuggestionServiceProxy {
	return &SuggestionServiceProxy{remote: remote}
}

func (p *SuggestionServiceProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ ISuggestionService = (*SuggestionServiceProxy)(nil)

func (p *SuggestionServiceProxy) GetSuggestions(
	ctx context.Context,
) ([]Suggestion, error) {
	var _result []Suggestion
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISuggestionService)

	_reply, _err := p.remote.Transact(ctx, TransactionISuggestionServiceGetSuggestions, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_count, _err := _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}

	if _count >= 0 {
		_result = make([]Suggestion, _count)
		for _i := int32(0); _i < _count; _i++ {
			if _err = _result[_i].UnmarshalParcel(_reply); _err != nil {
				return _result, _err
			}
		}
	}
	return _result, nil
}

func (p *SuggestionServiceProxy) DismissSuggestion(
	ctx context.Context,
	suggestion Suggestion,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISuggestionService)
	if _err := suggestion.MarshalParcel(_data); _err != nil {
		return _err
	}

	_reply, _err := p.remote.Transact(ctx, TransactionISuggestionServiceDismissSuggestion, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *SuggestionServiceProxy) LaunchSuggestion(
	ctx context.Context,
	suggestion Suggestion,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISuggestionService)
	if _err := suggestion.MarshalParcel(_data); _err != nil {
		return _err
	}

	_reply, _err := p.remote.Transact(ctx, TransactionISuggestionServiceLaunchSuggestion, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
