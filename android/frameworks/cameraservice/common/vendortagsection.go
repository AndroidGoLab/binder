...
var _ parcel.Parcelable = (*VendorTagSection)(nil)

func (s *VendorTagSection) MarshalParcel(
	p *parcel.Parcel,
) error {
	_headerPos := parcel.WriteParcelableHeader(p)
	p.WriteString16(s.SectionName)
	if s.Tags == nil {
		p.WriteInt32(-1)
	} else {
...
func (s *VendorTagSection) UnmarshalParcel(
	...
	s.SectionName, _err = p.ReadString16()
	if _err != nil {
		return _err
	}
	...
	if _count0 >= 0 {
		s.Tags = make([]VendorTag, _count0)
		for _i := int32(0); _i < _count0; _i++ {
			if _, _err = p.ReadInt32(); _err != nil {
				return _err
			}
			if _err = s.Tags[_i].UnmarshalParcel(p); _err != nil {
				return _err
			}
		}
	}
