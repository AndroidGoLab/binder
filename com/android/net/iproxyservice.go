...
func (p *ProxyServiceProxy) ResolvePacFile(
	...
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIProxyService, MethodIProxyServiceResolvePacFile)
...
func (p *ProxyServiceProxy) SetPacFile(
	...
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIProxyService)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIProxyService, MethodIProxyServiceSetPacFile)
	_compiledDescs := []string{
		"Ljava/lang/String;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
