package common

import (
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

type AudioHalAttributesGroup struct {
	StreamType      AudioStreamType
	VolumeGroupName string
	Attributes      []AudioAttributes
}

var _ parcel.Parcelable = (*AudioHalAttributesGroup)(nil)

func (s *AudioHalAttributesGroup) MarshalParcel(
	p *parcel.Parcel,
) error {
	_headerPos := parcel.WriteParcelableHeader(p)
	p.WriteInt32(int32(s.StreamType))
	p.WriteString16(s.VolumeGroupName)
	if s.Attributes == nil {
		p.WriteInt32(-1)
	} else {
		p.WriteInt32(int32(len(s.Attributes)))
		for _, _item := range s.Attributes {
			if _err := _item.MarshalParcel(p); _err != nil {
				return _err
			}
		}
	}

	parcel.WriteParcelableFooter(p, _headerPos)
	return nil
}

func (s *AudioHalAttributesGroup) UnmarshalParcel(
	p *parcel.Parcel,
) error {
	_endPos, _err := parcel.ReadParcelableHeader(p)
	if _err != nil {
		return _err
	}

	_streamTypeRaw, _err := p.ReadInt32()
	if _err != nil {
		return _err
	}
	s.StreamType = AudioStreamType(_streamTypeRaw)

	s.VolumeGroupName, _err = p.ReadString16()
	if _err != nil {
		return _err
	}

	var _count0 int32
	_count0, _err = p.ReadInt32()
	if _err != nil {
		return _err
	}
	if _count0 >= 0 {
		s.Attributes = make([]AudioAttributes, _count0)
		for _i := int32(0); _i < _count0; _i++ {
			if _err = s.Attributes[_i].UnmarshalParcel(p); _err != nil {
				return _err
			}
		}
	}

	parcel.SkipToParcelableEnd(p, _endPos)
	return nil
}
