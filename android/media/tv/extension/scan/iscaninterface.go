package scan

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	os "github.com/xaionaro-go/aidl/android/os"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIScanInterface = "android.media.tv.extension.scan.IScanInterface"

const (
	TransactionIScanInterfaceCreateSession = binder.FirstCallTransaction + 0
	TransactionIScanInterfaceGetParameters = binder.FirstCallTransaction + 1
)

type IScanInterface interface {
	AsBinder() binder.IBinder
	CreateSession(ctx context.Context, broadcastType int32, countryCode string, operator string, listener IScanListener) (binder.IBinder, error)
	GetParameters(ctx context.Context, broadcastType int32, countryCode string, operator string, params os.Bundle) (os.Bundle, error)
}

type ScanInterfaceProxy struct {
	remote binder.IBinder
}

func NewScanInterfaceProxy(
	remote binder.IBinder,
) *ScanInterfaceProxy {
	return &ScanInterfaceProxy{remote: remote}
}

func (p *ScanInterfaceProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IScanInterface = (*ScanInterfaceProxy)(nil)

func (p *ScanInterfaceProxy) CreateSession(
	ctx context.Context,
	broadcastType int32,
	countryCode string,
	operator string,
	listener IScanListener,
) (binder.IBinder, error) {
	var _result binder.IBinder
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIScanInterface)
	_data.WriteInt32(broadcastType)
	_data.WriteString16(countryCode)
	_data.WriteString16(operator)
	_data.WriteStrongBinder(listener.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, TransactionIScanInterfaceCreateSession, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_handle, _err := _reply.ReadStrongBinder()
	if _err != nil {
		return _result, _err
	}
	_result = binder.NewProxyBinder(p.remote.Transport(), _handle)
	return _result, nil
}

func (p *ScanInterfaceProxy) GetParameters(
	ctx context.Context,
	broadcastType int32,
	countryCode string,
	operator string,
	params os.Bundle,
) (os.Bundle, error) {
	var _result os.Bundle
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIScanInterface)
	_data.WriteInt32(broadcastType)
	_data.WriteString16(countryCode)
	_data.WriteString16(operator)
	if _err := params.MarshalParcel(_data); _err != nil {
		return _result, _err
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIScanInterfaceGetParameters, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	if _err = _result.UnmarshalParcel(_reply); _err != nil {
		return _result, _err
	}
	return _result, nil
}
