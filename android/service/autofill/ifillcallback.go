package autofill

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIFillCallback = "android.service.autofill.IFillCallback"

const (
	TransactionIFillCallbackOnCancellable = binder.FirstCallTransaction + 0
	TransactionIFillCallbackOnSuccess     = binder.FirstCallTransaction + 1
	TransactionIFillCallbackOnFailure     = binder.FirstCallTransaction + 2
)

type IFillCallback interface {
	AsBinder() binder.IBinder
	OnCancellable(ctx context.Context, cancellation interface{}) error
	OnSuccess(ctx context.Context, response FillResponse) error
	OnFailure(ctx context.Context, requestId int32, message interface{}) error
}

type FillCallbackProxy struct {
	remote binder.IBinder
}

func NewFillCallbackProxy(
	remote binder.IBinder,
) *FillCallbackProxy {
	return &FillCallbackProxy{remote: remote}
}

func (p *FillCallbackProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IFillCallback = (*FillCallbackProxy)(nil)

func (p *FillCallbackProxy) OnCancellable(
	ctx context.Context,
	cancellation interface{},
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIFillCallback)

	_, _err := p.remote.Transact(ctx, TransactionIFillCallbackOnCancellable, binder.FlagOneway, _data)
	return _err
}

func (p *FillCallbackProxy) OnSuccess(
	ctx context.Context,
	response FillResponse,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIFillCallback)
	if _err := response.MarshalParcel(_data); _err != nil {
		return _err
	}

	_, _err := p.remote.Transact(ctx, TransactionIFillCallbackOnSuccess, binder.FlagOneway, _data)
	return _err
}

func (p *FillCallbackProxy) OnFailure(
	ctx context.Context,
	requestId int32,
	message interface{},
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIFillCallback)
	_data.WriteInt32(requestId)

	_, _err := p.remote.Transact(ctx, TransactionIFillCallbackOnFailure, binder.FlagOneway, _data)
	return _err
}
