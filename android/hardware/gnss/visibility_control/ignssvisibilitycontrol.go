package visibility_control

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIGnssVisibilityControl = "android.hardware.gnss.visibility_control.IGnssVisibilityControl"

const (
	TransactionIGnssVisibilityControlEnableNfwLocationAccess = binder.FirstCallTransaction + 0
	TransactionIGnssVisibilityControlSetCallback             = binder.FirstCallTransaction + 1
)

type IGnssVisibilityControl interface {
	AsBinder() binder.IBinder
	EnableNfwLocationAccess(ctx context.Context, proxyApps []string) error
	SetCallback(ctx context.Context, callback IGnssVisibilityControlCallback) error
}

type GnssVisibilityControlProxy struct {
	remote binder.IBinder
}

func NewGnssVisibilityControlProxy(
	remote binder.IBinder,
) *GnssVisibilityControlProxy {
	return &GnssVisibilityControlProxy{remote: remote}
}

func (p *GnssVisibilityControlProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IGnssVisibilityControl = (*GnssVisibilityControlProxy)(nil)

func (p *GnssVisibilityControlProxy) EnableNfwLocationAccess(
	ctx context.Context,
	proxyApps []string,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIGnssVisibilityControl)
	if proxyApps == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(proxyApps)))
		for _, _item := range proxyApps {
			_data.WriteString(_item)
		}
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIGnssVisibilityControlEnableNfwLocationAccess, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *GnssVisibilityControlProxy) SetCallback(
	ctx context.Context,
	callback IGnssVisibilityControlCallback,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIGnssVisibilityControl)
	_data.WriteStrongBinder(callback.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, TransactionIGnssVisibilityControlSetCallback, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
