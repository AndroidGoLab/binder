package telephony

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIBootstrapAuthenticationCallback = "android.telephony.IBootstrapAuthenticationCallback"

const (
	TransactionIBootstrapAuthenticationCallbackOnKeysAvailable         = binder.FirstCallTransaction + 0
	TransactionIBootstrapAuthenticationCallbackOnAuthenticationFailure = binder.FirstCallTransaction + 1
)

type IBootstrapAuthenticationCallback interface {
	AsBinder() binder.IBinder
	OnKeysAvailable(ctx context.Context, token int32, gbaKey []byte, btId string) error
	OnAuthenticationFailure(ctx context.Context, token int32, reason int32) error
}

type BootstrapAuthenticationCallbackProxy struct {
	remote binder.IBinder
}

func NewBootstrapAuthenticationCallbackProxy(
	remote binder.IBinder,
) *BootstrapAuthenticationCallbackProxy {
	return &BootstrapAuthenticationCallbackProxy{remote: remote}
}

func (p *BootstrapAuthenticationCallbackProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IBootstrapAuthenticationCallback = (*BootstrapAuthenticationCallbackProxy)(nil)

func (p *BootstrapAuthenticationCallbackProxy) OnKeysAvailable(
	ctx context.Context,
	token int32,
	gbaKey []byte,
	btId string,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIBootstrapAuthenticationCallback)
	_data.WriteInt32(token)
	if gbaKey == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(gbaKey)))
		for _, _item := range gbaKey {
			_data.WritePaddedByte(_item)
		}
	}
	_data.WriteString16(btId)

	_, _err := p.remote.Transact(ctx, TransactionIBootstrapAuthenticationCallbackOnKeysAvailable, binder.FlagOneway, _data)
	return _err
}

func (p *BootstrapAuthenticationCallbackProxy) OnAuthenticationFailure(
	ctx context.Context,
	token int32,
	reason int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIBootstrapAuthenticationCallback)
	_data.WriteInt32(token)
	_data.WriteInt32(reason)

	_, _err := p.remote.Transact(ctx, TransactionIBootstrapAuthenticationCallbackOnAuthenticationFailure, binder.FlagOneway, _data)
	return _err
}
