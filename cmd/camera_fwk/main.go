...
func (c *callback) OnCaptureStarted(_ context.Context, extras fwkDevice.CaptureResultExtras, ts int64) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.framesReceived++
...
func (c *callback) OnDeviceError(_ context.Context, code fwkDevice.ErrorCode, extras fwkDevice.CaptureResultExtras) error {
	fmt.Printf("  >> OnDeviceError: code=%d requestId=%d\n", code, extras.RequestId)
	return nil
}
func (c *callback) OnDeviceIdle(_ context.Context) error {
	fmt.Println("  >> OnDeviceIdle")
	return nil
}
func (c *callback) OnPrepared(_ context.Context, streamId int32) error {
	fmt.Printf("  >> OnPrepared: stream %d\n", streamId)
	return nil
}
func (c *callback) OnRepeatingRequestError(_ context.Context, lastFrame int64, reqId int32) error {
	fmt.Printf("  >> OnRepeatingRequestError: frame=%d req=%d\n", lastFrame, reqId)
	return nil
}
func (c *callback) OnResultReceived(_ context.Context, meta fwkDevice.CaptureMetadataInfo, extras fwkDevice.CaptureResultExtras, _ []fwkDevice.PhysicalCaptureResultInfo) error {
	fmt.Printf("  >> OnResultReceived: requestId=%d frameNumber=%d tag=%d\n",
		extras.RequestId, extras.FrameNumber, meta.Tag)
	return nil
}
func (c *callback) OnClientSharedAccessPriorityChanged(_ context.Context, primary bool) error {
...
// Pixel formats.
const (
	halPixelFormatImplementationDefined = 0x22 // HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED
...
func writeGraphicBufferToParcel(p *parcel.Parcel, buf *slotBuffer, bufID uint64) {
...
func bufferSizeForFormat(w, h uint32, format int32) int64 {
	switch format {
	case halPixelFormatYCbCr420_888:
...
// createStreamRawDeferred creates a deferred stream using raw parcel I/O,
// to verify the base OutputConfiguration format is correct.
func createStreamRawDeferred(
	ctx context.Context,
	...
	data.WriteInt32(0)
	data.WriteInt32(0)  // rotation
	data.WriteInt32(-1) // windowGroupId
	data.WriteString16("")
	data.WriteInt32(width)
	data.WriteInt32(height)
	data.WriteBool(true) // isDeferred

	// surfaces: empty array (for v2 compatibility)
	data.WriteInt32(0)

	fmt.Printf("  [DEBUG] Deferred parcel pre-footer size: %d\n", data.Len())

	parcel.WriteParcelableFooter(data, headerPos)

	fmt.Printf("  [DEBUG] Deferred parcel size: %d, objects: %v\n", data.Len(), data.Objects())
	fmt.Printf("  [DEBUG] Deferred parcel hex: %s\n", hex.EncodeToString(data.Data()))
...
// submitRequestListFixed sends a SubmitRequestList with correct NDK AIDL
// wire format. The NDK backend writes int32(1) null indicators before
// parcelable types in arrays (via AParcel_writeParcelable) and before
// parcelable union variants (via WriteToParcelFor -> AParcel_writeParcelable).
func submitRequestListFixed(
	ctx context.Context,
	deviceUser fwkDevice.ICameraDeviceUser,
	metadataBytes []byte,
	streamId int32,
	...
	// requestList: array of 1 CaptureRequest
	data.WriteInt32(1)

	// CaptureRequest element: AParcel_writeParcelable writes int32(1) + writeToParcel
	data.WriteInt32(1) // non-null indicator
	reqHeaderPos := parcel.WriteParcelableHeader(data)

	// physicalCameraSettings: array of 1
	data.WriteInt32(1)
	// PhysicalCameraSettings element: AParcel_writeParcelable writes int32(1) + writeToParcel
	data.WriteInt32(1) // non-null indicator
	pcsHeaderPos := parcel.WriteParcelableHeader(data)
	data.WriteString16("0") // id

	// CaptureMetadataInfo: INLINE non-nullable union field.
	// Inline fields in a parcelable DO NOT get null indicators; they are
	// written directly via writeToParcel.
	cmiHeaderPos := parcel.WriteParcelableHeader(data)
	data.WriteInt32(fwkDevice.CaptureMetadataInfoTagMetadata) // tag = metadata

	// CameraMetadata: union variant. The NDK-generated union writeToParcel
	// uses WriteToParcelFor -> AParcel_writeParcelable, which writes int32(1)
	// before calling writeToParcel.
	data.WriteInt32(1) // non-null indicator for CameraMetadata variant
	cmHeaderPos := parcel.WriteParcelableHeader(data)
	data.WriteByteArray(metadataBytes)
	parcel.WriteParcelableFooter(data, cmHeaderPos)
	parcel.WriteParcelableFooter(data, cmiHeaderPos)
	parcel.WriteParcelableFooter(data, pcsHeaderPos)

	// streamAndWindowIds: array of 1
	data.WriteInt32(1)
	// StreamAndWindowId element: AParcel_writeParcelable writes int32(1)
	data.WriteInt32(1) // non-null indicator
	sawHeaderPos := parcel.WriteParcelableHeader(data)
	data.WriteInt32(streamId)
	data.WriteInt32(0) // windowId
	parcel.WriteParcelableFooter(data, sawHeaderPos)

	parcel.WriteParcelableFooter(data, reqHeaderPos)
	data.WriteBool(isRepeating)

	fmt.Printf("  [DEBUG] submitRequestListFixed: size=%d\n", data.Len())
...
// submitRequestListRawFmq sends a SubmitRequestList using fmqMetadataSize=0
// (empty metadata via FMQ path) to test if the parcel structure is correct
// without the metadata bytes.
func submitRequestListRawFmq(
	ctx context.Context,
	deviceUser fwkDevice.ICameraDeviceUser,
	streamId int32,
	isRepeating bool,
	...
	data.WriteInt32(1) // CaptureRequest non-null
	reqHeaderPos := parcel.WriteParcelableHeader(data)

	// physicalCameraSettings: array of 1
	data.WriteInt32(1)
	data.WriteInt32(1) // PhysicalCameraSettings non-null
	pcsHeaderPos := parcel.WriteParcelableHeader(data)
	data.WriteString16("0") // id

	data.WriteInt32(1) // CaptureMetadataInfo non-null
	cmiHeaderPos := parcel.WriteParcelableHeader(data)
	data.WriteInt32(fwkDevice.CaptureMetadataInfoTagFmqMetadataSize) // tag = fmqMetadataSize
	data.WriteInt64(0)                                                // fmqMetadataSize = 0
	parcel.WriteParcelableFooter(data, cmiHeaderPos)
	parcel.WriteParcelableFooter(data, pcsHeaderPos)

	// streamAndWindowIds: array of 1
	data.WriteInt32(1)
	data.WriteInt32(1) // StreamAndWindowId non-null
	sawHeaderPos := parcel.WriteParcelableHeader(data)
	data.WriteInt32(streamId)
	data.WriteInt32(0) // windowId
	parcel.WriteParcelableFooter(data, sawHeaderPos)
	...
	fmt.Printf("  [DEBUG] SubmitRequest(fmq) parcel size: %d, hex: %s\n",
		data.Len(), hex.EncodeToString(data.Data()))
...
// submitRequestListRaw sends a SubmitRequestList using raw parcel I/O.
func submitRequestListRaw(
	ctx context.Context,
	deviceUser fwkDevice.ICameraDeviceUser,
	metadataBytes []byte,
	streamId int32,
	...
	// NDK AIDL format: AParcel_writeParcelable writes int32(1) (non-null)
	// before array elements (via AParcel_writeStdVectorParcelableElement).
	// However, inline non-nullable parcelable/union fields within a parcelable
	// are written directly via writeToParcel (no null indicator).
	data.WriteInt32(1) // requestList: array of 1

	data.WriteInt32(1) // CaptureRequest non-null (array element)
	reqHeaderPos := parcel.WriteParcelableHeader(data)

	// physicalCameraSettings: array of 1
	data.WriteInt32(1)
	data.WriteInt32(1) // PhysicalCameraSettings non-null (array element)
	pcsHeaderPos := parcel.WriteParcelableHeader(data)
	data.WriteString16("0") // id

	// CaptureMetadataInfo: inline non-nullable field, NO null indicator.
	cmiStartPos := data.Len()
	cmiHeaderPos := parcel.WriteParcelableHeader(data)
	data.WriteInt32(fwkDevice.CaptureMetadataInfoTagMetadata) // tag = metadata

	// CameraMetadata: inline non-nullable field within union, NO null indicator.
	cmStartPos := data.Len()
	cmHeaderPos := parcel.WriteParcelableHeader(data)
	metaStartPos := data.Len()
	data.WriteByteArray(metadataBytes) // byte[] metadata (compact, not per-byte padded)
	parcel.WriteParcelableFooter(data, cmHeaderPos)
	parcel.WriteParcelableFooter(data, cmiHeaderPos)
	parcel.WriteParcelableFooter(data, pcsHeaderPos)

	// Dump the metadata section of the parcel for debugging.
	fmt.Printf("  [DEBUG] CMI starts at parcel offset %d\n", cmiStartPos)
	fmt.Printf("  [DEBUG] CM starts at parcel offset %d\n", cmStartPos)
	fmt.Printf("  [DEBUG] Metadata byte[] starts at parcel offset %d\n", metaStartPos)
	// Show the bytes around the metadata start.
	dumpStart := cmiStartPos
	dumpEnd := metaStartPos + 68 // first 64 bytes of metadata + 4-byte length
	if dumpEnd > data.Len() {
		dumpEnd = data.Len()
	}
	fmt.Printf("  [DEBUG] Parcel bytes [%d:%d]: %s\n", dumpStart, dumpEnd,
		hex.EncodeToString(data.Data()[dumpStart:dumpEnd]))

	// streamAndWindowIds: array of 1
	data.WriteInt32(1)
	data.WriteInt32(1) // StreamAndWindowId non-null (array element)
	sawHeaderPos := parcel.WriteParcelableHeader(data)
	data.WriteInt32(streamId)
	data.WriteInt32(0) // windowId
	parcel.WriteParcelableFooter(data, sawHeaderPos)
	...
	txCode := fwkDevice.TransactionICameraDeviceUserSubmitRequestList
	handle := deviceUser.AsBinder().Handle()
	fmt.Printf("  [DEBUG] SubmitRequest: handle=%d, code=%d, parcel size=%d, objects=%v\n",
		handle, txCode, data.Len(), data.Objects())
