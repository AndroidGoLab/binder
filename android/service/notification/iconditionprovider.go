...
func (p *ConditionProviderProxy) OnSubscribe(
	...
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConditionProvider)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConditionProvider, MethodIConditionProviderOnSubscribe)
...
func (p *ConditionProviderProxy) OnUnsubscribe(
	...
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIConditionProvider)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIConditionProvider, MethodIConditionProviderOnUnsubscribe)
	_compiledDescs := []string{
		"Landroid/net/Uri;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
