package location

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIGnssStatusListener = "android.location.IGnssStatusListener"

const (
	TransactionIGnssStatusListenerOnGnssStarted     = binder.FirstCallTransaction + 0
	TransactionIGnssStatusListenerOnGnssStopped     = binder.FirstCallTransaction + 1
	TransactionIGnssStatusListenerOnFirstFix        = binder.FirstCallTransaction + 2
	TransactionIGnssStatusListenerOnSvStatusChanged = binder.FirstCallTransaction + 3
)

type IGnssStatusListener interface {
	AsBinder() binder.IBinder
	OnGnssStarted(ctx context.Context) error
	OnGnssStopped(ctx context.Context) error
	OnFirstFix(ctx context.Context, ttff int32) error
	OnSvStatusChanged(ctx context.Context, gnssStatus GnssStatus) error
}

type GnssStatusListenerProxy struct {
	remote binder.IBinder
}

func NewGnssStatusListenerProxy(
	remote binder.IBinder,
) *GnssStatusListenerProxy {
	return &GnssStatusListenerProxy{remote: remote}
}

func (p *GnssStatusListenerProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IGnssStatusListener = (*GnssStatusListenerProxy)(nil)

func (p *GnssStatusListenerProxy) OnGnssStarted(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIGnssStatusListener)

	_, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIGnssStatusListener, "onGnssStarted"), binder.FlagOneway, _data)
	return _err
}

func (p *GnssStatusListenerProxy) OnGnssStopped(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIGnssStatusListener)

	_, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIGnssStatusListener, "onGnssStopped"), binder.FlagOneway, _data)
	return _err
}

func (p *GnssStatusListenerProxy) OnFirstFix(
	ctx context.Context,
	ttff int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIGnssStatusListener)
	_data.WriteInt32(ttff)

	_, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIGnssStatusListener, "onFirstFix"), binder.FlagOneway, _data)
	return _err
}

func (p *GnssStatusListenerProxy) OnSvStatusChanged(
	ctx context.Context,
	gnssStatus GnssStatus,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIGnssStatusListener)
	if _err := gnssStatus.MarshalParcel(_data); _err != nil {
		return _err
	}

	_, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIGnssStatusListener, "onSvStatusChanged"), binder.FlagOneway, _data)
	return _err
}
