package shared

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIHomeControlsRemoteProxy = "com.android.systemui.dreams.homecontrols.shared.IHomeControlsRemoteProxy"

const (
	TransactionIHomeControlsRemoteProxyRegisterListenerForCurrentUser   = binder.FirstCallTransaction + 0
	TransactionIHomeControlsRemoteProxyUnregisterListenerForCurrentUser = binder.FirstCallTransaction + 1
)

type IHomeControlsRemoteProxy interface {
	AsBinder() binder.IBinder
	RegisterListenerForCurrentUser(ctx context.Context, callback IOnControlsSettingsChangeListener) error
	UnregisterListenerForCurrentUser(ctx context.Context, callback IOnControlsSettingsChangeListener) error
}

type HomeControlsRemoteProxyProxy struct {
	remote binder.IBinder
}

func NewHomeControlsRemoteProxyProxy(
	remote binder.IBinder,
) *HomeControlsRemoteProxyProxy {
	return &HomeControlsRemoteProxyProxy{remote: remote}
}

func (p *HomeControlsRemoteProxyProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IHomeControlsRemoteProxy = (*HomeControlsRemoteProxyProxy)(nil)

func (p *HomeControlsRemoteProxyProxy) RegisterListenerForCurrentUser(
	ctx context.Context,
	callback IOnControlsSettingsChangeListener,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIHomeControlsRemoteProxy)
	_data.WriteStrongBinder(callback.AsBinder().Handle())

	_, _err := p.remote.Transact(ctx, TransactionIHomeControlsRemoteProxyRegisterListenerForCurrentUser, binder.FlagOneway, _data)
	return _err
}

func (p *HomeControlsRemoteProxyProxy) UnregisterListenerForCurrentUser(
	ctx context.Context,
	callback IOnControlsSettingsChangeListener,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIHomeControlsRemoteProxy)
	_data.WriteStrongBinder(callback.AsBinder().Handle())

	_, _err := p.remote.Transact(ctx, TransactionIHomeControlsRemoteProxyUnregisterListenerForCurrentUser, binder.FlagOneway, _data)
	return _err
}
