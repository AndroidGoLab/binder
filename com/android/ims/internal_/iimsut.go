package internal

import (
	"context"
	os "github.com/xaionaro-go/aidl/android/os"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIImsUt = "com.android.ims.internal.IImsUt"

const (
	TransactionIImsUtClose                            = binder.FirstCallTransaction + 0
	TransactionIImsUtQueryCallBarring                 = binder.FirstCallTransaction + 1
	TransactionIImsUtQueryCallForward                 = binder.FirstCallTransaction + 2
	TransactionIImsUtQueryCallWaiting                 = binder.FirstCallTransaction + 3
	TransactionIImsUtQueryCLIR                        = binder.FirstCallTransaction + 4
	TransactionIImsUtQueryCLIP                        = binder.FirstCallTransaction + 5
	TransactionIImsUtQueryCOLR                        = binder.FirstCallTransaction + 6
	TransactionIImsUtQueryCOLP                        = binder.FirstCallTransaction + 7
	TransactionIImsUtTransact                         = binder.FirstCallTransaction + 8
	TransactionIImsUtUpdateCallBarring                = binder.FirstCallTransaction + 9
	TransactionIImsUtUpdateCallForward                = binder.FirstCallTransaction + 10
	TransactionIImsUtUpdateCallWaiting                = binder.FirstCallTransaction + 11
	TransactionIImsUtUpdateCLIR                       = binder.FirstCallTransaction + 12
	TransactionIImsUtUpdateCLIP                       = binder.FirstCallTransaction + 13
	TransactionIImsUtUpdateCOLR                       = binder.FirstCallTransaction + 14
	TransactionIImsUtUpdateCOLP                       = binder.FirstCallTransaction + 15
	TransactionIImsUtSetListener                      = binder.FirstCallTransaction + 16
	TransactionIImsUtQueryCallBarringForServiceClass  = binder.FirstCallTransaction + 17
	TransactionIImsUtUpdateCallBarringForServiceClass = binder.FirstCallTransaction + 18
	TransactionIImsUtUpdateCallBarringWithPassword    = binder.FirstCallTransaction + 19
)

type IImsUt interface {
	AsBinder() binder.IBinder
	Close(ctx context.Context) error
	QueryCallBarring(ctx context.Context, cbType int32) (int32, error)
	QueryCallForward(ctx context.Context, condition int32, number string) (int32, error)
	QueryCallWaiting(ctx context.Context) (int32, error)
	QueryCLIR(ctx context.Context) (int32, error)
	QueryCLIP(ctx context.Context) (int32, error)
	QueryCOLR(ctx context.Context) (int32, error)
	QueryCOLP(ctx context.Context) (int32, error)
	Transact(ctx context.Context, ssInfo os.Bundle) (int32, error)
	UpdateCallBarring(ctx context.Context, cbType int32, action int32, barrList []string) (int32, error)
	UpdateCallForward(ctx context.Context, action int32, condition int32, number string, serviceClass int32, timeSeconds int32) (int32, error)
	UpdateCallWaiting(ctx context.Context, enable bool, serviceClass int32) (int32, error)
	UpdateCLIR(ctx context.Context, clirMode int32) (int32, error)
	UpdateCLIP(ctx context.Context, enable bool) (int32, error)
	UpdateCOLR(ctx context.Context, presentation int32) (int32, error)
	UpdateCOLP(ctx context.Context, enable bool) (int32, error)
	SetListener(ctx context.Context, listener IImsUtListener) error
	QueryCallBarringForServiceClass(ctx context.Context, cbType int32, serviceClass int32) (int32, error)
	UpdateCallBarringForServiceClass(ctx context.Context, cbType int32, action int32, barrList []string, serviceClass int32) (int32, error)
	UpdateCallBarringWithPassword(ctx context.Context, cbType int32, action int32, barrList []string, serviceClass int32, password string) (int32, error)
}

type ImsUtProxy struct {
	remote binder.IBinder
}

func NewImsUtProxy(
	remote binder.IBinder,
) *ImsUtProxy {
	return &ImsUtProxy{remote: remote}
}

func (p *ImsUtProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IImsUt = (*ImsUtProxy)(nil)

func (p *ImsUtProxy) Close(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "close"), 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *ImsUtProxy) QueryCallBarring(
	ctx context.Context,
	cbType int32,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)
	_data.WriteInt32(cbType)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "queryCallBarring"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) QueryCallForward(
	ctx context.Context,
	condition int32,
	number string,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)
	_data.WriteInt32(condition)
	_data.WriteString16(number)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "queryCallForward"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) QueryCallWaiting(
	ctx context.Context,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "queryCallWaiting"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) QueryCLIR(
	ctx context.Context,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "queryCLIR"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) QueryCLIP(
	ctx context.Context,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "queryCLIP"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) QueryCOLR(
	ctx context.Context,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "queryCOLR"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) QueryCOLP(
	ctx context.Context,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "queryCOLP"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) Transact(
	ctx context.Context,
	ssInfo os.Bundle,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)
	if _err := ssInfo.MarshalParcel(_data); _err != nil {
		return _result, _err
	}

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "transact"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) UpdateCallBarring(
	ctx context.Context,
	cbType int32,
	action int32,
	barrList []string,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)
	_data.WriteInt32(cbType)
	_data.WriteInt32(action)
	if barrList == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(barrList)))
		for _, _item := range barrList {
			_data.WriteString16(_item)
		}
	}

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "updateCallBarring"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) UpdateCallForward(
	ctx context.Context,
	action int32,
	condition int32,
	number string,
	serviceClass int32,
	timeSeconds int32,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)
	_data.WriteInt32(action)
	_data.WriteInt32(condition)
	_data.WriteString16(number)
	_data.WriteInt32(serviceClass)
	_data.WriteInt32(timeSeconds)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "updateCallForward"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) UpdateCallWaiting(
	ctx context.Context,
	enable bool,
	serviceClass int32,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)
	_data.WriteBool(enable)
	_data.WriteInt32(serviceClass)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "updateCallWaiting"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) UpdateCLIR(
	ctx context.Context,
	clirMode int32,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)
	_data.WriteInt32(clirMode)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "updateCLIR"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) UpdateCLIP(
	ctx context.Context,
	enable bool,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)
	_data.WriteBool(enable)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "updateCLIP"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) UpdateCOLR(
	ctx context.Context,
	presentation int32,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)
	_data.WriteInt32(presentation)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "updateCOLR"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) UpdateCOLP(
	ctx context.Context,
	enable bool,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)
	_data.WriteBool(enable)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "updateCOLP"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) SetListener(
	ctx context.Context,
	listener IImsUtListener,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)
	_data.WriteStrongBinder(listener.AsBinder().Handle())

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "setListener"), 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *ImsUtProxy) QueryCallBarringForServiceClass(
	ctx context.Context,
	cbType int32,
	serviceClass int32,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)
	_data.WriteInt32(cbType)
	_data.WriteInt32(serviceClass)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "queryCallBarringForServiceClass"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) UpdateCallBarringForServiceClass(
	ctx context.Context,
	cbType int32,
	action int32,
	barrList []string,
	serviceClass int32,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)
	_data.WriteInt32(cbType)
	_data.WriteInt32(action)
	if barrList == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(barrList)))
		for _, _item := range barrList {
			_data.WriteString16(_item)
		}
	}
	_data.WriteInt32(serviceClass)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "updateCallBarringForServiceClass"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *ImsUtProxy) UpdateCallBarringWithPassword(
	ctx context.Context,
	cbType int32,
	action int32,
	barrList []string,
	serviceClass int32,
	password string,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIImsUt)
	_data.WriteInt32(cbType)
	_data.WriteInt32(action)
	if barrList == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(barrList)))
		for _, _item := range barrList {
			_data.WriteString16(_item)
		}
	}
	_data.WriteInt32(serviceClass)
	_data.WriteString16(password)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIImsUt, "updateCallBarringWithPassword"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}
