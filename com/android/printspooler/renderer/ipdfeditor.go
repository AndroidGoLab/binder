package renderer

import (
	"context"
	print "github.com/xaionaro-go/aidl/android/print"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIPdfEditor = "com.android.printspooler.renderer.IPdfEditor"

const (
	TransactionIPdfEditorOpenDocument         = binder.FirstCallTransaction + 0
	TransactionIPdfEditorRemovePages          = binder.FirstCallTransaction + 1
	TransactionIPdfEditorApplyPrintAttributes = binder.FirstCallTransaction + 2
	TransactionIPdfEditorWrite                = binder.FirstCallTransaction + 3
	TransactionIPdfEditorCloseDocument        = binder.FirstCallTransaction + 4
)

type IPdfEditor interface {
	AsBinder() binder.IBinder
	OpenDocument(ctx context.Context, source int32) (int32, error)
	RemovePages(ctx context.Context, pages []print.PageRange) error
	ApplyPrintAttributes(ctx context.Context, attributes print.PrintAttributes) error
	Write(ctx context.Context, destination int32) error
	CloseDocument(ctx context.Context) error
}

type PdfEditorProxy struct {
	remote binder.IBinder
}

func NewPdfEditorProxy(
	remote binder.IBinder,
) *PdfEditorProxy {
	return &PdfEditorProxy{remote: remote}
}

func (p *PdfEditorProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IPdfEditor = (*PdfEditorProxy)(nil)

func (p *PdfEditorProxy) OpenDocument(
	ctx context.Context,
	source int32,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIPdfEditor)
	_data.WriteFileDescriptor(source)

	_reply, _err := p.remote.Transact(ctx, TransactionIPdfEditorOpenDocument, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *PdfEditorProxy) RemovePages(
	ctx context.Context,
	pages []print.PageRange,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIPdfEditor)
	if pages == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(pages)))
		for _, _item := range pages {
			if _err := _item.MarshalParcel(_data); _err != nil {
				return _err
			}
		}
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIPdfEditorRemovePages, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *PdfEditorProxy) ApplyPrintAttributes(
	ctx context.Context,
	attributes print.PrintAttributes,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIPdfEditor)
	if _err := attributes.MarshalParcel(_data); _err != nil {
		return _err
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIPdfEditorApplyPrintAttributes, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *PdfEditorProxy) Write(
	ctx context.Context,
	destination int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIPdfEditor)
	_data.WriteFileDescriptor(destination)

	_reply, _err := p.remote.Transact(ctx, TransactionIPdfEditorWrite, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *PdfEditorProxy) CloseDocument(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIPdfEditor)

	_reply, _err := p.remote.Transact(ctx, TransactionIPdfEditorCloseDocument, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
