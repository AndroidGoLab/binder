package view

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIRemoteAnimationRunner = "android.view.IRemoteAnimationRunner"

const (
	TransactionIRemoteAnimationRunnerOnAnimationStart     = binder.FirstCallTransaction + 0
	TransactionIRemoteAnimationRunnerOnAnimationCancelled = binder.FirstCallTransaction + 1
)

type IRemoteAnimationRunner interface {
	AsBinder() binder.IBinder
	OnAnimationStart(ctx context.Context, transit int32, apps []RemoteAnimationTarget, wallpapers []RemoteAnimationTarget, nonApps []RemoteAnimationTarget, finishedCallback IRemoteAnimationFinishedCallback) error
	OnAnimationCancelled(ctx context.Context) error
}

type RemoteAnimationRunnerProxy struct {
	remote binder.IBinder
}

func NewRemoteAnimationRunnerProxy(
	remote binder.IBinder,
) *RemoteAnimationRunnerProxy {
	return &RemoteAnimationRunnerProxy{remote: remote}
}

func (p *RemoteAnimationRunnerProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IRemoteAnimationRunner = (*RemoteAnimationRunnerProxy)(nil)

func (p *RemoteAnimationRunnerProxy) OnAnimationStart(
	ctx context.Context,
	transit int32,
	apps []RemoteAnimationTarget,
	wallpapers []RemoteAnimationTarget,
	nonApps []RemoteAnimationTarget,
	finishedCallback IRemoteAnimationFinishedCallback,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIRemoteAnimationRunner)
	_data.WriteInt32(transit)
	if apps == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(apps)))
		for _, _item := range apps {
			if _err := _item.MarshalParcel(_data); _err != nil {
				return _err
			}
		}
	}
	if wallpapers == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(wallpapers)))
		for _, _item := range wallpapers {
			if _err := _item.MarshalParcel(_data); _err != nil {
				return _err
			}
		}
	}
	if nonApps == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(nonApps)))
		for _, _item := range nonApps {
			if _err := _item.MarshalParcel(_data); _err != nil {
				return _err
			}
		}
	}
	_data.WriteStrongBinder(finishedCallback.AsBinder().Handle())

	_, _err := p.remote.Transact(ctx, TransactionIRemoteAnimationRunnerOnAnimationStart, binder.FlagOneway, _data)
	return _err
}

func (p *RemoteAnimationRunnerProxy) OnAnimationCancelled(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIRemoteAnimationRunner)

	_, _err := p.remote.Transact(ctx, TransactionIRemoteAnimationRunnerOnAnimationCancelled, binder.FlagOneway, _data)
	return _err
}
