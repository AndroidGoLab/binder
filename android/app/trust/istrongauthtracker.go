package trust

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIStrongAuthTracker = "android.app.trust.IStrongAuthTracker"

const (
	TransactionIStrongAuthTrackerOnStrongAuthRequiredChanged          = binder.FirstCallTransaction + 0
	TransactionIStrongAuthTrackerOnIsNonStrongBiometricAllowedChanged = binder.FirstCallTransaction + 1
)

type IStrongAuthTracker interface {
	AsBinder() binder.IBinder
	OnStrongAuthRequiredChanged(ctx context.Context, strongAuthRequired int32, userId int32) error
	OnIsNonStrongBiometricAllowedChanged(ctx context.Context, allowed bool, userId int32) error
}

type StrongAuthTrackerProxy struct {
	remote binder.IBinder
}

func NewStrongAuthTrackerProxy(
	remote binder.IBinder,
) *StrongAuthTrackerProxy {
	return &StrongAuthTrackerProxy{remote: remote}
}

func (p *StrongAuthTrackerProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IStrongAuthTracker = (*StrongAuthTrackerProxy)(nil)

func (p *StrongAuthTrackerProxy) OnStrongAuthRequiredChanged(
	ctx context.Context,
	strongAuthRequired int32,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIStrongAuthTracker)
	_data.WriteInt32(strongAuthRequired)
	_data.WriteInt32(userId)

	_, _err := p.remote.Transact(ctx, TransactionIStrongAuthTrackerOnStrongAuthRequiredChanged, binder.FlagOneway, _data)
	return _err
}

func (p *StrongAuthTrackerProxy) OnIsNonStrongBiometricAllowedChanged(
	ctx context.Context,
	allowed bool,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIStrongAuthTracker)
	_data.WriteBool(allowed)
	_data.WriteInt32(userId)

	_, _err := p.remote.Transact(ctx, TransactionIStrongAuthTrackerOnIsNonStrongBiometricAllowedChanged, binder.FlagOneway, _data)
	return _err
}
