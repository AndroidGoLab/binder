// Code generated by genparcelgo. DO NOT EDIT.

package ims

import (
	"github.com/xaionaro-go/binder/parcel"
)

type ImsCallProfile struct {
	ServiceType                    int32
	CallType                       int32
	EmergencyServiceCategories     int32
	EmergencyCallRouting           int32
	EmergencyCallTesting           bool
	HasKnownUserIntentEmergency    bool
	RestrictCause                  int32
	CallerNumberVerificationStatus int32
	// FilteredExtras: opaque type — not yet supported
	// MediaProfile: opaque type — not yet supported
	// EmergencyUrns: opaque type — not yet supported
	// AcceptedRtpHeaderExtensionTypes.toArray(): opaque type — not yet supported
}

var _ parcel.Parcelable = (*ImsCallProfile)(nil)

func (s *ImsCallProfile) MarshalParcel(
	p *parcel.Parcel,
) error {
	p.WriteInt32(s.ServiceType)
	p.WriteInt32(s.CallType)
	p.WriteInt32(s.EmergencyServiceCategories)
	p.WriteInt32(s.EmergencyCallRouting)
	p.WriteBool(s.EmergencyCallTesting)
	p.WriteBool(s.HasKnownUserIntentEmergency)
	p.WriteInt32(s.RestrictCause)
	p.WriteInt32(s.CallerNumberVerificationStatus)
	return nil
}

func (s *ImsCallProfile) UnmarshalParcel(
	p *parcel.Parcel,
) error {
	var _err error
	s.ServiceType, _err = p.ReadInt32()
	if _err != nil {
		return _err
	}
	s.CallType, _err = p.ReadInt32()
	if _err != nil {
		return _err
	}
	s.EmergencyServiceCategories, _err = p.ReadInt32()
	if _err != nil {
		return _err
	}
	s.EmergencyCallRouting, _err = p.ReadInt32()
	if _err != nil {
		return _err
	}
	s.EmergencyCallTesting, _err = p.ReadBool()
	if _err != nil {
		return _err
	}
	s.HasKnownUserIntentEmergency, _err = p.ReadBool()
	if _err != nil {
		return _err
	}
	s.RestrictCause, _err = p.ReadInt32()
	if _err != nil {
		return _err
	}
	s.CallerNumberVerificationStatus, _err = p.ReadInt32()
	if _err != nil {
		return _err
	}
	return nil
}
