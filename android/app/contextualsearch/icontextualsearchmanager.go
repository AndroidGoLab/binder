package contextualsearch

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIContextualSearchManager = "android.app.contextualsearch.IContextualSearchManager"

const (
	TransactionIContextualSearchManagerStartContextualSearch    = binder.FirstCallTransaction + 0
	TransactionIContextualSearchManagerGetContextualSearchState = binder.FirstCallTransaction + 1
)

type IContextualSearchManager interface {
	AsBinder() binder.IBinder
	StartContextualSearch(ctx context.Context, entrypoint int32) error
	GetContextualSearchState(ctx context.Context, token binder.IBinder, callback IContextualSearchCallback) error
}

type ContextualSearchManagerProxy struct {
	remote binder.IBinder
}

func NewContextualSearchManagerProxy(
	remote binder.IBinder,
) *ContextualSearchManagerProxy {
	return &ContextualSearchManagerProxy{remote: remote}
}

func (p *ContextualSearchManagerProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IContextualSearchManager = (*ContextualSearchManagerProxy)(nil)

func (p *ContextualSearchManagerProxy) StartContextualSearch(
	ctx context.Context,
	entrypoint int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIContextualSearchManager)
	_data.WriteInt32(entrypoint)

	_, _err := p.remote.Transact(ctx, TransactionIContextualSearchManagerStartContextualSearch, binder.FlagOneway, _data)
	return _err
}

func (p *ContextualSearchManagerProxy) GetContextualSearchState(
	ctx context.Context,
	token binder.IBinder,
	callback IContextualSearchCallback,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIContextualSearchManager)
	_data.WriteStrongBinder(token.Handle())
	_data.WriteStrongBinder(callback.AsBinder().Handle())

	_, _err := p.remote.Transact(ctx, TransactionIContextualSearchManagerGetContextualSearchState, binder.FlagOneway, _data)
	return _err
}
