package omapi

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorISecureElementService = "android.se.omapi.ISecureElementService"

const (
	TransactionISecureElementServiceGetReaders        = binder.FirstCallTransaction + 0
	TransactionISecureElementServiceGetReader         = binder.FirstCallTransaction + 1
	TransactionISecureElementServiceIsNfcEventAllowed = binder.FirstCallTransaction + 2
)

type ISecureElementService interface {
	AsBinder() binder.IBinder
	GetReaders(ctx context.Context) ([]string, error)
	GetReader(ctx context.Context, reader string) (ISecureElementReader, error)
	IsNfcEventAllowed(ctx context.Context, reader string, aid []byte, packageNames []string, userId int32) ([]bool, error)
}

type SecureElementServiceProxy struct {
	remote binder.IBinder
}

func NewSecureElementServiceProxy(
	remote binder.IBinder,
) *SecureElementServiceProxy {
	return &SecureElementServiceProxy{remote: remote}
}

func (p *SecureElementServiceProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ ISecureElementService = (*SecureElementServiceProxy)(nil)

func (p *SecureElementServiceProxy) GetReaders(
	ctx context.Context,
) ([]string, error) {
	var _result []string
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISecureElementService)

	_reply, _err := p.remote.Transact(ctx, TransactionISecureElementServiceGetReaders, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_count, _err := _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}

	if _count >= 0 {
		_result = make([]string, _count)
		for _i := int32(0); _i < _count; _i++ {
			_result[_i], _err = _reply.ReadString16()
			if _err != nil {
				return _result, _err
			}
		}
	}
	return _result, nil
}

func (p *SecureElementServiceProxy) GetReader(
	ctx context.Context,
	reader string,
) (ISecureElementReader, error) {
	var _result ISecureElementReader
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISecureElementService)
	_data.WriteString16(reader)

	_reply, _err := p.remote.Transact(ctx, TransactionISecureElementServiceGetReader, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_handle, _err := _reply.ReadStrongBinder()
	if _err != nil {
		return _result, _err
	}
	_result = NewSecureElementReaderProxy(binder.NewProxyBinder(p.remote.Transport(), _handle))
	return _result, nil
}

func (p *SecureElementServiceProxy) IsNfcEventAllowed(
	ctx context.Context,
	reader string,
	aid []byte,
	packageNames []string,
	userId int32,
) ([]bool, error) {
	var _result []bool
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorISecureElementService)
	_data.WriteString16(reader)
	if aid == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(aid)))
		for _, _item := range aid {
			_data.WritePaddedByte(_item)
		}
	}
	if packageNames == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(packageNames)))
		for _, _item := range packageNames {
			_data.WriteString16(_item)
		}
	}
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionISecureElementServiceIsNfcEventAllowed, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_count, _err := _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}

	if _count >= 0 {
		_result = make([]bool, _count)
		for _i := int32(0); _i < _count; _i++ {
			_result[_i], _err = _reply.ReadBool()
			if _err != nil {
				return _result, _err
			}
		}
	}
	return _result, nil
}
