package cec

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIHdmiCec = "android.hardware.tv.hdmi.cec.IHdmiCec"

const (
	TransactionIHdmiCecAddLogicalAddress        = binder.FirstCallTransaction + 0
	TransactionIHdmiCecClearLogicalAddress      = binder.FirstCallTransaction + 1
	TransactionIHdmiCecEnableAudioReturnChannel = binder.FirstCallTransaction + 2
	TransactionIHdmiCecGetCecVersion            = binder.FirstCallTransaction + 3
	TransactionIHdmiCecGetPhysicalAddress       = binder.FirstCallTransaction + 4
	TransactionIHdmiCecGetVendorId              = binder.FirstCallTransaction + 5
	TransactionIHdmiCecSendMessage              = binder.FirstCallTransaction + 6
	TransactionIHdmiCecSetCallback              = binder.FirstCallTransaction + 7
	TransactionIHdmiCecSetLanguage              = binder.FirstCallTransaction + 8
	TransactionIHdmiCecEnableWakeupByOtp        = binder.FirstCallTransaction + 9
	TransactionIHdmiCecEnableCec                = binder.FirstCallTransaction + 10
	TransactionIHdmiCecEnableSystemCecControl   = binder.FirstCallTransaction + 11
)

type IHdmiCec interface {
	AsBinder() binder.IBinder
	AddLogicalAddress(ctx context.Context, addr CecLogicalAddress) (Result, error)
	ClearLogicalAddress(ctx context.Context) error
	EnableAudioReturnChannel(ctx context.Context, portId int32, enable bool) error
	GetCecVersion(ctx context.Context) (int32, error)
	GetPhysicalAddress(ctx context.Context) (int32, error)
	GetVendorId(ctx context.Context) (int32, error)
	SendMessage(ctx context.Context, message CecMessage) (SendMessageResult, error)
	SetCallback(ctx context.Context, callback *IHdmiCecCallback) error
	SetLanguage(ctx context.Context, language string) error
	EnableWakeupByOtp(ctx context.Context, value bool) error
	EnableCec(ctx context.Context, value bool) error
	EnableSystemCecControl(ctx context.Context, value bool) error
}

type HdmiCecProxy struct {
	remote binder.IBinder
}

func NewHdmiCecProxy(
	remote binder.IBinder,
) *HdmiCecProxy {
	return &HdmiCecProxy{remote: remote}
}

func (p *HdmiCecProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IHdmiCec = (*HdmiCecProxy)(nil)

func (p *HdmiCecProxy) AddLogicalAddress(
	ctx context.Context,
	addr CecLogicalAddress,
) (Result, error) {
	var _result Result
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIHdmiCec)
	_data.WritePaddedByte(byte(addr))

	_reply, _err := p.remote.Transact(ctx, TransactionIHdmiCecAddLogicalAddress, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_raw, _err := _reply.ReadPaddedByte()
	if _err != nil {
		return _result, _err
	}
	_result = Result(_raw)
	return _result, nil
}

func (p *HdmiCecProxy) ClearLogicalAddress(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIHdmiCec)

	_reply, _err := p.remote.Transact(ctx, TransactionIHdmiCecClearLogicalAddress, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *HdmiCecProxy) EnableAudioReturnChannel(
	ctx context.Context,
	portId int32,
	enable bool,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIHdmiCec)
	_data.WriteInt32(portId)
	_data.WriteBool(enable)

	_reply, _err := p.remote.Transact(ctx, TransactionIHdmiCecEnableAudioReturnChannel, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *HdmiCecProxy) GetCecVersion(
	ctx context.Context,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIHdmiCec)

	_reply, _err := p.remote.Transact(ctx, TransactionIHdmiCecGetCecVersion, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *HdmiCecProxy) GetPhysicalAddress(
	ctx context.Context,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIHdmiCec)

	_reply, _err := p.remote.Transact(ctx, TransactionIHdmiCecGetPhysicalAddress, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *HdmiCecProxy) GetVendorId(
	ctx context.Context,
) (int32, error) {
	var _result int32
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIHdmiCec)

	_reply, _err := p.remote.Transact(ctx, TransactionIHdmiCecGetVendorId, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *HdmiCecProxy) SendMessage(
	ctx context.Context,
	message CecMessage,
) (SendMessageResult, error) {
	var _result SendMessageResult
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIHdmiCec)
	if _err := message.MarshalParcel(_data); _err != nil {
		return _result, _err
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIHdmiCecSendMessage, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_raw, _err := _reply.ReadPaddedByte()
	if _err != nil {
		return _result, _err
	}
	_result = SendMessageResult(_raw)
	return _result, nil
}

func (p *HdmiCecProxy) SetCallback(
	ctx context.Context,
	callback *IHdmiCecCallback,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIHdmiCec)
	if callback != nil {
		_data.WriteStrongBinder((*callback).AsBinder().Handle())
	} else {
		_data.WriteInt32(-1)
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIHdmiCecSetCallback, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *HdmiCecProxy) SetLanguage(
	ctx context.Context,
	language string,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIHdmiCec)
	_data.WriteString16(language)

	_reply, _err := p.remote.Transact(ctx, TransactionIHdmiCecSetLanguage, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *HdmiCecProxy) EnableWakeupByOtp(
	ctx context.Context,
	value bool,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIHdmiCec)
	_data.WriteBool(value)

	_reply, _err := p.remote.Transact(ctx, TransactionIHdmiCecEnableWakeupByOtp, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *HdmiCecProxy) EnableCec(
	ctx context.Context,
	value bool,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIHdmiCec)
	_data.WriteBool(value)

	_reply, _err := p.remote.Transact(ctx, TransactionIHdmiCecEnableCec, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *HdmiCecProxy) EnableSystemCecControl(
	ctx context.Context,
	value bool,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIHdmiCec)
	_data.WriteBool(value)

	_reply, _err := p.remote.Transact(ctx, TransactionIHdmiCecEnableSystemCecControl, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
