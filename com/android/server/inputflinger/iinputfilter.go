package inputflinger

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIInputFilter = "com.android.server.inputflinger.IInputFilter"

const (
	TransactionIInputFilterIsEnabled                  = binder.FirstCallTransaction + 0
	TransactionIInputFilterNotifyKey                  = binder.FirstCallTransaction + 1
	TransactionIInputFilterNotifyInputDevicesChanged  = binder.FirstCallTransaction + 2
	TransactionIInputFilterNotifyConfigurationChanged = binder.FirstCallTransaction + 3
	TransactionIInputFilterDumpFilter                 = binder.FirstCallTransaction + 4
)

type IInputFilter interface {
	AsBinder() binder.IBinder
	IsEnabled(ctx context.Context) (bool, error)
	NotifyKey(ctx context.Context, event KeyEvent) error
	NotifyInputDevicesChanged(ctx context.Context, deviceInfos []DeviceInfo) error
	NotifyConfigurationChanged(ctx context.Context, config InputFilterConfiguration) error
	DumpFilter(ctx context.Context) (string, error)
}

type InputFilterProxy struct {
	remote binder.IBinder
}

func NewInputFilterProxy(
	remote binder.IBinder,
) *InputFilterProxy {
	return &InputFilterProxy{remote: remote}
}

func (p *InputFilterProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IInputFilter = (*InputFilterProxy)(nil)

func (p *InputFilterProxy) IsEnabled(
	ctx context.Context,
) (bool, error) {
	var _result bool
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputFilter)

	_reply, _err := p.remote.Transact(ctx, TransactionIInputFilterIsEnabled, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadBool()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *InputFilterProxy) NotifyKey(
	ctx context.Context,
	event KeyEvent,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputFilter)
	if _err := event.MarshalParcel(_data); _err != nil {
		return _err
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIInputFilterNotifyKey, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *InputFilterProxy) NotifyInputDevicesChanged(
	ctx context.Context,
	deviceInfos []DeviceInfo,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputFilter)
	if deviceInfos == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(deviceInfos)))
		for _, _item := range deviceInfos {
			if _err := _item.MarshalParcel(_data); _err != nil {
				return _err
			}
		}
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIInputFilterNotifyInputDevicesChanged, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *InputFilterProxy) NotifyConfigurationChanged(
	ctx context.Context,
	config InputFilterConfiguration,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputFilter)
	if _err := config.MarshalParcel(_data); _err != nil {
		return _err
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIInputFilterNotifyConfigurationChanged, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *InputFilterProxy) DumpFilter(
	ctx context.Context,
) (string, error) {
	var _result string
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputFilter)

	_reply, _err := p.remote.Transact(ctx, TransactionIInputFilterDumpFilter, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_result, _err = _reply.ReadString16()
	if _err != nil {
		return _result, _err
	}
	return _result, nil
}
