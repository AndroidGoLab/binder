package telecom

import (
	"context"
	net "github.com/xaionaro-go/aidl/android/net"
	androidTelecom "github.com/xaionaro-go/aidl/android/telecom"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorICallRedirectionService = "com.android.internal.telecom.ICallRedirectionService"

const (
	TransactionICallRedirectionServicePlaceCall     = binder.FirstCallTransaction + 0
	TransactionICallRedirectionServiceNotifyTimeout = binder.FirstCallTransaction + 1
)

type ICallRedirectionService interface {
	AsBinder() binder.IBinder
	PlaceCall(ctx context.Context, adapter ICallRedirectionAdapter, handle net.Uri, initialPhoneAccount androidTelecom.PhoneAccountHandle, allowInteractiveResponse bool) error
	NotifyTimeout(ctx context.Context) error
}

type CallRedirectionServiceProxy struct {
	remote binder.IBinder
}

func NewCallRedirectionServiceProxy(
	remote binder.IBinder,
) *CallRedirectionServiceProxy {
	return &CallRedirectionServiceProxy{remote: remote}
}

func (p *CallRedirectionServiceProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ ICallRedirectionService = (*CallRedirectionServiceProxy)(nil)

func (p *CallRedirectionServiceProxy) PlaceCall(
	ctx context.Context,
	adapter ICallRedirectionAdapter,
	handle net.Uri,
	initialPhoneAccount androidTelecom.PhoneAccountHandle,
	allowInteractiveResponse bool,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorICallRedirectionService)
	_data.WriteStrongBinder(adapter.AsBinder().Handle())
	if _err := handle.MarshalParcel(_data); _err != nil {
		return _err
	}
	if _err := initialPhoneAccount.MarshalParcel(_data); _err != nil {
		return _err
	}
	_data.WriteBool(allowInteractiveResponse)

	_, _err := p.remote.Transact(ctx, TransactionICallRedirectionServicePlaceCall, binder.FlagOneway, _data)
	return _err
}

func (p *CallRedirectionServiceProxy) NotifyTimeout(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorICallRedirectionService)

	_, _err := p.remote.Transact(ctx, TransactionICallRedirectionServiceNotifyTimeout, binder.FlagOneway, _data)
	return _err
}
