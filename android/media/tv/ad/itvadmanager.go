package ad

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	graphics "github.com/xaionaro-go/aidl/android/graphics"
	tv "github.com/xaionaro-go/aidl/android/media/tv"
	net "github.com/xaionaro-go/aidl/android/net"
	os "github.com/xaionaro-go/aidl/android/os"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorITvAdManager = "android.media.tv.ad.ITvAdManager"

const (
	TransactionITvAdManagerGetTvAdServiceList       = binder.FirstCallTransaction + 0
	TransactionITvAdManagerSendAppLinkCommand       = binder.FirstCallTransaction + 1
	TransactionITvAdManagerCreateSession            = binder.FirstCallTransaction + 2
	TransactionITvAdManagerReleaseSession           = binder.FirstCallTransaction + 3
	TransactionITvAdManagerStartAdService           = binder.FirstCallTransaction + 4
	TransactionITvAdManagerStopAdService            = binder.FirstCallTransaction + 5
	TransactionITvAdManagerResetAdService           = binder.FirstCallTransaction + 6
	TransactionITvAdManagerSetSurface               = binder.FirstCallTransaction + 7
	TransactionITvAdManagerDispatchSurfaceChanged   = binder.FirstCallTransaction + 8
	TransactionITvAdManagerSendCurrentVideoBounds   = binder.FirstCallTransaction + 9
	TransactionITvAdManagerSendCurrentChannelUri    = binder.FirstCallTransaction + 10
	TransactionITvAdManagerSendTrackInfoList        = binder.FirstCallTransaction + 11
	TransactionITvAdManagerSendCurrentTvInputId     = binder.FirstCallTransaction + 12
	TransactionITvAdManagerSendSigningResult        = binder.FirstCallTransaction + 13
	TransactionITvAdManagerNotifyError              = binder.FirstCallTransaction + 14
	TransactionITvAdManagerNotifyTvMessage          = binder.FirstCallTransaction + 15
	TransactionITvAdManagerRegisterCallback         = binder.FirstCallTransaction + 16
	TransactionITvAdManagerUnregisterCallback       = binder.FirstCallTransaction + 17
	TransactionITvAdManagerCreateMediaView          = binder.FirstCallTransaction + 18
	TransactionITvAdManagerRelayoutMediaView        = binder.FirstCallTransaction + 19
	TransactionITvAdManagerRemoveMediaView          = binder.FirstCallTransaction + 20
	TransactionITvAdManagerNotifyTvInputSessionData = binder.FirstCallTransaction + 21
)

type ITvAdManager interface {
	AsBinder() binder.IBinder
	GetTvAdServiceList(ctx context.Context, userId int32) ([]TvAdServiceInfo, error)
	SendAppLinkCommand(ctx context.Context, serviceId string, command os.Bundle, userId int32) error
	CreateSession(ctx context.Context, client ITvAdClient, serviceId string, type_ string, seq int32, userId int32) error
	ReleaseSession(ctx context.Context, sessionToken binder.IBinder, userId int32) error
	StartAdService(ctx context.Context, sessionToken binder.IBinder, userId int32) error
	StopAdService(ctx context.Context, sessionToken binder.IBinder, userId int32) error
	ResetAdService(ctx context.Context, sessionToken binder.IBinder, userId int32) error
	SetSurface(ctx context.Context, sessionToken binder.IBinder, surface interface{}, userId int32) error
	DispatchSurfaceChanged(ctx context.Context, sessionToken binder.IBinder, format int32, width int32, height int32, userId int32) error
	SendCurrentVideoBounds(ctx context.Context, sessionToken binder.IBinder, bounds graphics.Rect, userId int32) error
	SendCurrentChannelUri(ctx context.Context, sessionToken binder.IBinder, channelUri net.Uri, userId int32) error
	SendTrackInfoList(ctx context.Context, sessionToken binder.IBinder, tracks []tv.TvTrackInfo, userId int32) error
	SendCurrentTvInputId(ctx context.Context, sessionToken binder.IBinder, inputId string, userId int32) error
	SendSigningResult(ctx context.Context, sessionToken binder.IBinder, signingId string, result []byte, userId int32) error
	NotifyError(ctx context.Context, sessionToken binder.IBinder, errMsg string, params os.Bundle, userId int32) error
	NotifyTvMessage(ctx context.Context, sessionToken binder.IBinder, type_ int32, data os.Bundle, userId int32) error
	RegisterCallback(ctx context.Context, callback ITvAdManagerCallback, userId int32) error
	UnregisterCallback(ctx context.Context, callback ITvAdManagerCallback, userId int32) error
	CreateMediaView(ctx context.Context, sessionToken binder.IBinder, windowToken binder.IBinder, frame graphics.Rect, userId int32) error
	RelayoutMediaView(ctx context.Context, sessionToken binder.IBinder, frame graphics.Rect, userId int32) error
	RemoveMediaView(ctx context.Context, sessionToken binder.IBinder, userId int32) error
	NotifyTvInputSessionData(ctx context.Context, sessionToken binder.IBinder, type_ string, data os.Bundle, userId int32) error
}

type TvAdManagerProxy struct {
	remote binder.IBinder
}

func NewTvAdManagerProxy(
	remote binder.IBinder,
) *TvAdManagerProxy {
	return &TvAdManagerProxy{remote: remote}
}

func (p *TvAdManagerProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ ITvAdManager = (*TvAdManagerProxy)(nil)

func (p *TvAdManagerProxy) GetTvAdServiceList(
	ctx context.Context,
	userId int32,
) ([]TvAdServiceInfo, error) {
	var _result []TvAdServiceInfo
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerGetTvAdServiceList, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_count, _err := _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}

	if _count >= 0 {
		_result = make([]TvAdServiceInfo, _count)
		for _i := int32(0); _i < _count; _i++ {
			if _err = _result[_i].UnmarshalParcel(_reply); _err != nil {
				return _result, _err
			}
		}
	}
	return _result, nil
}

func (p *TvAdManagerProxy) SendAppLinkCommand(
	ctx context.Context,
	serviceId string,
	command os.Bundle,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteString16(serviceId)
	if _err := command.MarshalParcel(_data); _err != nil {
		return _err
	}
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerSendAppLinkCommand, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) CreateSession(
	ctx context.Context,
	client ITvAdClient,
	serviceId string,
	type_ string,
	seq int32,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(client.AsBinder().Handle())
	_data.WriteString16(serviceId)
	_data.WriteString16(type_)
	_data.WriteInt32(seq)
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerCreateSession, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) ReleaseSession(
	ctx context.Context,
	sessionToken binder.IBinder,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerReleaseSession, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) StartAdService(
	ctx context.Context,
	sessionToken binder.IBinder,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerStartAdService, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) StopAdService(
	ctx context.Context,
	sessionToken binder.IBinder,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerStopAdService, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) ResetAdService(
	ctx context.Context,
	sessionToken binder.IBinder,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerResetAdService, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) SetSurface(
	ctx context.Context,
	sessionToken binder.IBinder,
	surface interface{},
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerSetSurface, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) DispatchSurfaceChanged(
	ctx context.Context,
	sessionToken binder.IBinder,
	format int32,
	width int32,
	height int32,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	_data.WriteInt32(format)
	_data.WriteInt32(width)
	_data.WriteInt32(height)
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerDispatchSurfaceChanged, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) SendCurrentVideoBounds(
	ctx context.Context,
	sessionToken binder.IBinder,
	bounds graphics.Rect,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	if _err := bounds.MarshalParcel(_data); _err != nil {
		return _err
	}
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerSendCurrentVideoBounds, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) SendCurrentChannelUri(
	ctx context.Context,
	sessionToken binder.IBinder,
	channelUri net.Uri,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	if _err := channelUri.MarshalParcel(_data); _err != nil {
		return _err
	}
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerSendCurrentChannelUri, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) SendTrackInfoList(
	ctx context.Context,
	sessionToken binder.IBinder,
	tracks []tv.TvTrackInfo,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	if tracks == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(tracks)))
		for _, _item := range tracks {
			if _err := _item.MarshalParcel(_data); _err != nil {
				return _err
			}
		}
	}
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerSendTrackInfoList, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) SendCurrentTvInputId(
	ctx context.Context,
	sessionToken binder.IBinder,
	inputId string,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	_data.WriteString16(inputId)
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerSendCurrentTvInputId, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) SendSigningResult(
	ctx context.Context,
	sessionToken binder.IBinder,
	signingId string,
	result []byte,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	_data.WriteString16(signingId)
	if result == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(result)))
		for _, _item := range result {
			_data.WritePaddedByte(_item)
		}
	}
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerSendSigningResult, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) NotifyError(
	ctx context.Context,
	sessionToken binder.IBinder,
	errMsg string,
	params os.Bundle,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	_data.WriteString16(errMsg)
	if _err := params.MarshalParcel(_data); _err != nil {
		return _err
	}
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerNotifyError, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) NotifyTvMessage(
	ctx context.Context,
	sessionToken binder.IBinder,
	type_ int32,
	data os.Bundle,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	_data.WriteInt32(type_)
	if _err := data.MarshalParcel(_data); _err != nil {
		return _err
	}
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerNotifyTvMessage, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) RegisterCallback(
	ctx context.Context,
	callback ITvAdManagerCallback,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(callback.AsBinder().Handle())
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerRegisterCallback, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) UnregisterCallback(
	ctx context.Context,
	callback ITvAdManagerCallback,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(callback.AsBinder().Handle())
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerUnregisterCallback, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) CreateMediaView(
	ctx context.Context,
	sessionToken binder.IBinder,
	windowToken binder.IBinder,
	frame graphics.Rect,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	_data.WriteStrongBinder(windowToken.Handle())
	if _err := frame.MarshalParcel(_data); _err != nil {
		return _err
	}
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerCreateMediaView, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) RelayoutMediaView(
	ctx context.Context,
	sessionToken binder.IBinder,
	frame graphics.Rect,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	if _err := frame.MarshalParcel(_data); _err != nil {
		return _err
	}
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerRelayoutMediaView, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) RemoveMediaView(
	ctx context.Context,
	sessionToken binder.IBinder,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerRemoveMediaView, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TvAdManagerProxy) NotifyTvInputSessionData(
	ctx context.Context,
	sessionToken binder.IBinder,
	type_ string,
	data os.Bundle,
	userId int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITvAdManager)
	_data.WriteStrongBinder(sessionToken.Handle())
	_data.WriteString16(type_)
	if _err := data.MarshalParcel(_data); _err != nil {
		return _err
	}
	_data.WriteInt32(userId)

	_reply, _err := p.remote.Transact(ctx, TransactionITvAdManagerNotifyTvInputSessionData, 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
