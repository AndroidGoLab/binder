package core

import (
	"context"
	coreITelephony "github.com/xaionaro-go/aidl/android/hardware/audio/core/ITelephony"
	common "github.com/xaionaro-go/aidl/android/media/audio/common"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorITelephony = "android.hardware.audio.core.ITelephony"

const (
	TransactionITelephonyGetSupportedAudioModes = binder.FirstCallTransaction + 0
	TransactionITelephonySwitchAudioMode        = binder.FirstCallTransaction + 1
	TransactionITelephonySetTelecomConfig       = binder.FirstCallTransaction + 2
)

type ITelephony interface {
	AsBinder() binder.IBinder
	GetSupportedAudioModes(ctx context.Context) ([]common.AudioMode, error)
	SwitchAudioMode(ctx context.Context, mode common.AudioMode) error
	SetTelecomConfig(ctx context.Context, config coreITelephony.TelecomConfig) (coreITelephony.TelecomConfig, error)
}

type TelephonyProxy struct {
	remote binder.IBinder
}

func NewTelephonyProxy(
	remote binder.IBinder,
) *TelephonyProxy {
	return &TelephonyProxy{remote: remote}
}

func (p *TelephonyProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ ITelephony = (*TelephonyProxy)(nil)

func (p *TelephonyProxy) GetSupportedAudioModes(
	ctx context.Context,
) ([]common.AudioMode, error) {
	var _result []common.AudioMode
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITelephony)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorITelephony, "getSupportedAudioModes"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_count, _err := _reply.ReadInt32()
	if _err != nil {
		return _result, _err
	}

	if _count >= 0 {
		_result = make([]common.AudioMode, _count)
		for _i := int32(0); _i < _count; _i++ {
			_raw, _err := _reply.ReadInt32()
			if _err != nil {
				return _result, _err
			}
			_result[_i] = common.AudioMode(_raw)
		}
	}
	return _result, nil
}

func (p *TelephonyProxy) SwitchAudioMode(
	ctx context.Context,
	mode common.AudioMode,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITelephony)
	_data.WriteInt32(int32(mode))

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorITelephony, "switchAudioMode"), 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *TelephonyProxy) SetTelecomConfig(
	ctx context.Context,
	config coreITelephony.TelecomConfig,
) (coreITelephony.TelecomConfig, error) {
	var _result coreITelephony.TelecomConfig
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorITelephony)
	if _err := config.MarshalParcel(_data); _err != nil {
		return _result, _err
	}

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorITelephony, "setTelecomConfig"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	if _err = _result.UnmarshalParcel(_reply); _err != nil {
		return _result, _err
	}
	return _result, nil
}
