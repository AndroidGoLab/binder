package drm

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIDrmFactory = "android.hardware.drm.IDrmFactory"

const (
	TransactionIDrmFactoryCreateDrmPlugin           = binder.FirstCallTransaction + 0
	TransactionIDrmFactoryCreateCryptoPlugin        = binder.FirstCallTransaction + 1
	TransactionIDrmFactoryGetSupportedCryptoSchemes = binder.FirstCallTransaction + 2
)

type IDrmFactory interface {
	AsBinder() binder.IBinder
	CreateDrmPlugin(ctx context.Context, uuid Uuid, appPackageName string) (IDrmPlugin, error)
	CreateCryptoPlugin(ctx context.Context, uuid Uuid, initData []byte) (ICryptoPlugin, error)
	GetSupportedCryptoSchemes(ctx context.Context) (CryptoSchemes, error)
}

type DrmFactoryProxy struct {
	remote binder.IBinder
}

func NewDrmFactoryProxy(
	remote binder.IBinder,
) *DrmFactoryProxy {
	return &DrmFactoryProxy{remote: remote}
}

func (p *DrmFactoryProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IDrmFactory = (*DrmFactoryProxy)(nil)

func (p *DrmFactoryProxy) CreateDrmPlugin(
	ctx context.Context,
	uuid Uuid,
	appPackageName string,
) (IDrmPlugin, error) {
	var _result IDrmPlugin
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIDrmFactory)
	if _err := uuid.MarshalParcel(_data); _err != nil {
		return _result, _err
	}
	_data.WriteString16(appPackageName)

	_reply, _err := p.remote.Transact(ctx, TransactionIDrmFactoryCreateDrmPlugin, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_handle, _err := _reply.ReadStrongBinder()
	if _err != nil {
		return _result, _err
	}
	_result = NewDrmPluginProxy(binder.NewProxyBinder(p.remote.Transport(), _handle))
	return _result, nil
}

func (p *DrmFactoryProxy) CreateCryptoPlugin(
	ctx context.Context,
	uuid Uuid,
	initData []byte,
) (ICryptoPlugin, error) {
	var _result ICryptoPlugin
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIDrmFactory)
	if _err := uuid.MarshalParcel(_data); _err != nil {
		return _result, _err
	}
	if initData == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(initData)))
		for _, _item := range initData {
			_data.WritePaddedByte(_item)
		}
	}

	_reply, _err := p.remote.Transact(ctx, TransactionIDrmFactoryCreateCryptoPlugin, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	_handle, _err := _reply.ReadStrongBinder()
	if _err != nil {
		return _result, _err
	}
	_result = NewCryptoPluginProxy(binder.NewProxyBinder(p.remote.Transport(), _handle))
	return _result, nil
}

func (p *DrmFactoryProxy) GetSupportedCryptoSchemes(
	ctx context.Context,
) (CryptoSchemes, error) {
	var _result CryptoSchemes
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIDrmFactory)

	_reply, _err := p.remote.Transact(ctx, TransactionIDrmFactoryGetSupportedCryptoSchemes, 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	if _err = _result.UnmarshalParcel(_reply); _err != nil {
		return _result, _err
	}
	return _result, nil
}
