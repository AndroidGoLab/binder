package biometrics

import (
	"github.com/AndroidGoLab/binder/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

type PromptInfo struct {
	LogoRes                             int32
	LogoDescription                     string
	UseDefaultTitle                     bool
	UseDefaultSubtitle                  bool
	ConfirmationRequested               bool
	DeviceCredentialAllowed             bool
	Authenticators                      int32
	DisallowBiometricsIfPolicyExists    bool
	ReceiveSystemEvents                 bool
	AllowBackgroundAuthentication       bool
	IgnoreEnrollmentState               bool
	IsForLegacyFingerprintManager       bool
	ShowEmergencyCallButton             bool
	UseParentProfileForDeviceCredential bool
}

var _ parcel.Parcelable = (*PromptInfo)(nil)

func (s *PromptInfo) MarshalParcel(
	p *parcel.Parcel,
) error {
	p.WriteInt32(s.LogoRes)
	p.WriteInt32(0) // null LogoBitmap
	p.WriteString16(s.LogoDescription)
	p.WriteInt32(-1) // null Title
	p.WriteBool(s.UseDefaultTitle)
	p.WriteInt32(-1) // null Subtitle
	p.WriteBool(s.UseDefaultSubtitle)
	p.WriteInt32(-1) // null Description
	p.WriteInt32(0)  // null ContentView
	p.WriteInt32(-1) // null DeviceCredentialTitle
	p.WriteInt32(-1) // null DeviceCredentialSubtitle
	p.WriteInt32(-1) // null DeviceCredentialDescription
	p.WriteInt32(-1) // null NegativeButtonText
	p.WriteBool(s.ConfirmationRequested)
	p.WriteBool(s.DeviceCredentialAllowed)
	p.WriteInt32(s.Authenticators)
	p.WriteBool(s.DisallowBiometricsIfPolicyExists)
	p.WriteBool(s.ReceiveSystemEvents)
	p.WriteInt32(-1) // null AllowedSensorIds
	p.WriteBool(s.AllowBackgroundAuthentication)
	p.WriteBool(s.IgnoreEnrollmentState)
	p.WriteBool(s.IsForLegacyFingerprintManager)
	p.WriteBool(s.ShowEmergencyCallButton)
	p.WriteBool(s.UseParentProfileForDeviceCredential)
	return nil
}

func (s *PromptInfo) UnmarshalParcel(
	p *parcel.Parcel,
) error {
	var _err error
	s.LogoRes, _err = p.ReadInt32()
	if _err != nil {
		return _err
	}
	{
		_opaqueFlag, _opaqueErr := p.ReadInt32()
		if _opaqueErr != nil {
			return _opaqueErr
		}
		if _opaqueFlag != 0 {
			return nil // non-null LogoBitmap: cannot skip unknown-size typed object
		}
	}
	s.LogoDescription, _err = p.ReadString16()
	if _err != nil {
		return _err
	}
	return nil // opaque Title: cannot skip without known wire format
	s.UseDefaultTitle, _err = p.ReadBool()
	if _err != nil {
		return _err
	}
	return nil // opaque Subtitle: cannot skip without known wire format
	s.UseDefaultSubtitle, _err = p.ReadBool()
	if _err != nil {
		return _err
	}
	return nil // opaque Description: cannot skip without known wire format
	{
		_opaqueFlag, _opaqueErr := p.ReadInt32()
		if _opaqueErr != nil {
			return _opaqueErr
		}
		if _opaqueFlag != 0 {
			return nil // non-null ContentView: cannot skip unknown-size typed object
		}
	}
	return nil // opaque DeviceCredentialTitle: cannot skip without known wire format
	return nil // opaque DeviceCredentialSubtitle: cannot skip without known wire format
	return nil // opaque DeviceCredentialDescription: cannot skip without known wire format
	return nil // opaque NegativeButtonText: cannot skip without known wire format
	s.ConfirmationRequested, _err = p.ReadBool()
	if _err != nil {
		return _err
	}
	s.DeviceCredentialAllowed, _err = p.ReadBool()
	if _err != nil {
		return _err
	}
	s.Authenticators, _err = p.ReadInt32()
	if _err != nil {
		return _err
	}
	s.DisallowBiometricsIfPolicyExists, _err = p.ReadBool()
	if _err != nil {
		return _err
	}
	s.ReceiveSystemEvents, _err = p.ReadBool()
	if _err != nil {
		return _err
	}
	return nil // opaque AllowedSensorIds: cannot skip without known wire format
	s.AllowBackgroundAuthentication, _err = p.ReadBool()
	if _err != nil {
		return _err
	}
	s.IgnoreEnrollmentState, _err = p.ReadBool()
	if _err != nil {
		return _err
	}
	s.IsForLegacyFingerprintManager, _err = p.ReadBool()
	if _err != nil {
		return _err
	}
	s.ShowEmergencyCallButton, _err = p.ReadBool()
	if _err != nil {
		return _err
	}
	s.UseParentProfileForDeviceCredential, _err = p.ReadBool()
	if _err != nil {
		return _err
	}
	return nil
}
