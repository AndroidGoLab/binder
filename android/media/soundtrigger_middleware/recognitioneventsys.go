package soundtrigger_middleware

import (
	"github.com/xaionaro-go/aidl/binder"
	soundtrigger "github.com/xaionaro-go/aidl/android/hardware/soundtrigger"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

type RecognitionEventSys struct {
	RecognitionEvent       soundtrigger.SoundTriggerRecognitionEvent
	HalEventReceivedMillis int64
	Token                  binder.IBinder
}

var _ parcel.Parcelable = (*RecognitionEventSys)(nil)

func (s *RecognitionEventSys) MarshalParcel(
	p *parcel.Parcel,
) error {
	_headerPos := parcel.WriteParcelableHeader(p)
	if _err := s.RecognitionEvent.MarshalParcel(p); _err != nil {
		return _err
	}
	p.WriteInt64(s.HalEventReceivedMillis)
	p.WriteStrongBinder(s.Token.Handle())

	parcel.WriteParcelableFooter(p, _headerPos)
	return nil
}

func (s *RecognitionEventSys) UnmarshalParcel(
	p *parcel.Parcel,
) error {
	_endPos, _err := parcel.ReadParcelableHeader(p)
	if _err != nil {
		return _err
	}

	if _err = s.RecognitionEvent.UnmarshalParcel(p); _err != nil {
		return _err
	}

	s.HalEventReceivedMillis, _err = p.ReadInt64()
	if _err != nil {
		return _err
	}

	_tokenHandle, _err := p.ReadStrongBinder()
	if _err != nil {
		return _err
	}
	s.Token = binder.NewProxyBinder(nil, _tokenHandle)

	parcel.SkipToParcelableEnd(p, _endPos)
	return nil
}
