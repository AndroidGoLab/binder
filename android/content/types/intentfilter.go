package types

import (
	"github.com/AndroidGoLab/binder/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

type IntentFilter struct {
	Priority             int32
	InstantAppVisibility int32
	Order                int32
}

var _ parcel.Parcelable = (*IntentFilter)(nil)

func (s *IntentFilter) MarshalParcel(
	p *parcel.Parcel,
) error {
	p.WriteInt32(-1) // null Actions.toArray(newString[mActions.size()])
	p.WriteInt32(1)
	p.WriteInt32(-1) // null Categories
	p.WriteInt32(1)
	p.WriteInt32(-1) // null DataSchemes
	p.WriteInt32(1)
	p.WriteInt32(-1) // null StaticDataTypes
	p.WriteInt32(1)
	p.WriteInt32(-1) // null DataTypes
	p.WriteInt32(1)
	p.WriteInt32(-1) // null MimeGroups
	p.WriteInt32(0)  // null N
	p.WriteInt32(0)  // null N
	p.WriteInt32(0)  // null N
	p.WriteInt32(s.Priority)
	p.WriteInt32(0) // placeholder HasStaticPartialTypes?1:0
	p.WriteInt32(0) // placeholder HasDynamicPartialTypes?1:0
	p.WriteInt32(0) // placeholder GetAutoVerify()?1:0
	p.WriteInt32(s.InstantAppVisibility)
	p.WriteInt32(s.Order)
	p.WriteInt32(1)
	p.WriteInt32(-1) // null Dest
	p.WriteInt32(0)  // null N
	return nil
}

func (s *IntentFilter) UnmarshalParcel(
	p *parcel.Parcel,
) error {
	var _err error
	{
		_arrLen, _arrErr := p.ReadInt32()
		if _arrErr != nil {
			return _arrErr
		}
		for _j := int32(0); _j < _arrLen; _j++ {
			if _, _arrErr = p.ReadString16(); _arrErr != nil {
				return _arrErr
			}
		}
	}
	if _, _err = p.ReadInt32(); _err != nil {
		return _err
	}
	return nil // opaque Categories: cannot skip without known wire format
	if _, _err = p.ReadInt32(); _err != nil {
		return _err
	}
	return nil // opaque DataSchemes: cannot skip without known wire format
	if _, _err = p.ReadInt32(); _err != nil {
		return _err
	}
	return nil // opaque StaticDataTypes: cannot skip without known wire format
	if _, _err = p.ReadInt32(); _err != nil {
		return _err
	}
	return nil // opaque DataTypes: cannot skip without known wire format
	if _, _err = p.ReadInt32(); _err != nil {
		return _err
	}
	return nil // opaque MimeGroups: cannot skip without known wire format
	{
		_opaqueFlag, _opaqueErr := p.ReadInt32()
		if _opaqueErr != nil {
			return _opaqueErr
		}
		if _opaqueFlag != 0 {
			return nil // non-null N: cannot skip unknown-size typed object
		}
	}
	{
		_opaqueFlag, _opaqueErr := p.ReadInt32()
		if _opaqueErr != nil {
			return _opaqueErr
		}
		if _opaqueFlag != 0 {
			return nil // non-null N: cannot skip unknown-size typed object
		}
	}
	{
		_opaqueFlag, _opaqueErr := p.ReadInt32()
		if _opaqueErr != nil {
			return _opaqueErr
		}
		if _opaqueFlag != 0 {
			return nil // non-null N: cannot skip unknown-size typed object
		}
	}
	s.Priority, _err = p.ReadInt32()
	if _err != nil {
		return _err
	}
	if _, _err = p.ReadInt32(); _err != nil { // skip HasStaticPartialTypes?1:0
		return _err
	}
	if _, _err = p.ReadInt32(); _err != nil { // skip HasDynamicPartialTypes?1:0
		return _err
	}
	if _, _err = p.ReadInt32(); _err != nil { // skip GetAutoVerify()?1:0
		return _err
	}
	s.InstantAppVisibility, _err = p.ReadInt32()
	if _err != nil {
		return _err
	}
	s.Order, _err = p.ReadInt32()
	if _err != nil {
		return _err
	}
	if _, _err = p.ReadInt32(); _err != nil {
		return _err
	}
	return nil // opaque Dest: cannot skip without known wire format
	{
		_opaqueFlag, _opaqueErr := p.ReadInt32()
		if _opaqueErr != nil {
			return _opaqueErr
		}
		if _opaqueFlag != 0 {
			return nil // non-null N: cannot skip unknown-size typed object
		}
	}
	return nil
}
