package rkp

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIRegistration = "android.security.rkp.IRegistration"

const (
	TransactionIRegistrationGetKey                = binder.FirstCallTransaction + 0
	TransactionIRegistrationCancelGetKey          = binder.FirstCallTransaction + 1
	TransactionIRegistrationStoreUpgradedKeyAsync = binder.FirstCallTransaction + 2
)

type IRegistration interface {
	AsBinder() binder.IBinder
	GetKey(ctx context.Context, keyId int32, callback IGetKeyCallback) error
	CancelGetKey(ctx context.Context, callback IGetKeyCallback) error
	StoreUpgradedKeyAsync(ctx context.Context, oldKeyBlob []byte, newKeyBlob []byte, callback IStoreUpgradedKeyCallback) error
}

type RegistrationProxy struct {
	remote binder.IBinder
}

func NewRegistrationProxy(
	remote binder.IBinder,
) *RegistrationProxy {
	return &RegistrationProxy{remote: remote}
}

func (p *RegistrationProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IRegistration = (*RegistrationProxy)(nil)

func (p *RegistrationProxy) GetKey(
	ctx context.Context,
	keyId int32,
	callback IGetKeyCallback,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIRegistration)
	_data.WriteInt32(keyId)
	_data.WriteStrongBinder(callback.AsBinder().Handle())

	_, _err := p.remote.Transact(ctx, TransactionIRegistrationGetKey, binder.FlagOneway, _data)
	return _err
}

func (p *RegistrationProxy) CancelGetKey(
	ctx context.Context,
	callback IGetKeyCallback,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIRegistration)
	_data.WriteStrongBinder(callback.AsBinder().Handle())

	_, _err := p.remote.Transact(ctx, TransactionIRegistrationCancelGetKey, binder.FlagOneway, _data)
	return _err
}

func (p *RegistrationProxy) StoreUpgradedKeyAsync(
	ctx context.Context,
	oldKeyBlob []byte,
	newKeyBlob []byte,
	callback IStoreUpgradedKeyCallback,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIRegistration)
	if oldKeyBlob == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(oldKeyBlob)))
		for _, _item := range oldKeyBlob {
			_data.WritePaddedByte(_item)
		}
	}
	if newKeyBlob == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(newKeyBlob)))
		for _, _item := range newKeyBlob {
			_data.WritePaddedByte(_item)
		}
	}
	_data.WriteStrongBinder(callback.AsBinder().Handle())

	_, _err := p.remote.Transact(ctx, TransactionIRegistrationStoreUpgradedKeyAsync, binder.FlagOneway, _data)
	return _err
}
