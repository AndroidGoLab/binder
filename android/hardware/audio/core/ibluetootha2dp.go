...
func (p *BluetoothA2dpProxy) SetEnabled(
	...
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIBluetoothA2dp)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIBluetoothA2dp, MethodIBluetoothA2dpSetEnabled)
...
func (p *BluetoothA2dpProxy) ReconfigureOffload(
	...
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIBluetoothA2dp)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIBluetoothA2dp, MethodIBluetoothA2dpReconfigureOffload)
	_compiledDescs := []string{
