package audio

import (
	"context"
	common "github.com/xaionaro-go/aidl/android/hardware/audio/common"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIBluetoothAudioPort = "android.hardware.bluetooth.audio.IBluetoothAudioPort"

const (
	TransactionIBluetoothAudioPortGetPresentationPosition = binder.FirstCallTransaction + 0
	TransactionIBluetoothAudioPortStartStream             = binder.FirstCallTransaction + 1
	TransactionIBluetoothAudioPortStopStream              = binder.FirstCallTransaction + 2
	TransactionIBluetoothAudioPortSuspendStream           = binder.FirstCallTransaction + 3
	TransactionIBluetoothAudioPortUpdateSourceMetadata    = binder.FirstCallTransaction + 4
	TransactionIBluetoothAudioPortUpdateSinkMetadata      = binder.FirstCallTransaction + 5
	TransactionIBluetoothAudioPortSetLatencyMode          = binder.FirstCallTransaction + 6
)

type IBluetoothAudioPort interface {
	AsBinder() binder.IBinder
	GetPresentationPosition(ctx context.Context) (PresentationPosition, error)
	StartStream(ctx context.Context, isLowLatency bool) error
	StopStream(ctx context.Context) error
	SuspendStream(ctx context.Context) error
	UpdateSourceMetadata(ctx context.Context, sourceMetadata common.SourceMetadata) error
	UpdateSinkMetadata(ctx context.Context, sinkMetadata common.SinkMetadata) error
	SetLatencyMode(ctx context.Context, latencyMode LatencyMode) error
}

type BluetoothAudioPortProxy struct {
	remote binder.IBinder
}

func NewBluetoothAudioPortProxy(
	remote binder.IBinder,
) *BluetoothAudioPortProxy {
	return &BluetoothAudioPortProxy{remote: remote}
}

func (p *BluetoothAudioPortProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IBluetoothAudioPort = (*BluetoothAudioPortProxy)(nil)

func (p *BluetoothAudioPortProxy) GetPresentationPosition(
	ctx context.Context,
) (PresentationPosition, error) {
	var _result PresentationPosition
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIBluetoothAudioPort)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIBluetoothAudioPort, "getPresentationPosition"), 0, _data)
	if _err != nil {
		return _result, _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _result, _err
	}

	if _err = _result.UnmarshalParcel(_reply); _err != nil {
		return _result, _err
	}
	return _result, nil
}

func (p *BluetoothAudioPortProxy) StartStream(
	ctx context.Context,
	isLowLatency bool,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIBluetoothAudioPort)
	_data.WriteBool(isLowLatency)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIBluetoothAudioPort, "startStream"), 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *BluetoothAudioPortProxy) StopStream(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIBluetoothAudioPort)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIBluetoothAudioPort, "stopStream"), 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *BluetoothAudioPortProxy) SuspendStream(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIBluetoothAudioPort)

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIBluetoothAudioPort, "suspendStream"), 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *BluetoothAudioPortProxy) UpdateSourceMetadata(
	ctx context.Context,
	sourceMetadata common.SourceMetadata,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIBluetoothAudioPort)
	if _err := sourceMetadata.MarshalParcel(_data); _err != nil {
		return _err
	}

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIBluetoothAudioPort, "updateSourceMetadata"), 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *BluetoothAudioPortProxy) UpdateSinkMetadata(
	ctx context.Context,
	sinkMetadata common.SinkMetadata,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIBluetoothAudioPort)
	if _err := sinkMetadata.MarshalParcel(_data); _err != nil {
		return _err
	}

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIBluetoothAudioPort, "updateSinkMetadata"), 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}

func (p *BluetoothAudioPortProxy) SetLatencyMode(
	ctx context.Context,
	latencyMode LatencyMode,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIBluetoothAudioPort)
	_data.WriteInt32(int32(latencyMode))

	_reply, _err := p.remote.Transact(ctx, p.remote.ResolveCode(DescriptorIBluetoothAudioPort, "setLatencyMode"), 0, _data)
	if _err != nil {
		return _err
	}
	defer _reply.Recycle()

	if _err = binder.ReadStatus(_reply); _err != nil {
		return _err
	}

	return nil
}
