package IGlanceableHubWidgetManagerService

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	model "github.com/xaionaro-go/aidl/com/android/systemui/communal/shared/model"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIGlanceableHubWidgetsListener = "com.android.systemui.communal.widgets.IGlanceableHubWidgetManagerService.IGlanceableHubWidgetsListener"

const (
	TransactionIGlanceableHubWidgetsListenerOnWidgetsUpdated = binder.FirstCallTransaction + 0
)

type IGlanceableHubWidgetsListener interface {
	AsBinder() binder.IBinder
	OnWidgetsUpdated(ctx context.Context, widgets []model.CommunalWidgetContentModel) error
}

type GlanceableHubWidgetsListenerProxy struct {
	remote binder.IBinder
}

func NewGlanceableHubWidgetsListenerProxy(
	remote binder.IBinder,
) *GlanceableHubWidgetsListenerProxy {
	return &GlanceableHubWidgetsListenerProxy{remote: remote}
}

func (p *GlanceableHubWidgetsListenerProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IGlanceableHubWidgetsListener = (*GlanceableHubWidgetsListenerProxy)(nil)

func (p *GlanceableHubWidgetsListenerProxy) OnWidgetsUpdated(
	ctx context.Context,
	widgets []model.CommunalWidgetContentModel,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIGlanceableHubWidgetsListener)
	if widgets == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(widgets)))
		for _, _item := range widgets {
			if _err := _item.MarshalParcel(_data); _err != nil {
				return _err
			}
		}
	}

	_, _err := p.remote.Transact(ctx, TransactionIGlanceableHubWidgetsListenerOnWidgetsUpdated, binder.FlagOneway, _data)
	return _err
}
