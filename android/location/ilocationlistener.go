package location

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorILocationListener = "android.location.ILocationListener"

const (
	TransactionILocationListenerOnLocationChanged        = binder.FirstCallTransaction + 0
	TransactionILocationListenerOnProviderEnabledChanged = binder.FirstCallTransaction + 1
	TransactionILocationListenerOnFlushComplete          = binder.FirstCallTransaction + 2
)

type ILocationListener interface {
	AsBinder() binder.IBinder
	OnLocationChanged(ctx context.Context, locations []Location, onCompleteCallback *interface{}) error
	OnProviderEnabledChanged(ctx context.Context, provider string, enabled bool) error
	OnFlushComplete(ctx context.Context, requestCode int32) error
}

type LocationListenerProxy struct {
	remote binder.IBinder
}

func NewLocationListenerProxy(
	remote binder.IBinder,
) *LocationListenerProxy {
	return &LocationListenerProxy{remote: remote}
}

func (p *LocationListenerProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ ILocationListener = (*LocationListenerProxy)(nil)

func (p *LocationListenerProxy) OnLocationChanged(
	ctx context.Context,
	locations []Location,
	onCompleteCallback *interface{},
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorILocationListener)
	if locations == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(locations)))
		for _, _item := range locations {
			if _err := _item.MarshalParcel(_data); _err != nil {
				return _err
			}
		}
	}

	_, _err := p.remote.Transact(ctx, TransactionILocationListenerOnLocationChanged, binder.FlagOneway, _data)
	return _err
}

func (p *LocationListenerProxy) OnProviderEnabledChanged(
	ctx context.Context,
	provider string,
	enabled bool,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorILocationListener)
	_data.WriteString16(provider)
	_data.WriteBool(enabled)

	_, _err := p.remote.Transact(ctx, TransactionILocationListenerOnProviderEnabledChanged, binder.FlagOneway, _data)
	return _err
}

func (p *LocationListenerProxy) OnFlushComplete(
	ctx context.Context,
	requestCode int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorILocationListener)
	_data.WriteInt32(requestCode)

	_, _err := p.remote.Transact(ctx, TransactionILocationListenerOnFlushComplete, binder.FlagOneway, _data)
	return _err
}
