...
func (p *RecognitionServiceManagerProxy) CreateSession(
	...
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIRecognitionServiceManager, MethodIRecognitionServiceManagerCreateSession)
...
func (p *RecognitionServiceManagerProxy) SetTemporaryComponent(
	...
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorIRecognitionServiceManager)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorIRecognitionServiceManager, MethodIRecognitionServiceManagerSetTemporaryComponent)
	_compiledDescs := []string{
		"Landroid/content/ComponentName;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
