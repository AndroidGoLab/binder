package uwb

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIUwbClientCallback = "android.hardware.uwb.IUwbClientCallback"

const (
	TransactionIUwbClientCallbackOnUciMessage = binder.FirstCallTransaction + 0
	TransactionIUwbClientCallbackOnHalEvent   = binder.FirstCallTransaction + 1
)

type IUwbClientCallback interface {
	AsBinder() binder.IBinder
	OnUciMessage(ctx context.Context, data []byte) error
	OnHalEvent(ctx context.Context, event UwbEvent, status UwbStatus) error
}

type UwbClientCallbackProxy struct {
	remote binder.IBinder
}

func NewUwbClientCallbackProxy(
	remote binder.IBinder,
) *UwbClientCallbackProxy {
	return &UwbClientCallbackProxy{remote: remote}
}

func (p *UwbClientCallbackProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IUwbClientCallback = (*UwbClientCallbackProxy)(nil)

func (p *UwbClientCallbackProxy) OnUciMessage(
	ctx context.Context,
	data []byte,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIUwbClientCallback)
	if data == nil {
		_data.WriteInt32(-1)
	} else {
		_data.WriteInt32(int32(len(data)))
		for _, _item := range data {
			_data.WritePaddedByte(_item)
		}
	}

	_, _err := p.remote.Transact(ctx, TransactionIUwbClientCallbackOnUciMessage, binder.FlagOneway, _data)
	return _err
}

func (p *UwbClientCallbackProxy) OnHalEvent(
	ctx context.Context,
	event UwbEvent,
	status UwbStatus,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIUwbClientCallback)
	_data.WriteInt32(int32(event))
	_data.WriteInt32(int32(status))

	_, _err := p.remote.Transact(ctx, TransactionIUwbClientCallbackOnHalEvent, binder.FlagOneway, _data)
	return _err
}
