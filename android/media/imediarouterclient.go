package media

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIMediaRouterClient = "android.media.IMediaRouterClient"

const (
	TransactionIMediaRouterClientOnStateChanged       = binder.FirstCallTransaction + 0
	TransactionIMediaRouterClientOnRestoreRoute       = binder.FirstCallTransaction + 1
	TransactionIMediaRouterClientOnGroupRouteSelected = binder.FirstCallTransaction + 2
)

type IMediaRouterClient interface {
	AsBinder() binder.IBinder
	OnStateChanged(ctx context.Context) error
	OnRestoreRoute(ctx context.Context) error
	OnGroupRouteSelected(ctx context.Context, routeId string) error
}

type MediaRouterClientProxy struct {
	remote binder.IBinder
}

func NewMediaRouterClientProxy(
	remote binder.IBinder,
) *MediaRouterClientProxy {
	return &MediaRouterClientProxy{remote: remote}
}

func (p *MediaRouterClientProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IMediaRouterClient = (*MediaRouterClientProxy)(nil)

func (p *MediaRouterClientProxy) OnStateChanged(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIMediaRouterClient)

	_, _err := p.remote.Transact(ctx, TransactionIMediaRouterClientOnStateChanged, binder.FlagOneway, _data)
	return _err
}

func (p *MediaRouterClientProxy) OnRestoreRoute(
	ctx context.Context,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIMediaRouterClient)

	_, _err := p.remote.Transact(ctx, TransactionIMediaRouterClientOnRestoreRoute, binder.FlagOneway, _data)
	return _err
}

func (p *MediaRouterClientProxy) OnGroupRouteSelected(
	ctx context.Context,
	routeId string,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIMediaRouterClient)
	_data.WriteString16(routeId)

	_, _err := p.remote.Transact(ctx, TransactionIMediaRouterClientOnGroupRouteSelected, binder.FlagOneway, _data)
	return _err
}
