...
func (p *TvInputCallbackProxy) Notify(
	...
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorITvInputCallback, MethodITvInputCallbackNotify)
	_compiledDescs := []string{
		"Landroid/hardware/tv/input/TvInputEvent;",
	}
...
func (p *TvInputCallbackProxy) NotifyTvMessageEvent(
	...
	defer _data.Recycle()
	_data.WriteInterfaceToken(DescriptorITvInputCallback)
	_sig := binder.ResolveMethodSignature(p.Remote, ctx, DescriptorITvInputCallback, MethodITvInputCallbackNotifyTvMessageEvent)
	_compiledDescs := []string{
		"Landroid/hardware/tv/input/TvMessageEvent;",
	}
	if _sig == nil || binder.SignatureMatches(_compiledDescs, _sig) {
		_data.WriteInt32(1)
		if _err := event.MarshalParcel(_data); _err != nil {
