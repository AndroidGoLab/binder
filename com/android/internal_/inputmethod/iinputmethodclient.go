package inputmethod

import (
	"context"
	"github.com/xaionaro-go/aidl/binder"
	viewInputmethod "github.com/xaionaro-go/aidl/android/view/inputmethod"
	"github.com/xaionaro-go/aidl/parcel"
)

// Code generated by aidlgen. DO NOT EDIT.

const DescriptorIInputMethodClient = "com.android.internal.inputmethod.IInputMethodClient"

const (
	TransactionIInputMethodClientOnBindMethod                  = binder.FirstCallTransaction + 0
	TransactionIInputMethodClientOnStartInputResult            = binder.FirstCallTransaction + 1
	TransactionIInputMethodClientOnBindAccessibilityService    = binder.FirstCallTransaction + 2
	TransactionIInputMethodClientOnUnbindMethod                = binder.FirstCallTransaction + 3
	TransactionIInputMethodClientOnUnbindAccessibilityService  = binder.FirstCallTransaction + 4
	TransactionIInputMethodClientSetActive                     = binder.FirstCallTransaction + 5
	TransactionIInputMethodClientSetInteractive                = binder.FirstCallTransaction + 6
	TransactionIInputMethodClientSetImeVisibility              = binder.FirstCallTransaction + 7
	TransactionIInputMethodClientScheduleStartInputIfNecessary = binder.FirstCallTransaction + 8
	TransactionIInputMethodClientReportFullscreenMode          = binder.FirstCallTransaction + 9
	TransactionIInputMethodClientSetImeTraceEnabled            = binder.FirstCallTransaction + 10
	TransactionIInputMethodClientThrowExceptionFromSystem      = binder.FirstCallTransaction + 11
)

type IInputMethodClient interface {
	AsBinder() binder.IBinder
	OnBindMethod(ctx context.Context, res InputBindResult) error
	OnStartInputResult(ctx context.Context, res InputBindResult, startInputSeq int32) error
	OnBindAccessibilityService(ctx context.Context, res InputBindResult, id int32) error
	OnUnbindMethod(ctx context.Context, sequence int32, unbindReason int32) error
	OnUnbindAccessibilityService(ctx context.Context, sequence int32, id int32) error
	SetActive(ctx context.Context, active bool, fullscreen bool) error
	SetInteractive(ctx context.Context, active bool, fullscreen bool) error
	SetImeVisibility(ctx context.Context, visible bool, statsToken *viewInputmethod.ImeTrackerToken) error
	ScheduleStartInputIfNecessary(ctx context.Context, fullscreen bool) error
	ReportFullscreenMode(ctx context.Context, fullscreen bool) error
	SetImeTraceEnabled(ctx context.Context, enabled bool) error
	ThrowExceptionFromSystem(ctx context.Context, message string) error
}

type InputMethodClientProxy struct {
	remote binder.IBinder
}

func NewInputMethodClientProxy(
	remote binder.IBinder,
) *InputMethodClientProxy {
	return &InputMethodClientProxy{remote: remote}
}

func (p *InputMethodClientProxy) AsBinder() binder.IBinder {
	return p.remote
}

var _ IInputMethodClient = (*InputMethodClientProxy)(nil)

func (p *InputMethodClientProxy) OnBindMethod(
	ctx context.Context,
	res InputBindResult,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputMethodClient)
	if _err := res.MarshalParcel(_data); _err != nil {
		return _err
	}

	_, _err := p.remote.Transact(ctx, TransactionIInputMethodClientOnBindMethod, binder.FlagOneway, _data)
	return _err
}

func (p *InputMethodClientProxy) OnStartInputResult(
	ctx context.Context,
	res InputBindResult,
	startInputSeq int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputMethodClient)
	if _err := res.MarshalParcel(_data); _err != nil {
		return _err
	}
	_data.WriteInt32(startInputSeq)

	_, _err := p.remote.Transact(ctx, TransactionIInputMethodClientOnStartInputResult, binder.FlagOneway, _data)
	return _err
}

func (p *InputMethodClientProxy) OnBindAccessibilityService(
	ctx context.Context,
	res InputBindResult,
	id int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputMethodClient)
	if _err := res.MarshalParcel(_data); _err != nil {
		return _err
	}
	_data.WriteInt32(id)

	_, _err := p.remote.Transact(ctx, TransactionIInputMethodClientOnBindAccessibilityService, binder.FlagOneway, _data)
	return _err
}

func (p *InputMethodClientProxy) OnUnbindMethod(
	ctx context.Context,
	sequence int32,
	unbindReason int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputMethodClient)
	_data.WriteInt32(sequence)
	_data.WriteInt32(unbindReason)

	_, _err := p.remote.Transact(ctx, TransactionIInputMethodClientOnUnbindMethod, binder.FlagOneway, _data)
	return _err
}

func (p *InputMethodClientProxy) OnUnbindAccessibilityService(
	ctx context.Context,
	sequence int32,
	id int32,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputMethodClient)
	_data.WriteInt32(sequence)
	_data.WriteInt32(id)

	_, _err := p.remote.Transact(ctx, TransactionIInputMethodClientOnUnbindAccessibilityService, binder.FlagOneway, _data)
	return _err
}

func (p *InputMethodClientProxy) SetActive(
	ctx context.Context,
	active bool,
	fullscreen bool,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputMethodClient)
	_data.WriteBool(active)
	_data.WriteBool(fullscreen)

	_, _err := p.remote.Transact(ctx, TransactionIInputMethodClientSetActive, binder.FlagOneway, _data)
	return _err
}

func (p *InputMethodClientProxy) SetInteractive(
	ctx context.Context,
	active bool,
	fullscreen bool,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputMethodClient)
	_data.WriteBool(active)
	_data.WriteBool(fullscreen)

	_, _err := p.remote.Transact(ctx, TransactionIInputMethodClientSetInteractive, binder.FlagOneway, _data)
	return _err
}

func (p *InputMethodClientProxy) SetImeVisibility(
	ctx context.Context,
	visible bool,
	statsToken *viewInputmethod.ImeTrackerToken,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputMethodClient)
	_data.WriteBool(visible)
	if statsToken != nil {
		if _err := (*statsToken).MarshalParcel(_data); _err != nil {
			return _err
		}
	} else {
		_data.WriteInt32(-1)
	}

	_, _err := p.remote.Transact(ctx, TransactionIInputMethodClientSetImeVisibility, binder.FlagOneway, _data)
	return _err
}

func (p *InputMethodClientProxy) ScheduleStartInputIfNecessary(
	ctx context.Context,
	fullscreen bool,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputMethodClient)
	_data.WriteBool(fullscreen)

	_, _err := p.remote.Transact(ctx, TransactionIInputMethodClientScheduleStartInputIfNecessary, binder.FlagOneway, _data)
	return _err
}

func (p *InputMethodClientProxy) ReportFullscreenMode(
	ctx context.Context,
	fullscreen bool,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputMethodClient)
	_data.WriteBool(fullscreen)

	_, _err := p.remote.Transact(ctx, TransactionIInputMethodClientReportFullscreenMode, binder.FlagOneway, _data)
	return _err
}

func (p *InputMethodClientProxy) SetImeTraceEnabled(
	ctx context.Context,
	enabled bool,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputMethodClient)
	_data.WriteBool(enabled)

	_, _err := p.remote.Transact(ctx, TransactionIInputMethodClientSetImeTraceEnabled, binder.FlagOneway, _data)
	return _err
}

func (p *InputMethodClientProxy) ThrowExceptionFromSystem(
	ctx context.Context,
	message string,
) error {
	_data := parcel.New()
	_data.WriteInterfaceToken(DescriptorIInputMethodClient)
	_data.WriteString16(message)

	_, _err := p.remote.Transact(ctx, TransactionIInputMethodClientThrowExceptionFromSystem, binder.FlagOneway, _data)
	return _err
}
